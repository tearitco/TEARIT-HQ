...
> **⚑ 2026-09-02: IMPLEMENTED (first cut).** This design is now built as
> the two-repo pipeline in `../tearit-install/` + `../make-payload.sh` +
> `../payload-src/` (pushed to `tearitco/tearit-install` +
> `tearitco/tearit-hq-payload`). Product name is now **tearit-hq**
> (passed as an argument, not hardcoded — §11 here is superseded on the
> literal names, kept for the shape). §4 payload layout is still broadly
> right; §5.4 build-in-place is exactly what `bootstrap.sh` does. Binary
> names in §4/§6 predate the 2026-09-01 consolidation (no separate
> `khtpm_strip_parser.+x` / `tp_desktop_window*.+x` — one
> `khtpm_core_render.+x` serves both). Below is the original design,
> unchanged.
>
