...
void log_event(const char* msg) {
    ...
    if (asprintf(&path, "%s/pieces/master_ledger/master_ledger.txt", project_root) != -1) {
        FILE *f = fopen(path, "a");
        if (f) { time_t now = time(NULL); fprintf(f, "[%ld] EventFire: %s | Source: collect_op\n", now, msg); fclose(f); }
        free(path);
    }
}
...
void hit_frame_marker() {
    ...
    if (asprintf(&path, "%s/pieces/display/frame_changed.txt", project_root) != -1) {
        FILE *f = fopen(path, "a"); if (f) { fprintf(f, "X\n"); fclose(f); } free(path);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    char *map_path = NULL;
    asprintf(&map_path, "%s/projects/%s/maps/%s", project_root, current_project, active_map_file);
    ...
    if (sy >= 0 && sy < rows && sx >= 0 && sx < (int)strlen(map[sy])) {
        ...
        if (tile == 'T') {
            map[sy][sx] = '.';
            mf = fopen(map_path, "w"); if (mf) { for (int i = 0; i < rows; i++) fputs(map[i], mf); fclose(mf); }
            char hap_s[16]; get_piece_state(piece_id, "happiness", hap_s);
            int hap = atoi(hap_s); char new_hap[16]; sprintf(new_hap, "%d", hap + 10);
            set_piece_state(piece_id, "happiness", new_hap);
            log_event("Treasure Collected!"); printf("Collected Treasure at (%d,%d)\n", sx, sy);
        } else {
            log_event("Nothing to collect here."); printf("Nothing at (%d,%d)\n", sx, sy);
        }
    }

    free(map_path); hit_frame_marker();
    return 0;
}
