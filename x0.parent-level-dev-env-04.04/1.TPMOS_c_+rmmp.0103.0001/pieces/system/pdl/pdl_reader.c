...
static int find_piece_pdl_recursive(const char* dir_path, const char* piece_id, char* out_path, size_t out_size, int depth) {
    DIR *dir;
    struct dirent *entry;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (strcmp(entry->d_name, piece_id) == 0) {
            char pdl_path[4096];
            /* Try piece_id.pdl then piece.pdl */
            snprintf(pdl_path, sizeof(pdl_path), "%s/%s.pdl", child, piece_id);
        ...
        if (find_piece_pdl_recursive(child, piece_id, out_path, out_size, depth + 1)) {
            closedir(dir);
            return 1;
        }
    }
...
static int resolve_piece_pdl_path(const char* piece_id, char* out_path, size_t out_sz,
    ...
    char pieces_root[MAX_PATH];

    /* 0. Try a Wraith-style project_id directly (e.g.
       "wraith-alpha/wraith-projects/piececraft-wraith") -- these are
       full slash-paths under projects/, not a bare directory NAME the
       piece_id recursive search below is built to find, and they use
       project.pdl, not piece.pdl. A cheap single access() check, tried
       first since it's the common case for context-menu's METHOD
       lookups and doesn't require walking any directory tree. Added
       2026-07-13 so pdl_reader.+x can serve as the context-menu
       project's dispatcher against real Wraith project.pdl files
       without duplicating get_method()/list_methods()'s parsing logic
       in a second tool (Bible section 11 reuse). */
    snprintf(out_path, out_sz, "%s/projects/%s/project.pdl", project_root, piece_id);
    ...
    if (find_piece_pdl_recursive(pieces_root, piece_id, out_path, out_sz, 0)) return 0;
