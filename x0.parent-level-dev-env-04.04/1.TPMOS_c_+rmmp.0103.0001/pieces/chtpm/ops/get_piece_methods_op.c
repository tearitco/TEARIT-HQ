...
void resolve_paths() {
    /* 1. Try to find project_root from CWD anchors */
    char cwd[MAX_PATH];
    if (getcwd(cwd, sizeof(cwd))) {
        char *p = cwd;
        while (p && strlen(p) > 1) {
            char pieces_path[MAX_PATH];
            snprintf(pieces_path, sizeof(pieces_path), "%s/pieces", p);
            if (access(pieces_path, F_OK) == 0) {
                strncpy(project_root, p, sizeof(project_root) - 1);
                break;
            }
            p = strrchr(p, '/');
            if (p) *p = '\0';
    ...
    /* 2. Overwrite with location_kvp if available */
    FILE *kvp = fopen("pieces/locations/location_kvp", "r");
    if (!kvp) {
        char kvp_path[MAX_PATH];
        snprintf(kvp_path, sizeof(kvp_path), "%s/pieces/locations/location_kvp", project_root);
        kvp = fopen(kvp_path, "r");
    }
        ...
        while (fgets(line, sizeof(line), kvp)) {
            ...
            if (eq) {
                *eq = '\0';
                ...
                if (strcmp(k, "project_root") == 0 && *v) snprintf(project_root, sizeof(project_root), "%s", v);
            }
        }
        fclose(kvp);
    }
...
char* resolve_pdl_path(const char* active_id, const char* project_id) {
    ...
    snprintf(root_path, sizeof(root_path), "%s/pieces", project_root);
    if (find_pdl_recursive(root_path, active_id, found_path, sizeof(found_path), 0)) {
        return strdup(found_path);
    }

    /* 3. Final Legacy Fallbacks */
    char *path = NULL;
