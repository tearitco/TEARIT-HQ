...
/* Multiple <module> tags per layout (2026-07-20/21 direct instruction,
 * ported here from shared-ops/chtpm_parser_pal.c and
 * 1.muchi-pal-agent's own copy after the same fix landed there first:
 * "we need to modify it so it can run multiple modules... its meant to
 * be like html/js" - i.e. every <module> tag in a page runs
 * concurrently, not just the last one). The FIRST <module> tag in a
 * layout keeps using current_module_pid/current_module_path exactly as
 * before - untouched, zero behavior change for every existing single-
 * module project, including run_module_synchronous()'s use of
 * current_module_path as THE key-dispatch target for one-shot-module
 ...
 * NOT auto-killed on layout switch (unlike the primary slot) since
 * nothing currently using this needs that lifecycle - see
 * launch_extra_module()'s own comment if that's ever needed. Slot count
 * is generous, not a real architectural ceiling - bump
 * MAX_EXTRA_MODULES if a layout ever needs more concurrent modules than
 * that. */
            ...
            if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
                ...
                if (focus_index >= 0 && focus_index < element_count) {
                    ...
                    if (child_el->num_children > 0 && strcmp(child_el->onClick, "ACTIVATE") == 0) {
                        ...
                        if (current_module_path[0]) { cleanup_module(); }
                        focus_index = 0;
                        parse_chtm();
                        initialize_focus();
                        compose_frame();
