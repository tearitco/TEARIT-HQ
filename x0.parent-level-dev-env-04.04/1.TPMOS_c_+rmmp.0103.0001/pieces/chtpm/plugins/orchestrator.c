...
void launch_and_register(const char* name, const char* path, const char* args, bool quiet) {
    ...
    if (pid == 0) {
        ...
        /* RE-WRAITH: Redirect logs for transparency */
        if (strstr(name, "parser")) {
            freopen("parser.log", "a", stdout); freopen("parser.log", "a", stderr);
        } else if (strstr(name, "manager")) {
            freopen("manager.log", "a", stdout); freopen("manager.log", "a", stderr);
        } else if (quiet) {
