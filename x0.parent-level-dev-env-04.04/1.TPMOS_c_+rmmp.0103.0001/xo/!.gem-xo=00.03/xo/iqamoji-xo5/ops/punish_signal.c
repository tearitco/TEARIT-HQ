...
int main(int argc, char **argv) {
    ...
    snprintf(log_path, sizeof(log_path), "%s/pieces/rl/punish.log", project_root);
    ...
    if (log) {
        ...
        fprintf(log, "[%s] PUNISH|goal=%s|amount=%d|reason=%s\n", ts, goal, amount, reason);
        fclose(log);
    }
