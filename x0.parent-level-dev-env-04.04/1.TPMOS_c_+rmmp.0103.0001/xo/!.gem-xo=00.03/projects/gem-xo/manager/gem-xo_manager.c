...
static char* run_tool(const char* tool_name, char* const args[], bool sandbox) {
    ...
    if (pid == 0) {
        ...
        char *full_path = NULL;
        if (sandbox) {
            if (chdir("projects/gem-xo/sandbox") != 0) _exit(1);
            ...
            if (strchr(tool_name, '/') || tool_name[0] == '.') { execvp(tool_name, args); }
                ...
                if (asprintf(&full_path, "projects/gem-xo/ops/+x/%s", tool_name) != -1) { execvp(full_path, args); free(full_path); }
                execvp(tool_name, args); // Fallback to system path
            }
        }
        _exit(127);
...
void update_completions(void) {
    ...
    char *prefix = star + 1;
    char *p_args[] = {"projects/gem-xo/ops/+x/complete_path", (strlen(prefix) == 0 ? "." : prefix), NULL};
    ...
    if (matches && strlen(matches) > 0) {
        ...
        int count = 0;
        while (token && count < 5) {
            strncpy(g_comp_labels[count], token, sizeof(g_comp_labels[count]) - 1);
...
int main(int argc, char *argv[]) {
    ...
    mkdir("projects/gem-xo/state", 0755); mkdir("projects/gem-xo/sandbox", 0755); 
    mkdir("projects/gem-xo/sandbox/config", 0755);
    FILE *sandbox_ctx = fopen("projects/gem-xo/sandbox/config/context.txt", "w");
    if (sandbox_ctx) {
        fputs("project_id=gem-xo\nsandbox_depth=4\n", sandbox_ctx);
        fclose(sandbox_ctx);
    }
    FILE *sandbox_yolo = fopen("projects/gem-xo/sandbox/config/yolo.flag", "w");
    if (sandbox_yolo) {
        fputs("1\n", sandbox_yolo);
        fclose(sandbox_yolo);
    }
