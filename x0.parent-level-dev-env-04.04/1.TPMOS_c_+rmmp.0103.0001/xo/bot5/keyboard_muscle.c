...
void inject_mouse(int btn, int x, int y) {
    char *hist_path = NULL;
    if (asprintf(&hist_path, "%s/pieces/keyboard/history.txt", g_root) < 0) return;

    FILE *f = fopen(hist_path, "a");
    if (!f) {
        log_debug("INJECT MOUSE FAIL: Cannot open %s (%s)", hist_path, strerror(errno));
        free(hist_path);
        return;
    }
    ...
    char ts[64];
    strftime(ts, sizeof(ts), "%Y-%m-%d %H:%M:%S", t);
    fprintf(f, "[%s] MOUSE_EVENT: %d %d %d\n", ts, btn, x, y);
    fflush(f);
    fsync(fileno(f)); 
    fclose(f);
    log_debug("INJECT MOUSE SUCCESS: (%d, %d, %d) -> %s", btn, x, y, hist_path);
    free(hist_path);
}
...
int main(int argc, char *argv[]) {
    ...
    typedef enum { S_START, S_WAIT, S_INJECT, S_MOUSE, S_HOLD } State;
    State st = S_START;
    ...
    while (!should_exit) {
        switch(st) {
            ...
            case S_INJECT:log_debug("FSM: INJECTING ENTER (13)"); inject_key(13); st = S_MOUSE; break;
            case S_MOUSE: log_debug("FSM: INJECTING MOUSE TEST EVENT (32, 10, 10)"); inject_mouse(32, 10, 10); st = S_HOLD; break;
