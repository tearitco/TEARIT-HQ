...
### Controller Contract and PAL Bot Brains
The next step for the pet stack is to treat controllers as swappable modules rather than special logic hidden inside a manager.

In practice:
- a controller is an op, or a compact bundle of ops
- an editor is a higher-level controller with authoring and inspection features
- PAL is the preferred way to describe repeated bot behavior
- C stays at the thin execution boundary

This model makes it possible to change behavior without rewriting the engine:
- a manual player controller for local testing
- a scripted controller for repeatable automation
- an editor controller for map and state authoring
- a PAL-driven controller for autonomous bots

### Example: Future XO-Pets Control Loop
The op names below are illustrative. The point is the contract, not the naming.

```asm
; xo_pets_brain.asm - future autonomous pet loop
start:
...
That pattern keeps the architecture clean:
1. PAL chooses behavior.
2. Ops mutate state.
3. The manager routes and schedules.
4. The parser only renders the current files.

### Editor as a Controller
The editor should follow the same contract. It should be able to:
- create or configure a tank
- attach a controller
- author or export PAL
- write state back to disk

That is how the editor stays reusable instead of becoming a one-off UI tied to one game mode.
