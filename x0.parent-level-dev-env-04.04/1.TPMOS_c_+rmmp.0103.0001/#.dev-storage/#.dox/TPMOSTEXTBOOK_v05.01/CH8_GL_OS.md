...
In the current direction, Wraith is the broader host shell and `chtmgl` is the project family that should eventually run inside it. That means the GL-facing work is not just "draw the same screen in another window"; it is the path toward making current TPMOS projects open as Wraith-runnable modules, even if some names or wrappers change later to fit the architecture.

The practical rule is:
- `chtmgl-alpha` and related experiments are the current GL milestone line
- Wraith is the runtime/shell that should host them
- GL-OS-style rendering is the visible bridge between file-backed state and the visual surface
...
## 🔮 Future Vision
- **Wraith as the host shell:** Most GUI-facing TPMOS work will eventually launch under Wraith, including `chtmgl`-style projects
- **GL-OS as the render bridge:** Visual output still matters, but it is the bridge layer rather than the whole destination
