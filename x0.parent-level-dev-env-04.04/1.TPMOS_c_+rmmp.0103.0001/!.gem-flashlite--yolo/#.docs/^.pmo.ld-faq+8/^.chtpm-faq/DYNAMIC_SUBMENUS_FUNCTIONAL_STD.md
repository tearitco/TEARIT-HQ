...
## 5. Utility Strip Standard

For dynamic file browsers and similar scrolling submenu surfaces, the utility controls should stay on one shared strip row when space permits:

- `^_UP`
- `v_DOWN`
- thumb/range indicator

Example shape:

```xml
<button label="^_UP" onClick="FS_SCROLL_UP" />
<button label="v_DOWN" onClick="FS_SCROLL_DOWN" />
Thumb:[#-------]1-6/30
```

Rules:

1. The scroll controls stay nav-owned and numbered like other interactive rows.
2. The thumb is presentation truth, not a separate hidden state machine.
3. ASCII and GL should derive the same strip from the same project-authored truth.
4. Do not add a visible manual refresh control when a watcher/marker seam can keep the surface current.
