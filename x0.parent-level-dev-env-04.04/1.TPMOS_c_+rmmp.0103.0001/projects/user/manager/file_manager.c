...
void scan_saves() {
    char path[MAX_PATH];
    if (strcmp(active_user, "unknown") == 0) return;
    
    snprintf(path, sizeof(path), "%s/projects/user/pieces/profiles/%s/saves/%s", project_root, active_user, project_id);
...
void load_context() {
    ...
    snprintf(path, sizeof(path), "%s/projects/user/pieces/session", project_root);
    DIR *dir = opendir(path);
    if (dir) {
        struct dirent *entry;
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (dot && strcmp(dot, ".session") == 0) {
                strncpy(active_user, entry->d_name, dot - entry->d_name);
                active_user[dot - entry->d_name] = '\0';
                break;
            }
        }
        closedir(dir);
...
void sync_state() {
    ...
    if (f) {
        while (fgets(lines[lc], MAX_LINE, f) && lc < MAX_VARS-20) {
            ...
            if (strncmp(lines[lc], "active_user=", 12) == 0) continue;
...
void run_op(const char *cmd, const char *slot) {
    ...
    snprintf(shell_cmd, sizeof(shell_cmd), "'%s/projects/user/ops/+x/file_op.+x' '%s' '%s' '%s'", project_root, cmd, project_id, slot);
    system(shell_cmd);
