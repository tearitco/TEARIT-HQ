...
static char g_sandbox_root[MAX_PATH] = "projects/gem-dev/sandbox";
...
static void load_sandbox_root(const char *config_path, char *out, size_t out_size) {
    snprintf(out, out_size, "projects/gem-dev/sandbox");
    if (config_path && read_config_kv(config_path, "sandbox_root", out, out_size)) return;
}
...
static char* run_tool(const char* tool_name, char* const args[], bool sandbox) {
    ...
    if (pid == 0) {
        ...
        char *full_path = NULL;
        if (sandbox) {
            if (chdir("projects/gem-dev/sandbox") != 0) _exit(1);
            ...
            if (strchr(tool_name, '/') || tool_name[0] == '.') { execvp(tool_name, args); }
                ...
                if (asprintf(&full_path, "projects/gem-dev/ops/+x/%s", tool_name) != -1) { execvp(full_path, args); free(full_path); }
                execvp(tool_name, args); // Fallback to system path
            }
        }
        _exit(127);
    }
    if (pid > 0) log_pid(pid, "gem-dev-tool");
    close(pipefd[1]);
...
static void load_apis(void) {
    FILE *f = fopen("projects/gem-dev/config/apis.txt", "r");
...
void execute_pending_tool(void) {
    ...
    char *tmp_args = "projects/gem-dev/state/args.tmp";
    ...
    if (strcmp(g_pending_tool_name, "read_file") == 0) {
        char *p_p[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "path", NULL};
        ...
        char *p_p[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "path", NULL};
        char *p_c[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "content", NULL};
        ...
        char *p_c[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "cmd", NULL};
        ...
        char *p_q[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "query", NULL};
        ...
        char *p_p[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "path", NULL};
        char *p_s[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "search", NULL};
        char *p_r[] = {"projects/gem-dev/ops/+x/json_parser", tmp_args, "replace", NULL};
    ...
    if (result) {
        ...
        char *a_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", g_pending_function_call, NULL};
        run_tool(a_args[0], a_args, false);
        char *t_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "tool", result, NULL};
        ...
        if (raw_tool_followup_enabled()) {
            ...
            FILE *fresp = fopen("projects/gem-dev/state/last_response.txt", "w");
            if (fresp) { fputs(g_resp_area, fresp); fclose(fresp); }
            snprintf(g_ai_state, sizeof(g_ai_state), "IDLE");
            snprintf(g_fsm_state, sizeof(g_fsm_state), "IDLE");
            snprintf(g_sys_msg, sizeof(g_sys_msg), "Tool output ready.");
            char *r_args_app[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", g_resp_area, NULL};
        ...
        char *a_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", g_pending_function_call, NULL};
        run_tool(a_args[0], a_args, false);
        char *t_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "tool", "Error: Tool execution failed or unknown tool.", NULL};
        run_tool(t_args[0], t_args, false);
    }
...
void deny_pending_tool(void) {
    ...
    char *a_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", g_pending_function_call, NULL};
    run_tool(a_args[0], a_args, false);
    char *t_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "tool", "Error: Permission denied by user.", NULL};
    run_tool(t_args[0], t_args, false);
...
void update_completions(void) {
    ...
    char *p_args[] = {"projects/gem-dev/ops/+x/complete_path", query, NULL};
    ...
    if (matches && strlen(matches) > 0) {
        ...
        /* Focus the suggestions header using the numeric interactive index. */
...
void handle_choose_path(int index) {
    ...
    /* Restore focus to the pre-completion input field. */
    {
...
void audit_log(const char* user, const char* assistant) {
    char *path = "projects/gem-dev/pieces/world_01/map_01/iqabel/memories/history.txt";
    mkdir("projects/gem-dev/pieces/world_01/map_01/iqabel/memories", 0755);
    FILE *f = fopen(path, "a");
    if (f) {
...
static void resolve_local_model(void) {
    char* tags_file = "projects/gem-dev/state/ollama_tags.json";
    char* api_path = NULL; 
    ...
    char* find_args[] = {"projects/gem-dev/ops/+x/json_parser", tags_file, key, NULL};
...
void start_ai_query(const char* input) {
    char *ctx_file = "projects/gem-dev/state/context.json";
    char *tmp_prompt = "projects/gem-dev/state/prompt.json";
    char *tmp_llm = "projects/gem-dev/state/llm_response.json";
    char *tmp_content = "projects/gem-dev/state/llm_content.json";
    bool is_llamacpp = strstr(g_api_url, ":8080") != NULL;
    ...
    if (stat(ctx_file, &st) != 0 || st.st_size < 10) {
        ...
        if (persona && strlen(persona) > 5) {
            char *s_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "system", persona, NULL};
            ...
            char *s_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "system", default_persona, NULL};
            run_tool(s_args[0], s_args, false);
        }
    }
    if (input) {
        ...
        char *u_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "user", (char*)input, NULL};
    ...
    g_thinking_secs = 0;
    char *r_args[] = {"projects/gem-dev/ops/+x/json_state", "read", ctx_file, NULL};
    char *context = run_tool(r_args[0], r_args, false);
    
    char *p_args[] = {"projects/gem-dev/ops/+x/gemini_payload_builder", ctx_file, tmp_prompt, NULL};
...
void check_ai_status(void) {
    ...
    char *p_text[] = {"projects/gem-dev/ops/+x/json_parser", tmp_llm, "candidates[0].content.parts[0].text", NULL};
    ...
    if (!text_content || strlen(text_content) == 0) {
        ...
        char *p_text_fallback[] = {"projects/gem-dev/ops/+x/json_parser", tmp_llm, "text", NULL};
    ...
    char *p_func[] = {"projects/gem-dev/ops/+x/json_parser", tmp_llm, "candidates[0].content.parts[0].functionCall", NULL};
    function_call = run_tool(p_func[0], p_func, false);
    
    if (!function_call || strlen(function_call) == 0) {
        if (function_call) free(function_call);
        char *p_func_fallback[] = {"projects/gem-dev/ops/+x/json_parser", tmp_llm, "functionCall", NULL};
    ...
    if ((!text_content || strlen(text_content) == 0) && (!function_call || strlen(function_call) == 0)) {
        char *p_err[] = {"projects/gem-dev/ops/+x/json_parser", tmp_llm, "error.message", NULL};
                ...
                if (query) {
                    if (web_search_ignored()) {
                        ...
                        if (tool_result && strlen(tool_result) > 0) {
                            ...
                            char *u_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", "{\"name\":\"web_search\",\"args\":{\"query\":\"fallback\"}}", NULL};
                            run_tool(u_args[0], u_args, false);
                            char *t_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "tool", tool_result, NULL};
                            run_tool(t_args[0], t_args, false);
                            free(tool_result);
                            free(query);
                            free(error_msg);
    ...
    if (function_call && strlen(function_call) > 2) {
        FILE *ffunc = fopen("projects/gem-dev/state/function_call.tmp", "w");
        if (ffunc) { fputs(function_call, ffunc); fclose(ffunc); }
        char *p_tn[] = {"projects/gem-dev/ops/+x/json_parser", "projects/gem-dev/state/function_call.tmp", "name", NULL};
        tool_name = run_tool(p_tn[0], p_tn, false);
        char *p_ta[] = {"projects/gem-dev/ops/+x/json_parser", "projects/gem-dev/state/function_call.tmp", "args", NULL};
        args_json = run_tool(p_ta[0], p_ta, false);
        unlink("projects/gem-dev/state/function_call.tmp");
    }
    ...
    char *r_args_app[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", text_content, NULL};
    run_tool(r_args_app[0], r_args_app, false);
...
char* get_gui_var(const char* id) {
    char path[MAX_PATH];
    build_project_path(path, sizeof(path), "manager/gui_state.txt");
    char *line = NULL; size_t len = 0; FILE *f = fopen(path, "r");
    if (!f) return NULL;
    while (getline(&line, &len, f) != -1) {
        if (strncmp(line, id, strlen(id)) == 0 && line[strlen(id)] == '=') {
            char *val = strdup(line + strlen(id) + 1);
            if (!val) { fclose(f); free(line); return NULL; }
            val[strcspn(val, "\r\n")] = '\0';
...
void write_gui_state(void) {
    ...
    FILE *f = fopen(path, "w");
    if (f) {
        fprintf(f, "module_path=projects/gem-dev/manager/+x/gem-dev_manager.+x\n");
        fprintf(f, "active_layout_id=gem-dev.chtpm\n");
        fprintf(f, "ai_state=%s\n", g_ai_state);
        fprintf(f, "active_api=%s\n", g_api_url);
        fprintf(f, "ai_status_line=%s\n", g_ai_status_line);
        fprintf(f, "ctx_pct=%s\n", g_ctx_pct);
        fprintf(f, "iqabel_fsm=%s\n", g_fsm_state);
        fprintf(f, "gem-dev_response_area=%s\n", g_resp_area);
        
        if (g_completion_mode) {
            fprintf(f, "gem-dev_api_menu=\n");
        } else {
            fprintf(f, "gem-dev_api_menu=%s\n", g_menu_area);
        }
        ...
        /* Parser auto-activates cli_io from cli_input on state refresh. */
        fprintf(f, "cli_input=%s\n", g_input_text_state);
...
void process_input_trigger(void) {
    ...
    FILE *dbg = fopen("projects/gem-dev/manager/perm_debug.log", "a");
    if (dbg) {
    ...
    if (input && strlen(input) > 0) {
        ...
        if (strcmp(g_ai_state, "PENDING_PERM") == 0) {
            dbg = fopen("projects/gem-dev/manager/perm_debug.log", "a");
            if (dbg) {
...
int main(int argc, char *argv[]) {
    ...
    mkdir(sandbox_config_dir, 0755);
    char sandbox_ctx_path[MAX_PATH];
    snprintf(sandbox_ctx_path, sizeof(sandbox_ctx_path), "%s/config/context.txt", g_sandbox_root);
    FILE *sandbox_ctx = fopen(sandbox_ctx_path, "w");
    if (sandbox_ctx) {
        fputs("project_id=gem-dev\nsandbox_root=projects/gem-dev/sandbox\n", sandbox_ctx);
        fclose(sandbox_ctx);
    }
    FILE *sandbox_yolo = fopen("projects/gem-dev/sandbox/config/yolo.flag", "w");
    if (sandbox_yolo) {
        fputs("1\n", sandbox_yolo);
        fclose(sandbox_yolo);
    }
    ...
    // GLOBAL STARTUP CLEAR: hard-reset transient session/runtime files before any UI refresh.
    truncate_file(hist_path);
    truncate_file(buf_path);
    {
        char gui_path[MAX_PATH];
        build_project_path(gui_path, sizeof(gui_path), "manager/gui_state.txt");
        truncate_file(gui_path);
    }
    truncate_file("pieces/apps/player_app/state_changed.txt");
    truncate_file("projects/gem-dev/state/context.json");
    truncate_file("projects/gem-dev/state/prompt.json");
    truncate_file("projects/gem-dev/state/llm_response.json");
    truncate_file("projects/gem-dev/state/last_response.txt");
    truncate_file("projects/gem-dev/state/curl_debug.log");
    truncate_file("projects/gem-dev/state/args.tmp");
    ...
    while (!g_shutdown) {
        char *ctx_file = "projects/gem-dev/state/context.json";
        int pct = 0; char *r_args_pct[] = {"projects/gem-dev/ops/+x/json_state", "read", ctx_file, NULL};
                    ...
                    while (fgets(line, sizeof(line), hf)) {
                        ...
                        if (cmd_ptr) {
                            ...
                            if (key == 10 || key == 13) { process_input_trigger(); state_changed = 1; }
                                ...
                                char *r_args[] = {"projects/gem-dev/ops/+x/json_state", "read", ctx_file, NULL}; char *context = run_tool(r_args[0], r_args, false);
                                char *tmp_prompt = "projects/gem-dev/state/prompt.json"; char *tmp_llm = "projects/gem-dev/state/llm_response.json"; char *tmp_content = "projects/gem-dev/state/llm_content.json";
                                ...
                                if (content_json) {
                                    ...
                                    char *p_sum[] = {"projects/gem-dev/ops/+x/json_parser", tmp_content, "summary", NULL}; char *summary = run_tool(p_sum[0], p_sum, false);
                                    if (summary) {
                                        unlink(ctx_file); char *init_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "system", summary, NULL};
