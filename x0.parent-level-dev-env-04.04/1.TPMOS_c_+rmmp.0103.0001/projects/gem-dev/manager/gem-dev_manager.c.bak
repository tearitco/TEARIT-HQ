...
void update_completions(void) {
    ...
    const char *sandbox_root = "projects/gem-dev/sandbox";
    char query[1024];
    char *prefix = star + 1;

    if (strlen(prefix) == 0) {
        snprintf(query, sizeof(query), "%s/", sandbox_root);
    } else {
        snprintf(query, sizeof(query), "%s/%s", sandbox_root, prefix);
    }
    ...
    if (matches && strlen(matches) > 0) {
        ...
        while (token && count < 5) {
            ...
            if (strncmp(label, sandbox_root, root_len) == 0) {
                label += root_len;
                if (*label == '/') label++;
            }
        ...
        // We set active_gui_index to 'comp_root' so Parser knows it's active
        char *agi_path = "pieces/display/active_gui_index.txt";
        FILE *agi_f = fopen(agi_path, "w");
        if (agi_f) { fprintf(agi_f, "comp_root\n"); fclose(agi_f); }
...
char* get_gui_var(const char* id) {
    char *path = "projects/gem-dev/manager/gui_state.txt"; char *line = NULL; size_t len = 0; FILE *f = fopen(path, "r");
    if (!f) return NULL;
...
void write_gui_state(void) {
    char *path = "projects/gem-dev/manager/gui_state.txt";
...
int main(int argc, char *argv[]) {
    ...
    mkdir("projects/gem-dev/state", 0755); mkdir("projects/gem-dev/sandbox", 0755); 
    mkdir("projects/gem-dev/sandbox/config", 0755);
    FILE *sandbox_ctx = fopen("projects/gem-dev/sandbox/config/context.txt", "w");
    if (sandbox_ctx) {
    ...
    truncate_file("projects/gem-dev/manager/gui_state.txt");
