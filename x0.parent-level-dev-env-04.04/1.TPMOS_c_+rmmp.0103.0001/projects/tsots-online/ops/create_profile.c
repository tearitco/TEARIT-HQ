/*
 * create_profile.+x - tsots_online Op: Create a new tsots_online profile
 * Usage: ./create_profile.+x <project_path> <tsots_onlinename>
 */
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
int main(int argc, char *argv[]) {
    ...
    /* Validate tsots_onlinename (alphanumeric only) */
    for (int i = 0; tsots_onlinename[i]; i++) {
        if (!isalnum((unsigned char)tsots_onlinename[i]) && tsots_onlinename[i] != '_') {
            fprintf(stderr, "Error: Invalid tsots_onlinename (alphanumeric and _ only)\n");
            return 1;
        }
    }
    ...
    asprintf(&profile_path, "%s%s/%s", project_path, PROFILES_DIR, tsots_onlinename);
