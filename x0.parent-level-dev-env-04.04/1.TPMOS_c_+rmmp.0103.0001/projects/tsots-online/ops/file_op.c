...
char* get_active_tsots_online() {
    char session_dir[MAX_PATH];
    snprintf(session_dir, sizeof(session_dir), "%s/projects/tsots_online/pieces/session", project_root);
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (dot && strcmp(dot, ".session") == 0) {
            char *tsots_online = strdup(entry->d_name);
            tsots_online[dot - entry->d_name] = '\0';
            closedir(dir);
            return tsots_online;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    char *tsots_online = get_active_tsots_online();
    if (!tsots_online) {
        printf("Error: No active tsots_online session found.\n");
        return 1;
    }

    char save_base[MAX_PATH];
    snprintf(save_base, sizeof(save_base), "%s/projects/tsots_online/pieces/profiles/%s/saves/%s/%s", project_root, tsots_online, project_id, slot_name);
    ...
    if (strcmp(cmd, "save") == 0) {
        ...
        system(shell_cmd);
        printf("Project '%s' saved to slot '%s' for tsots_online '%s'.\n", project_id, slot_name, tsots_online);
        ...
        system(shell_cmd);
        printf("Project '%s' loaded from slot '%s' for tsots_online '%s'.\n", project_id, slot_name, tsots_online);
        ...
        snprintf(saves_dir, sizeof(saves_dir), "%s/projects/tsots_online/pieces/profiles/%s/saves/%s", project_root, tsots_online, project_id);
        ...
        if (access(save_base, F_OK) == 0) {
            ...
            system(shell_cmd);
            printf("Save slot '%s' deleted for tsots_online '%s'.\n", slot_name, tsots_online);
        } else {
            printf("Error: Save slot '%s' not found.\n", slot_name);
            free(tsots_online);
            return 1;
