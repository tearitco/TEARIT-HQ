...
static char current_browser_dir[MAX_PATH] = "projects/slop-ed-dev/games";
...
static void write_editor_state(void);
...
int resolve_piece_dir(const char* piece_id, char* out_dir, size_t out_sz) {
    ...
    // Check project-specific world/map path first
    ...
    // Check project-specific root pieces
    snprintf(out_dir, out_sz, "%s/projects/%s/pieces/%s", project_root, current_project, piece_id);
    if (access(out_dir, F_OK) == 0) return 1;

    // Check system-wide pieces
    snprintf(out_dir, out_sz, "%s/pieces/%s", project_root, piece_id);
    if (access(out_dir, F_OK) == 0) return 1;
...
void scan_maps() {
    char *path = NULL;
    // Scan directly from the game's maps directory
    if (asprintf(&path, "%s/%s/maps", project_root, active_game_name) == -1) return;
...
static int save_game_to_path(const char *rel_path) {
    ...
    if (strstr(rel_path, "projects/slop-ed-dev/games/") == rel_path) {
        strncpy(final_rel, rel_path, MAX_PATH - 1);
    } else if (strstr(rel_path, "games/") == rel_path) {
        snprintf(final_rel, MAX_PATH - 1, "projects/slop-ed-dev/%s", rel_path);
    } else {
        snprintf(final_rel, MAX_PATH - 1, "projects/slop-ed-dev/games/%s", rel_path);
    }
    final_rel[MAX_PATH-1] = '\0';
    ...
    // Switch context to the new save location
    if (strncmp(final_rel, "projects/", 9) == 0) {
        strncpy(current_project, final_rel + 9, sizeof(current_project) - 1);
    } else {
        strncpy(current_project, final_rel, sizeof(current_project) - 1);
    }
    strncpy(active_game_name, final_rel, sizeof(active_game_name) - 1);
...
static int load_game_from_path(const char *rel_path) {
    ...
    snprintf(full_src, sizeof(full_src), "%s/%s", project_root, final_rel);
    ...
    // Find the correct map index from the loaded metadata
    stage_map_idx = (map_count > 1) ? 1 : 0;
    for (int i = 1; i < map_count; i++) {
        char dir_i[64], dir_metadata[64];
        derive_map_dir_name(project_maps[i], dir_i, sizeof(dir_i));
        derive_map_dir_name(current_map_dir, dir_metadata, sizeof(dir_metadata));
        if (strcmp(dir_i, dir_metadata) == 0) {
            stage_map_idx = i;
            break;
        }
    }
    map_idx = stage_map_idx;

    char msg[MAX_LINE];
    snprintf(msg, sizeof(msg), "Loaded game: %s", final_rel);
    set_response(msg);
...
void handle_command(const char* cmd) {
    ...
    if (dbg) {
        fprintf(dbg, "MANAGER: handle_command received: '%s' (len: %zu)\n", cmd, strlen(cmd));
        fclose(dbg);
    }
    ...
    if (strncmp(cmd, "SET_DIR:", 8) == 0) {
        ...
        set_file_path_input(current_browser_dir);
        clear_search_query();
    ...
    } else if (strncmp(cmd, "SET_LOAD_GAME:", 14) == 0) {
        load_game_from_path(cmd + 14);
...
void write_gui_state() {
    ...
    if (asprintf(&path, "%s/projects/slop-ed-dev/manager/gui_state.txt", project_root) != -1) {
        FILE *f = fopen(path, "w");
        if (f) {
            fprintf(f, "module_path=projects/slop-ed-dev/manager/+x/slop-ed-dev_manager.+x\n");
            fprintf(f, "active_layout_id=slop-ed-dev.chtpm\n");
            ...
            append_aligned_button_attr(action_markup, sizeof(action_markup), "CANCEL", "href", "projects/slop-ed-dev/layouts/file_menu.chtpm", &next_display_num);
            fprintf(f, "browser_action_buttons_markup=%s\n", action_markup);

            // Generate project_selection_markup for load_menu.chtpm
            char selection_markup[8192] = "";
            DIR *gd = opendir("projects/slop-ed-dev/games");
            if (gd) {
                struct dirent *entry;
                while ((entry = readdir(gd)) != NULL) {
                    if (entry->d_name[0] == '.') continue;
                    char entry_path[MAX_PATH];
                    snprintf(entry_path, sizeof(entry_path), "projects/slop-ed-dev/games/%s", entry->d_name);
                    struct stat st;
...
void write_editor_state() {
    ...
    if (asprintf(&path, "%s/pieces/apps/player_app/manager/state.txt", project_root) != -1) {
        FILE *f = fopen(path, "r");
        char **lines = NULL;
        int line_count = 0, found_proj = 0, found_target = 0, found_z = 0, found_key = 0, found_focus = 0, found_game = 0, found_map = 0;
        char line_buf[MAX_LINE];
        if (f) {
            while (fgets(line_buf, sizeof(line_buf), f) && line_count < 49) {
                lines = realloc(lines, sizeof(char*) * (line_count + 1));
                ...
                if (eq) {
                    *eq = '\0';
                    ...
                    else if (strcmp(k, "active_game") == 0) { asprintf(&lines[line_count], "active_game=%s\n", active_game_name); found_game = 1; }
                    else if (strcmp(k, "current_map") == 0) { 
        ...
        if (!found_game && line_count < 50) { lines = realloc(lines, sizeof(char*) * (line_count + 1)); asprintf(&lines[line_count++], "active_game=%s\n", active_game_name); }
        if (!found_map && line_count < 50 && stage_map_idx >= 0 && stage_map_idx < map_count) { lines = realloc(lines, sizeof(char*) * (line_count + 1)); asprintf(&lines[line_count++], "current_map=%s\n", project_maps[stage_map_idx]); }
...
char my_layout_path[MAX_PATH] = "pieces/apps/slop-ed-dev/layouts/slop-ed-dev.chtpm";

void resolve_my_layout() {
    char *path = NULL;
    if (asprintf(&path, "%s/pieces/apps/slop-ed-dev/slop-ed-dev.pdl", project_root) == -1) return;
    FILE *f = fopen(path, "r");
    if (f) {
        char line[MAX_LINE];
        while (fgets(line, sizeof(line), f)) {
...
int is_active_layout() {
    ...
    if (fgets(line, sizeof(line), f)) { 
        ...
        int res = 0;
        if (cur && strstr(cur, "slop-ed-dev.chtpm") != NULL) res = 1;
        else {
            if (cur && strstr(cur, "pal_editor.chtpm") != NULL) res = 0;
            else if (cur && (strstr(cur, "pieces/apps/slop-ed-dev/layouts/") != NULL ||
                             strstr(cur, "projects/slop-ed-dev/layouts/") != NULL)) res = 1;
        }
        free(path); 
        return res; 
    }
...
void save_project() {
    ...
    if (f) {
        ...
        fprintf(f, "META         | entry_layout       | pieces/apps/slop-ed-dev/layouts/slop-ed-dev.chtpm\n\n");
...
int process_key(int key) {
    ...
    if (strcmp(layout, "file_menu.chtpm") == 0) {
        if (key == '7') {
            if (strcmp(active_game_name, "none") == 0) {
                browser_mode = 1; clear_search_query(); set_file_path_input("projects/slop-ed-dev/games/new-game");
                transition_to_layout("projects/slop-ed-dev/layouts/file_browser.chtpm");
        ...
        if (key == '9') { // Load
            transition_to_layout("projects/slop-ed-dev/layouts/load_menu.chtpm");
    ...
    if (key == 'e' || key == '7') {
        ...
        char *mgr_path = NULL;
        if (asprintf(&mgr_path, "%s/projects/slop-ed-dev/manager/state.txt", project_root) != -1) {
        ...
        transition_to_layout("projects/slop-ed-dev/layouts/pal_editor.chtpm");
        hit_frame_marker();
...
int main() {
    ...
    // Auto-load default-0000 game
    load_game_from_path("projects/slop-ed-dev/games/default-0000");
