...
> directory - this project needs a fundamentally different approach than
> groq-ollama (manual Llama3 prompt + /completion, not native tools[]),
> and re-introducing native tool schema here is a regression, not a fix.
...
The manager (`cpp-llm_manager.c`) is designed for maximum responsiveness and system health:
...
- **Sandbox Mode:** All tool executions (read/write/exec) are confined to `projects/cpp-llm/sandbox`.
