...
void update_stocks_sell(const char* user_hash, const char* symbol, float shares) {
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s/projects/yahoo/pieces/user_%s/state.txt", project_root, user_hash);
    char temp_path[MAX_PATH];
    snprintf(temp_path, sizeof(temp_path), "%s.tmp", path);
        ...
        if (colon) {
            *colon = '\0';
            if (strcmp(token, symbol) == 0) {
                ...
                float final_shares = current_shares - shares;
                if (final_shares > 0) {
                    if (strlen(new_stocks) > 0) strcat(new_stocks, ",");
                    char buf[64];
                    snprintf(buf, sizeof(buf), "%s:%.2f", symbol, final_shares);
...
int main(int argc, char *argv[]) {
    ...
    char *symbol = argv[2];
    float shares_to_sell = atof(argv[3]);
    ...
    // Check if user has enough shares (simplified check)
    // In a full implementation, we'd read stocks from state.txt
    
    float balance = get_state_float(user_hash, "balance");
    float proceeds = shares_to_sell * price;

    set_state_float(user_hash, "balance", balance + proceeds);
    update_stocks_sell(user_hash, symbol, shares_to_sell);
    
    printf("Sold %.2f shares of %s at $%.2f. New balance: $%.2f\n", shares_to_sell, symbol, price, balance + proceeds);
