...
void save_manager_state() {
    ...
    if (f) {
        char status_buf[256];
        char pet_x[32], pet_y[32], zom_x[32], zom_y[32], sel_x[32], sel_y[32], leg_x[32], leg_y[32];
        ...
        get_state_fast("legend_block", "pos_x", leg_x);
        get_state_fast("legend_block", "pos_y", leg_y);
        ...
        fprintf(f, "legend_block_pos_x=%s\nlegend_block_pos_y=%s\n", leg_x, leg_y);
...
void route_input(int key) {
    ...
    if (key == 'x' || key == 'X') {
        ...
        int nz = cz + 1; if (nz > 10) nz = 10;  /* Max Z=10 for verticality */
                ...
                if (f) {
                    ...
                    while (fgets(line, sizeof(line), f)) {
                        ...
                        if (cmd) {
                            ...
                            if (strcmp(action, "INTERACT") == 0) { // Host signals absolute map control
                                is_map_control = 1;
                                ...
                                sscanf(action + 12, "%f,%f,%f", &dx, &dy, &dz);
                                cam_pos[0] += dx;
                                cam_pos[1] += dy;
                                cam_pos[2] += dz;
                                save_manager_state();
                            ...
                            else if (strcmp(action, "OP fuzz-op::scan") == 0) route_input('1');
                            ...
                            if (key_code > 0) {
                                if (key_code == 27) { // ESC
                                    ...
                                    /* 1. Local Camera Mode Keys Removed (Handled via COMMAND: CAMERA_MODE) */
                                    int moved = 0;
