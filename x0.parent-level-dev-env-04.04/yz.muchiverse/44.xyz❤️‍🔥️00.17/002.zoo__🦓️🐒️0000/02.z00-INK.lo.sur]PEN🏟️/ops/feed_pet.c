 ...
 * Reduces a pet's hunger, small happiness bump. Generic over which pet -
 * takes the piece_id as its only argument, exactly like every other
 * xlector-dispatched method handler in this project (see
 * ops/xlector_input.c's own dispatch, which always calls
 * `<handler> <active_target_id>`).
 * Usage: feed_pet.+x <piece_id>
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    set_kv_int(state_path, "hunger", clamp(hunger - 30, 0, 100));
    set_kv_int(state_path, "happiness", clamp(happiness + 5, 0, 100));
    ...
    if (lf) { fprintf(lf, "%s eats happily.\n", name); fclose(lf); }
    return 0;
}
