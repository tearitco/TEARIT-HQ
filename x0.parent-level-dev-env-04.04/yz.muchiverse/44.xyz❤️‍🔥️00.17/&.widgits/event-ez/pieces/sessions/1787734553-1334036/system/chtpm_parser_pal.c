    ...
    while (fgets(line, MAX_LINE, f)) {
        ...
        *val_start = '\0';
        ...
        if (strcmp(cmd_val, "void") == 0 || !is_parser_cmd) {
            (void)asprintf(&btn, "<button label=\"%s\" onClick=\"KEY:%d\" /><br/>", trimmed_key, method_idx++);
        } else {
...
/* Set interact_mode in hero/state.txt so move_player moves the xlector
 * cursor (mode=1) instead of the hero (mode=0). Called by the INTERACT
 * branch on entry (mode=1) and ESC exit (mode=0). */
static void set_interact_mode(int mode) {
    char path[MAX_PATH];
 ...
 * hangs forever the instant it's used against a genuinely persistent
 * module (confirmed as the live, direct cause of egg-pals-13's own
 * reported "frozen/garbled, impossible to navigate" GUI). A persistent
 * `<module>` process (this project's own pal/main_loop_chtpm.pal,
 * launched once via launch_module() and looping independently) is the
 * correct, real-fuzz-op-manager.c-matching shape for ANY project,
 * including mutaclsym's own map/movement one - see main_loop_chtpm.pal's
 * own header comment. process_key()'s INTERACT branch below dispatches
 * via plain, fire-and-forget inject_raw_key(eff) into interact_relay.txt
 * instead; the already-running module picks it up on its own next loop
 * tick. Do not reintroduce a blocking wait here. */
...
void render_element(int idx, char* frame, int* p_global_counter, int* p_scoped_counter) {
    ...
    bool is_active = (idx == active_index) ||
        (active_index != -1 && strcmp(el->onClick, "INTERACT") == 0 && current_project_is_map_control_active());
    char pref[4];
...
void compose_frame() {
    ...
    /* ATOMIC WRITE (PITFALL #14/#15): plain fopen("w") truncates to 0
     * before fwrite completes, so the renderer can read an empty/partial
     * file mid-write = visible flicker. Write to .tmp first, then
     * rename() over the real path. POSIX rename() on the same filesystem
     * is atomic — the reader sees either the complete OLD or complete NEW
     * file, never a partial one. Same fix as compose_rgb_frame.c's own
     * rgb_frame.raw write (see that file's header comment at line 539). */
    char* cur_tmp = build_path_malloc("pieces/display/current_frame.txt.tmp");
    FILE *out_f = fopen(cur_tmp, "w");
    if (out_f) {
        fprintf(out_f, "%s", frame);
        fclose(out_f);
        rename(cur_tmp, cur_f);
...
void process_key(int key) {
    ...
    if (active_index == -1) {
        ...
        /* Active mode - match lpns+map+4 reference pattern exactly.
         * INTERACT check FIRST (with dedicated ESC), then generic ESC,
         * then submenus, cli_io, other onClick handlers. */
        UIElement *el = &elements[active_index];

        /* INTERACT mode: dedicated ESC + raw key injection (FIRST check,
         * matching lpns+map+4's chtpm_parser_pal.c line 3043) */
        if (strcmp(el->onClick, "INTERACT") == 0) {
            if (key == ESC_KEY || key == JOY_BUTTON_8) {
                /* ESC exits INTERACT mode - clean reset to top nav */
                set_interact_mode(0);
                active_index = -1;
                focus_index = 0;
                initialize_focus();
                ...
                /* All other keys: convert arrow keys and inject to relay */
                int eff = key;
                if (key == ARROW_LEFT) eff = 1000;
                else if (key == ARROW_RIGHT) eff = 1001;
                else if (key == ARROW_UP) eff = 1002;
                else if (key == ARROW_DOWN) eff = 1003;
                else if (key >= JOY_BUTTON_0 && key <= JOY_BUTTON_8)
                    eff = 2000 + (key - JOY_BUTTON_0);
                inject_raw_key(eff);
            }
        }
        /* KISS: ESC just deactivates, NEVER clears input */
        else if (key == ESC_KEY || key == JOY_BUTTON_8) {
            if (active_index != -1) {
                int old_active = active_index;
                int p = elements[active_index].parent_index;
                /* Find nearest ACTIVATE ancestor */
...
int main(int argc, char **argv) {
    ...
    /* Clear stale keys from a previous session so game_dispatch
     * doesn't replay old movement on first tick. */
    { FILE *cf = fopen("pieces/apps/player_app/interact_relay.txt", "w"); if (cf) fclose(cf); }
    parse_chtm();
    /* parse_chtm()->load_vars() repopulates active_gui_index from the
     * old gui_state.txt (which persists across Ctrl+C restarts). Clear it
     * here so initialize_focus() starts fresh, not restoring last session's
     * active selection. */
    set_var("active_gui_index", "0");
    initialize_focus(); compose_frame();
    struct stat st;
