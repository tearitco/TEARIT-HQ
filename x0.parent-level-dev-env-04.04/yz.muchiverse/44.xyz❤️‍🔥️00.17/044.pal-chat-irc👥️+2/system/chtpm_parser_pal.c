...
#pragma GCC diagnostic pop
    ...
    /* UNRECOGNIZED COMMAND (Pitfall #99) */
...
void process_key(int key) {
    ...
    if (active_index == -1) {
        ...
        /* QUIT KEY REMOVED (mass-refactor 2026-07-26) - this file's own
         * stray literal 'q'/'Q' exit(0) check was the one place the
         * 2026-07-20 "'q'-quits-everything removed" fix (see
         * keyboard_input.c's own header comment) hadn't reached yet:
         * this parser process would still exit(0) on a literal 'q'
         * keystroke even though keyboard_input.c stopped generating one.
         * Real quit is Ctrl+C only, handled directly in keyboard_input.c. */
        else {
