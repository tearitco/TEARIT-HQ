/* chat_switch_user - sets net/session.txt's own current_user_id to an
 * EXISTING user (refuses if the user directory doesn't exist) - v1 has
 * no password (see chat_create_user.c). This file is PER-SESSION (see
 * PAL-CHAT-IRC-STANDARD.txt sec. 1 - not user-pal's own persistent
 * current_login.txt), so it lives at $PRISC_PROJECT_ROOT which is the
 * session directory, not the real project root.
 *
 * Self-contained, no shared headers.
 * Usage: chat_switch_user.+x <user_id> */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: chat_switch_user.+x <user_id>\n");
        return 1;
    }
    resolve_root();
