...
# (itself copied from 041.pal-chain), same convention every project in
# this family follows.
set -e
...
if gcc $CFLAGS -o "system/gl_mirror" "system/gl_mirror.c" -lglut -lGL -lGLU -lX11 2>/tmp/civtxt_gl_mirror_build.log; then
    echo "    gl_mirror: built ok"
else
    echo "    gl_mirror: skipped (GLUT/GL not available - see /tmp/civtxt_gl_mirror_build.log)"
    rm -f /tmp/civtxt_gl_mirror_build.log
fi

echo "--- Building civ-txt ops ---"
gcc $CFLAGS -o "ops/+x/civ_menu_input.+x" "ops/civ_menu_input.c"
gcc $CFLAGS -o "ops/+x/civ_compose_frame.+x" "ops/civ_compose_frame.c"
