#!/bin/bash
# test-harn-same/button.sh - MINIMAL entry point for civ-txt's
# key-injection regression harness. Modeled directly on
# @.apps/my-chara-txt/test-harn-same/button.sh (same shape: real
...
case "$ACTION" in
    ...
    demo)
        ...
        bash "$SCRIPT_DIR/scenarios/demo_setup_and_turn.sh"
        ;;
    all)
    ...
    kill)
        ...
        ;;
    help|h|-h|--help)
        echo "civ-txt test-harn-same/button.sh"
        echo ""
        echo "Usage: ./button.sh <action>"
        echo "  compile, c, build   - Build the 4 reusable tk_* test ops"
        echo "  demo                - Run the default scenario (demo_setup_and_turn.sh)"
        echo "  all                 - Run every scenarios/demo_*.sh, print KPI summary"
        echo "  kill                - Kill any lingering civ-txt processes"
        ;;
    *)
