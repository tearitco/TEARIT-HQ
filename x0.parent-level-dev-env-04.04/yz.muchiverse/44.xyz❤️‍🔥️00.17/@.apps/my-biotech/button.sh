#!/bin/bash
# button.sh - launcher for my-biotech, modeled directly on my-chara-txt's
...
case "$ACTION" in
    ...
    run|r|start)
        ...
        mkdir -p "$SESSION_DIR/pieces/system" "$SESSION_DIR/pieces/display" \
                 ...
                 "$SESSION_DIR/pieces/os" "$SESSION_DIR/projects/my-biotech/manager"
        ...
        : > pieces/display/mybiotech_screen_changed.txt
        : > pieces/display/frame_changed.txt
        : > projects/my-biotech/manager/gui_state.txt
...
EOSTATE
        ...
        export PRISC_PROJECT_ID="my-biotech"
        export NO_NET=1
