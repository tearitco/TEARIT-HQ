...
int main(int argc, char *argv[]) {
    ...
    // Check owned contracts
    int option_index = -1;
    for (int i = 0; i < options_count; i++) {
        char exp_time[32];
        get_expiry_time(expiry, exp_time, sizeof(exp_time));
        if (strcmp(options_symbol[i], symbol) == 0 && 
            strcmp(options_type[i], opt_type) == 0 && 
            options_strike[i] == strike && 
            strcmp(options_expiry[i], exp_time) == 0) {
            option_index = i;
            break;
        }
    }
    if (option_index < 0 || options_contracts[option_index] < contracts) {
        fprintf(stderr, "[%s] Insufficient contracts: %.2f requested, %.2f owned\n", symbol, contracts,
                option_index >= 0 ? options_contracts[option_index] : 0.0);
        return 1;
    }

    float proceeds = price * contracts * 100; // 1 contract = 100 shares
    balance += proceeds;

    // Update options portfolio
    options_contracts[option_index] -= contracts;
    if (options_contracts[option_index] <= 0) {
        for (int i = option_index; i < options_count - 1; i++) {
            strcpy(options_symbol[i], options_symbol[i+1]);
            strcpy(options_type[i], options_type[i+1]);
            options_contracts[i] = options_contracts[i+1];
            options_strike[i] = options_strike[i+1];
            strcpy(options_expiry[i], options_expiry[i+1]);
        }
        options_count--;
    }
    ...
    if (history_count < MAX_HISTORY) {
        ...
        history_shares[history_count] = -contracts; // Negative for sell
