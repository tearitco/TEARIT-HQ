...
if gcc $CFLAGS -o "system/gl_mirror" "system/gl_mirror.c" -lglut -lGL -lGLU -lX11 2>/tmp/yahoo_gl_mirror_build.log; then
    echo "    gl_mirror: built ok"
else
    echo "    gl_mirror: skipped (GLUT/GL not available)"
    rm -f /tmp/yahoo_gl_mirror_build.log
fi

echo "--- Building yahoo-app ops ---"
gcc $CFLAGS -o "ops/+x/yahoo_menu_input.+x" "ops/yahoo_menu_input.c"
gcc $CFLAGS -o "ops/+x/yahoo_compose_frame.+x" "ops/yahoo_compose_frame.c"
