# button.ps1 - Windows launcher for yahoo-app (bank GUI). Linux: button.sh
# Usage: powershell -ExecutionPolicy Bypass -File .\button.ps1 <compile|run|kill|check|help>
...
$PROJECT_ID="yahoo-app"
$LAYOUT="pieces/chtpm/layouts/bank.chtpm"
...
function Invoke-Run{
  Write-Host "=== yahoo-app Win (bank.chtpm) ===" -ForegroundColor Cyan
  ...
  if(-not $parser -or -not $kb){ Write-Error "MISSING parser/keyboard PE - donor from 014.wsr failed"; return 1 }
  Invoke-Kill
  $sid="{0}-{1}" -f [int][double]::Parse((Get-Date -UFormat %s)),$PID
  $SESSION=Join-Path $SCRIPT_DIR "pieces\sessions\$sid"
  foreach($d in @("pieces\system\widget_cmds","pieces\display","pieces\apps\player_app","pieces\keyboard","pieces\os","projects\yahoo-app\manager","projects\yahoo-app\pieces\broker_select")){
  ...
  foreach($pair in @(
    ...
    @("projects\yahoo-app\pieces","projects\yahoo-app\pieces"),@("projects\yahoo-app\data","data")
  )){ Copy-TreeLink (Join-Path $SCRIPT_DIR $pair[0]) (Join-Path $SESSION $pair[1]) }
  foreach($f in @("pieces\apps\player_app\interact_relay.txt","pieces\keyboard\history.txt","pieces\display\frame_changed.txt","pieces\display\renderer_pulse.txt","pieces\system\widget_cmds\inbox.txt","pieces\system\widget_cmds\status.txt","projects\yahoo-app\manager\gui_state.txt")){
    Write-NoBom (Join-Path $SESSION $f) ""
  }
  $cfg=Join-Path $SESSION "pieces\system\config.txt"
  if(-not(Test-Path $cfg)){ Write-NoBom $cfg "user_hash=`nbank_balance=5000.00`ncurrent_broker=`nlast_lookup_symbol=`nlast_lookup_price=0.00`n" }
  $brokers=Join-Path $SESSION "pieces\system\brokers.txt"
  if(-not(Test-Path $brokers)){ Write-NoBom $brokers "yahoo_finance|Yahoo Finance|full`n" }
  $houseAbs=$HOUSE_DIR; try{$houseAbs=(Resolve-Path -LiteralPath $HOUSE_DIR).Path}catch{}
  Write-NoBom (Join-Path $SESSION "pieces\system\house_root.txt") ($houseAbs+"`n")
  Write-NoBom (Join-Path $SESSION "pieces\apps\player_app\state.txt") "module_path=system/prisc+x pal/main_loop_chtpm.pal`nproject_id=yahoo-app`nactive_target_id=bank`n"
  $env:PRISC_PROJECT_ROOT="."; $env:PRISC_PROJECT_ID=$PROJECT_ID; $env:PAL_LAYOUT=$LAYOUT; $env:SKIP_ORCH_COMPILE="1"
  ...
  Write-Host "Keys: THIS console (WT/conhost). Ctrl+C quits."
...
switch($a){
  ...
  default{
    Write-Host @"
yahoo-app button.ps1 (Windows bank GUI)
  compile | run | kill | check | help
  NO_GL=1  NO_TERM=1
  Donates PE from 014.wsr-pal when local bins are Linux ELF.
"@
    exit 0
  }
}
