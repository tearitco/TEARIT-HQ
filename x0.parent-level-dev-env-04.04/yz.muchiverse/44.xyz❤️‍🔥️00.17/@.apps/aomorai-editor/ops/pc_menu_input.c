 ...
 * whichever aomorai-editor screen is currently showing. Modeled directly on
 ...
 * was added.
 *
 * aomorai-editor-specific commands (clone phase):
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-truncation"
    snprintf(pdl_path, sizeof(pdl_path), "%s/projects/aomorai-editor/pieces/%s/piece.pdl", root, piece_id);
#pragma GCC diagnostic pop
...
static void write_chtpm_bridge(const char *piece_id) {
    ...
    if (cf) {
        fprintf(cf, "project_id=aomorai-editor\n");
 ...
 * decision_mode, no monster_types.txt registry lookup (aomorai-editor has
...
int main(int argc, char **argv) {
    ...
    snprintf(state_path, sizeof(state_path), "%s/projects/aomorai-editor/pieces/pc_menu/state.txt", project_root);
    ...
    if (cmd) {
        if (strcmp(cmd, "CONFIRM_START") == 0) {
             ...
             * See aomorai-editor-blueprint.md once written for the real
             * design this will eventually open. */
            snprintf(message, sizeof(message),
                     "View Editor - not built yet (see aomorai-editor-blueprint.md).");
        }
    }
