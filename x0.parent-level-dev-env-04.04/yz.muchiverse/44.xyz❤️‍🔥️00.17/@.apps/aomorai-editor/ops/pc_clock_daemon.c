 ...
 *
 * Adapted to aomorai-editor's own real schema (world_01/state.txt's
 ...
 * tick"/NPCs at all, that's aomorai-editor's own real addition on top.
 *
    ...
    while (!g_should_exit) {
        ...
        if (autotick_enabled && elapsed_ms > 0) {
            ...
            if (delta_game_ms > 0) {
                 ...
                 * aomorai-editor's own text screen, with its own real
                 ...
                 * "board-viewer:aomorai-editor" scoped entry) - a real,
                ...
                if (house_root[0]) {
                    ...
                    if (pf) {
                        ...
                        while (fgets(peer_line, sizeof(peer_line), pf)) {
                            ...
                            if (proj_tok && sess_tok && strcmp(proj_tok, "board-viewer:aomorai-editor") == 0) {
