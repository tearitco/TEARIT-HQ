...
if gcc $CFLAGS -o "system/gl_mirror" "system/gl_mirror.c" -lglut -lGL -lGLU -lX11 2>/tmp/aomorai-editor_gl_mirror_build.log; then
    echo "    gl_mirror: built ok"
else
    echo "    gl_mirror: skipped (GLUT/GL not available - see /tmp/aomorai-editor_gl_mirror_build.log)"
    rm -f /tmp/aomorai-editor_gl_mirror_build.log
fi

echo "--- Building aomorai-editor ops ---"
...
echo "    source than aomorai-editor's own copy had) - that feature needs"
