#!/bin/bash
# test-harn-same/button.sh - MINIMAL entry point for myne-qrypto/qtc's
# key-injection regression harness. Same shape as
# 045.muchi-pal-agent🤖️+1/test-harn-same/button.sh and
# my-chara-txt/test-harn-same/button.sh.
set -u
...
case "$ACTION" in
    ...
    demo)
        ...
        bash "$SCRIPT_DIR/scenarios/demo_login_screen_smoke.sh"
        ;;
    all)
    ...
    kill)
        ...
        ;;
    help|h|-h|--help)
        echo "myne-qrypto/qtc test-harn-same/button.sh"
        echo ""
        echo "Usage: ./button.sh <action>"
        echo "  compile, c, build   - Build the 4 reusable tk_* test ops"
        echo "  demo                - Run the default scenario (demo_login_screen_smoke.sh)"
        echo "  all                 - Run every scenarios/demo_*.sh, print KPI summary"
        echo "  kill                - Kill any lingering qtc processes"
        ;;
    *)
