...
char last_key_display[64] = "";  /* Persists across load_vars() wipes */
...
void compose_frame() {
    load_vars();
    if (last_key_display[0]) set_var("last_key", last_key_display);
    const char* active_id = get_var("active_target_id");
...
void process_key(int key) {
    ...
    strncpy(last_key_display, key_str, sizeof(last_key_display) - 1);
    last_key_display[sizeof(last_key_display) - 1] = '\0';
