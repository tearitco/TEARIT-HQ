...
/* CATEGORY B FIX 2026-08-20 (found live: RGB frame never updated in the
 * X11 window post symlink-elimination - PRISC_PROJECT_ROOT is now
 * $SCRIPT_DIR, not the disposable session dir, so this daemon's own
 * project_root-relative current_frame.txt/rgb_frame.raw read+write sites
 * silently diverged from the session dir x11_mirror actually watches).
 * Session-local display files (current_frame.txt, frame_changed.txt
 * pulse, rgb_frame.raw + receipt) resolve via session_root (CWD);
 * persistent game-data reads (hero state, registry/fonts/emoji assets)
 * stay on project_root. */
...
/* Forward decl - real definition is further down this file; write_receipt()
 * (added below, this same pass) needs it earlier than its original
 * position. */
static int read_kv_int_generic(const char *path, const char *key, int def);

/* REAL ADDITION 2026-08-17, direct user instruction ("modify txt receipts
 * so they match your png dmp so we dont have to dump png 2 get
 * understanding"): mirrors the same hero/camera fields sync_camera_to_
 * bv_state() (ops/game_dispatch.c) already keeps in sync, so a plain `cat`
 * of this receipt gives the same understanding a PNG dump would for
 * camera/interact debugging, without needing an actual pixel dump every
 * time. Local copy only (this file is NOT symlinked to shared-lib in this
 * project - see system/chtpm_rgb_render.c's own real, non-symlinked
 * status, confirmed via `ls -la` before editing), so this has zero effect
 * on any other project sharing the daemon's own shared-lib baseline. */
static void write_receipt(const char *path, size_t byte_count, uint64_t checksum) {
    char hero_path[MAX_PATH];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", project_root);
    int last_key = read_kv_int_generic(hero_path, "last_key", 0);
    int interact_mode = read_kv_int_generic(hero_path, "interact_mode", 0);
    int render_mode = read_kv_int_generic(hero_path, "render_mode", 0);
    int camera_mode = read_kv_int_generic(hero_path, "camera_mode", 0);
    int cam_yaw = read_kv_int_generic(hero_path, "cam_yaw", 0);
    int cam_pitch = read_kv_int_generic(hero_path, "cam_pitch", 0);
    int cam_pan_x = read_kv_int_generic(hero_path, "cam_pan_x", 0);
    int cam_pan_y = read_kv_int_generic(hero_path, "cam_pan_y", 0);
    int cam_pan_z = read_kv_int_generic(hero_path, "cam_pan_z", 0);
    int cam_z_level = read_kv_int_generic(hero_path, "cam_z_level", 0);
    int pos_x = read_kv_int_generic(hero_path, "pos_x", 0);
    int pos_y = read_kv_int_generic(hero_path, "pos_y", 0);
    int pos_z = read_kv_int_generic(hero_path, "pos_z", 0);
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        last_key, interact_mode, render_mode, camera_mode,
        cam_yaw, cam_pitch, cam_pan_x, cam_pan_y, cam_pan_z, cam_z_level,
        pos_x, pos_y, pos_z);
