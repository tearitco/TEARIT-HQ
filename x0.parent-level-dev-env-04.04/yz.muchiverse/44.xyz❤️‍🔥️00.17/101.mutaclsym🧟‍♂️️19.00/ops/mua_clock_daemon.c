/* mua_clock_daemon - real PERSISTENT game-clock process, direct port of
 ...
 *
 * Adapted to mutaclysm's own real schema (world_01/state.txt's
 ...
 * tick"/NPCs at all, that's mutaclysm's own real addition on top.
 *
 * Self-contained, no shared headers.
 * Usage: mua_clock_daemon.+x (no args, runs forever until SIGTERM/SIGINT) */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
    ...
    while (!g_should_exit) {
        ...
        if (autotick_enabled && elapsed_ms > 0) {
            ...
            if (delta_game_ms > 0) {
                 ...
                 * mutaclysm's own text screen, with its own real
                 ...
                 * "board-viewer:mutaclysm" scoped entry) - a real,
                ...
                if (house_root[0]) {
                    ...
                    if (pf) {
                        ...
                        while (fgets(peer_line, sizeof(peer_line), pf)) {
                            ...
                            if (proj_tok && sess_tok && strcmp(proj_tok, "board-viewer:mutaclysm") == 0) {
