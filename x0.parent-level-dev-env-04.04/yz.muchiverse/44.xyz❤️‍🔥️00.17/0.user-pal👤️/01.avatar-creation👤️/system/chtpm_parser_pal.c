...
void load_dynamic_methods(const char* active_id) {
    ...
    while (fgets(line, MAX_LINE, f)) {
        ...
        *val_start = '\0';
        ...
        /* CLI_IO:target_id[:mode] — real text fields as methods (same nav
         * numbers as buttons). Without this, inputs had to be hardcoded
         * above ${piece_methods} and stole slots 1..N. */
        if (strncmp(cmd_val, "CLI_IO:", 7) == 0) {
            char tid[128] = "", mode[64] = "";
            const char *rest = cmd_val + 7;
            const char *colon = strchr(rest, ':');
            if (colon) {
                size_t n = (size_t)(colon - rest);
                if (n >= sizeof(tid)) n = sizeof(tid) - 1;
                memcpy(tid, rest, n); tid[n] = '\0';
                snprintf(mode, sizeof(mode), "%s", colon + 1);
            } else {
                snprintf(tid, sizeof(tid), "%s", rest);
            }
            /* trim tid */
            char *tp = tid;
            while (*tp == ' ' || *tp == '\t') tp++;
            memmove(tid, tp, strlen(tp) + 1);
            for (int i = (int)strlen(tid) - 1; i >= 0 && (tid[i] == ' ' || tid[i] == '\t' || tid[i] == '\r' || tid[i] == '\n'); i--)
                tid[i] = '\0';
            if (!tid[0]) snprintf(tid, sizeof(tid), "input_text");
            if (mode[0])
                (void)asprintf(&btn,
                    "<cli_io id=\"%s\" label=\"%s\" target_id=\"%s\" input_mode=\"%s\" /><br/>",
                    tid, trimmed_key, tid, mode);
            else
                (void)asprintf(&btn,
                    "<cli_io id=\"%s\" label=\"%s\" target_id=\"%s\" /><br/>",
                    tid, trimmed_key, tid);
            (void)asprintf(&btn_compact,
                "<cli_io id=\"%s\" label=\"%s\" target_id=\"%s\" />",
                tid, trimmed_key, tid);
            /* Advance method_idx so later KEY:n buttons stay aligned with
             * METHOD row order (menu_input indexes by METHOD line). */
            method_idx++;
            /* no KEY:n on the field — Enter activates typing */
        } else {
...
#pragma GCC diagnostic pop
    ...
    if (view_path) {
        ...
        if (vf) {
            ...
            /* muchi-pals / avatar-creation layouts use ${panel_content}
             * as the info panel; same bytes as game_map/view.txt. */
        ...
        set_var("game_map", "[Map Loading...]");
            ...
            set_var("panel_content", "[Panel Loading...]");
    }
...
void render_element(int idx, char* frame, int* p_global_counter, int* p_scoped_counter) {
    ...
    /* Keep [+]/[-] fold markers visible even while ACTIVATE is engaged.
     * Older code stripped them so the row looked like focus/selector vanished
     * (user report: skin fold lost +/− on Enter). Fold state still flips
     * [+]↔[-] at parse time via fold_<id> gui_state. */
...
void process_key(int key) {
    ...
    /* Normal nav mode */
    if (active_index == -1) {
        /* DNA scroller rows: a/d (or left/right) while FOCUSED — no INTERACT
         * lock required. Keeps full numbered nav (w/s/digits) available. */
        if (focus_index >= 0 && focus_index < element_count) {
            UIElement *f_el = &elements[focus_index];
            if (strcmp(f_el->type, "scroller") == 0 && strcmp(f_el->onClick, "INTERACT") == 0) {
                if (key == 'a' || key == 'A' || key == ARROW_LEFT || key == 1000) {
                    inject_raw_key(1000);
                    clear_nav_on_next = true;
                    goto nav_marker;
                }
                if (key == 'd' || key == 'D' || key == ARROW_RIGHT || key == 1001) {
                    inject_raw_key(1001);
                    clear_nav_on_next = true;
                    goto nav_marker;
                }
        ...
        if (key == ESC_KEY || key == JOY_BUTTON_8) { 
            ...
            if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
                ...
                if (focus_index >= 0 && focus_index < element_count) {
                    ...
                    if (child_el->num_children > 0 && strcmp(child_el->onClick, "ACTIVATE") == 0) {
                        ...
                        /* Real bug: ACTIVATE submenu used send_command("INTERACT"),
                         * which is not a command verb — scroller never became
                         * active_index, so [^] never stuck and a/d never scrolled.
                         * Mirror nav-mode INTERACT: engage the focused child. */
                        active_index = focus_index;
                        clear_nav_on_next = true;
                        export_active_index();
            ...
            /* Scroller DNA strips: only a/d (left/right) stay in INTERACT.
             * w/s, digits, Esc leave INTERACT so numbered nav works again
             * (user report: [^] active but not navigable to other rows). */
            int leave_for_nav = 0;
            if (key == ESC_KEY || key == JOY_BUTTON_8) {
                active_index = -1;
                export_active_index();
            } else if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP ||
                       key == ARROW_DOWN || key == 's' || key == 'S' || key == JOY_DOWN ||
                       isdigit(key)) {
                active_index = -1;
                export_active_index();
                leave_for_nav = 1;
            } else if (key == 'a' || key == 'A' || key == ARROW_LEFT || key == 1000) {
                inject_raw_key(1000);
                clear_nav_on_next = true;
            } else if (key == 'd' || key == 'D' || key == ARROW_RIGHT || key == 1001) {
                inject_raw_key(1001);
                clear_nav_on_next = true;
            } else if (key == 10 || key == 13) {
                /* Second Enter exits INTERACT, keeps focus on strip */
                active_index = -1;
                export_active_index();
            } else {
                /* Other keys: ignore (do not swallow whole keyboard into module) */
                clear_nav_on_next = true;
            }
            if (leave_for_nav) {
                /* Re-enter nav mode handling for this same key */
                if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) {
                    int prev = focus_index; do { focus_index--; if (focus_index < 0) focus_index = element_count-1; } while (focus_index != prev && !is_navigable(focus_index));
                    digit_accum = 0; clear_nav_on_next = true; export_active_index();
                } else if (key == ARROW_DOWN || key == 's' || key == 'S' || key == JOY_DOWN) {
