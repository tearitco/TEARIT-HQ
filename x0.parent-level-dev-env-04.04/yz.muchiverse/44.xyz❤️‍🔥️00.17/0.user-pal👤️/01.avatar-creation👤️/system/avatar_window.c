...
/* --- Front-facing Minecraft-style character (DNA → pixels) ---
 * Used when sprite.csv is missing so desktop pet is never a bare blue circle. */
...
static int load_or_make_character_sprite(const char *pet_id, const char *state_path,
                                         unsigned char **out_pixels, int *out_res) {
    (void)pet_id;
    /* Desktop pet = front-facing MC body from DNA (skin/hair/shirt/pants).
     * Not a blue circle, not a flat emoji face-only sheet. */
    return synthesize_mc_front_sprite(state_path, out_pixels, out_res);
}
...
int main(int argc, char **argv) {
    ...
    if (pet_id) {
        if (load_or_make_character_sprite(pet_id, g_pet_state_path, &g_sprite_pixels, &g_sprite_res)) {
            upload_texture(g_sprite_pixels, g_sprite_res, &g_texture);
            g_has_texture = 1;
        } else {
            fprintf(stderr, "egg_window: could not build character for %s\n", pet_id);
        }
    }
...
int main(int argc, char **argv) {
    ...
    if (pet_id) {
        /* Front-facing character: sprite.csv or procedural MC body from DNA. */
        if (load_or_make_character_sprite(pet_id, pet_state_path, &g_sprite_pixels, &g_sprite_res)) {
            upload_texture(g_sprite_pixels, g_sprite_res, &g_texture);
            g_has_texture = 1;
        } else {
            fprintf(stderr, "egg_window: could not build character for %s\n", pet_id);
    ...
    while (running) {
        ...
        /* Rebuild front-facing body when DNA changes (skin/hair/shirt/pants). */
        if (pet_id && pet_state_path[0]) {
            char si[16]="", hair[32]="", shirt[32]="", pants[32]="", sig[128];
            read_kv_file(pet_state_path, "skin_index", si, sizeof(si));
            read_kv_file(pet_state_path, "hair_color", hair, sizeof(hair));
            read_kv_file(pet_state_path, "shirt_color", shirt, sizeof(shirt));
            read_kv_file(pet_state_path, "pants_color", pants, sizeof(pants));
            snprintf(sig, sizeof(sig), "%s|%s|%s|%s", si, hair, shirt, pants);
            if (strcmp(sig, last_dna_sig) != 0) {
                snprintf(last_dna_sig, sizeof(last_dna_sig), "%s", sig);
                free(g_sprite_pixels);
                g_sprite_pixels = NULL;
                g_sprite_res = 0;
                g_has_texture = 0;
                if (load_or_make_character_sprite(pet_id, pet_state_path,
                        &g_sprite_pixels, &g_sprite_res)) {
