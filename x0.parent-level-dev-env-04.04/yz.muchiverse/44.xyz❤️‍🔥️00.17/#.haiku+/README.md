...
**CORRECTED (2026-08-29 doc-audit pass):** this README described a
`for-user/`+`for-agent/` split where `for-user/` held live, actively-
maintained files. That's no longer true — `for-user/` was archived
this pass (`archive/for-user/`) because all 4 of its files described a
pre-reorg topology (`shared-ops/`, `pal-scripts/`, `2.muchi-verse-0.0/`)
that no longer exists, and it references files (`zest-er-summary.txt`,
`!.xyzos-standards.txt` without the `+1`) that don't exist under those
names anymore. This README is rewritten below to describe what's
actually here now, not the old split.
...
- **`for-agent/`** — Haiku's still-live working files:
  ...
  template, use as needed). `agent.txt`/`gotchas_by_project.txt` were
  archived this pass (pre-reorg paths); `debug_findings_2026-07-21.md`
  was archived as a resolved one-time incident report.
...
- **`tpmos-re-dox/`** — 3 files cited directly from real current source
  ...
  `!.HOUSE_STDS.md`.
- **`sonnet/`** — mostly current; `wildcard-completion-handoff.md` was
  archived this pass (its target task was never implemented and the
  project has moved on).
- **`archive/`** — everything retired this pass: `for-user/` (whole
  folder), `#.temp-dox/`, `#.calendar-dump/`, and the individual
  `for-agent/`/`sonnet/` files above.
