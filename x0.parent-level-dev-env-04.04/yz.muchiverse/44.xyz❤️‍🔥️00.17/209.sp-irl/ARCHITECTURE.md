# ARCHITECTURE — SP-IRL (Tactical Monsters)
...
| PvP Tactics | grid-based turn combat on overworld | shared map + LoS system |

## Module map

```text
src/gb.h       types, modes, API
src/main.c     glutInit, timer, input routing, overworld step + encounter, PvP setup
src/map.c      map.txt load, walkable, tile query
src/mon.c      mons.pdl / moves.pdl load, stats, type mult, damage, move/atk range
src/battle.c   wild battle phases: intro → menu → fight/run → win/lose
src/render.c   GBC full-color palette, tiles, battle boxes, HP bars, PvP grid
...
TITLE → PVP_SETUP → PVP_BATTLE → TITLE
```
