/* PAL-NATIVE FIX (see chtpm-to-pal-layout-plan.txt §8's own
 * interact+module work, and shared-ops/prisc+x.c's own identical fix,
 * applied there first): OP_READ_HISTORY's literal-path parsing used
 * to capture ONLY a destination register (rd), silently leaving the
 * position/cursor register (rs1) at its zero-initialized default -
 * register 0, which this VM's own main loop unconditionally resets to
 * 0 every single instruction (`regs[0] = 0`, RISC-V-style hardwired
 * zero - confirmed by reading this file's own main loop). Any
 * literal-path `read_history` call would therefore re-fseek to byte 0
 * and re-read only the very first entry, forever - permanently
 * broken incremental reads. Fixed by adding a real 3-argument form,
 * `read_history <path> xD, xS` (path, destination register, AND a
 * genuine position register), parsed BEFORE the old 1-register
 * fallback so it only ever activates for scripts that actually use
 ...
 * for wsr-pal's own keyboard_input.c. */
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#include <stdio.h>
...
static int resolve_piece_state_path(const char *proj_root, const char *proj_id,
...
#ifndef _WIN32
    /* Headroom beyond PROJ_MAX_PATH so gcc can prove these concatenations
    ...
    char pieces_dir[PROJ_MAX_PATH + 512];
    ...
    if (worlds) {
        ...
        while ((w = readdir(worlds)) != NULL) {
            ...
            char maps_dir[PROJ_MAX_PATH + 768];
            ...
            while ((m = readdir(maps)) != NULL) {
                ...
                char candidate[PROJ_MAX_PATH + 1024];
    ...
    if (pass == 2) {
        ...
        if (op_idx >= 0) {
            ...
            if (sscanf(args, "r%d, r%d", &i->rd, &i->rs1) == 2 || sscanf(args, "x%d, x%d", &i->rd, &i->rs1) == 2) {
                 ...
                 * register forms above didn't already match. */
            ...
            char *args = strstr(original, part) + strlen(part);
            while (*args == ' ' || *args == '\t') args++;
            /* LOCAL FIX (muchi-pals, 2026-07-19): the literal-path form
             * (`read_pos rd, "path"`) was never actually parsed here
             * despite OP_READ_POS's own exec branch already supporting
             * one (`char *path = strlen(i.literal_arg) > 0 ? ... `) -
             * every call silently fell through to the hardcoded default
             * (pieces/apps/player_app/history.txt), regardless of what
             * path was written in the .pal source. Same class of gap as
             * this file's own documented read_history literal-path fix
             * (see this file's top-of-file comment) - checked FIRST so
             * the plain `read_pos rd` (no path) form used elsewhere is
 ...
 * a lot here specifically: full_script_path is built from
 * PRISC_PROJECT_ROOT, and a project living under a path with spaces (e.g.
 * "...\egg-pals+5.2win (1)\egg-pals") was getting silently truncated at
 * the first space by cmd.exe's tokenizer when passed to popen()
 * unquoted - every single dispatched key (menu_input, compose_menu,
 * tick_pets) was failing with "'...' is not recognized as an internal or
 * external command" and being silently swallowed, which looked from the
 * outside like keyboard input just not doing anything. popen() itself is
 * also the wrong tool here since cmd.exe only understands double-quote
 * quoting anyway - CreateProcess is called directly instead. */
...
static FILE *win_run_capture(const char *exe_path, const char *arg1, HANDLE *out_proc) {
    char qexe[PROJ_MAX_PATH], qarg1[PROJ_MAX_PATH];
    char cmdline[PROJ_MAX_PATH * 2];
...
/* ECALL - real syscall-style instruction, ported verbatim from
 * shared-ops/prisc+x.c (see that file's own header comment on this
...
/* SYS_GET_KV_INT/SYS_SET_KV_INT - ported verbatim from shared-ops/
 * prisc+x.c (see that file's own header comment on these two for the
 * full design rationale). `ecall "path" "key"` (both literal_arg and
 * literal_arg2 used together). */
    ...
    if (strncmp(op->handler, "+x/", 3) == 0 || strstr(op->handler, "/+x/")) {
        ...
        if (ext) {
            ...
            if (strcmp(i->custom_name, "layout_op") == 0) {
                 ...
                 * (as a string either way - both platforms' helpers below
                 * take the argument as text). */
                char reg_arg[32];
                const char *arg = i->literal_arg;
                if (strlen(i->literal_arg) == 0) {
                    snprintf(reg_arg, sizeof(reg_arg), "%d", regs[i->rs1]);
                    arg = reg_arg;
                }
#ifdef _WIN32
                HANDLE proc;
                FILE *pipe = win_run_capture(full_script_path, arg, &proc);
                if (pipe) {
                    char result[256];
                    while (fgets(result, sizeof(result), pipe)) printf("%s", result);
                    win_run_close(pipe, proc);
                }
#else
                if (asprintf(&cmd, "\"%s\" \"%s\"", full_script_path, arg) != -1) {
...
int main(int argc, char **argv) {
    ...
    if (base_check) {
        ...
        char binary_path[PROJ_MAX_PATH];
#ifndef _WIN32
    ...
    while (pc < inst_count) {
        ...
        if (i.op == OP_CUSTOM) {
            ...
            /* Binary mode, not "r": this file is a fresh fopen() on every
             * single call (matching the file-based-IPC convention - see
             * this op's own header comment), and on MinGW/MSVCRT, ftell()
             * right after the FIRST read on a newly-opened *text*-mode
             * stream returns a stale 0 instead of the real position
             * (verified - subsequent ftell()s on that same still-open
             * handle are fine, but there never is a "subsequent" call
             * here). That pins x1 at 0 forever, so every dispatch re-reads
             * history.txt's first token - "reads a whole line" symptoms
             * with the pal script correctly enough. Binary mode sidesteps
             * it entirely and is equivalent here anyway since this file
             * is bare LF-terminated decimal integers, nothing \r\n mode
             * would need to translate. */
            FILE *hf = path ? fopen(path, "rb") : NULL;
            if (hf) {
