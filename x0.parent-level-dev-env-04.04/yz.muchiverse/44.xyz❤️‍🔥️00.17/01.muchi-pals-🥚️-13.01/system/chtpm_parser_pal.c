 ...
 * keypress"): send_command()/handle_launch_command() sometimes do their
 * own self-contained layout transition + compose_frame() internally
 * (LAUNCH: prefix, and the "Execute transition" PROJECT_ACTION-style
 * path - see each site's own comment), exactly like process_key()'s own
 * href branches do. Every process_key() call site that invokes
 * send_command() used to unconditionally set clear_nav_on_next=true
 * afterward too, so the main loop's marker check fired a SECOND
 * compose_frame() for the same keypress whenever one of these internal
 * transitions had already rendered. process_key() resets this to false
 * before dispatching; the two internal compose_frame() sites set it
 * true; every send_command() call site checks it instead of blindly
 * setting clear_nav_on_next. */
bool g_send_command_rendered = false;
...
/* RESYNC ON MODULE RELAUNCH (mass-refactor 2026-07-26, root cause of
 * "flicker on nearly every keypress" - confirmed by direct pty frame
 * capture: an href/PROJECT_ACTION/LAUNCH: transition still produced a
 * "[No Methods]" stale-content flash even after removing this file's own
 * eager compose_frame() calls). The OLD module (about to be killed by
 * cleanup_module()) can bump pieces/display/frame_changed.txt on its own
 * last natural ~30ms tick in the brief window before it's actually
 * reaped. main()'s own while(1) loop checks display_frame_ch's CURRENT
 * size unconditionally at the end of every iteration - including the
 * very same iteration that just ran cleanup_module()/parse_chtm() for
 * this transition - so that leftover, causally-unrelated marker growth
 * gets misread as "the new screen has real content", triggering an
 * immediate compose_frame() that reads whatever piece.pdl content
 * happens to already exist on disk for the new screen (stale/empty,
 * since the NEWLY launched module hasn't run its own first instruction
 * yet). Call this right after every cleanup_module()+parse_chtm() pair
 * to re-baseline the marker to ITS post-cleanup size, so only the fresh
 * module's own subsequent real tick can trigger the one correct render. */
char* build_path_malloc(const char* rel); /* forward decl, defined later in this file */
void resync_render_markers(void) {
    struct stat st;
...
/* debug.txt size cap (256 KB). Prevents runaway debug logging from
 * filling disk during a long session. Same principle as the
 * orchestrator.log cap in system/orchestrator.c. */
#define MAX_DBG_BYTES (256 * 1024)
static int dbg_capped = 0;

static void debug_log(const char *fmt, ...) {
    if (dbg_capped) return;
    struct stat st;
    if (stat("debug.txt", &st) == 0 && st.st_size >= MAX_DBG_BYTES) {
        dbg_capped = 1;
        return;
    }
...
void inject_command(const char* cmd) {
    ...
    debug_log("[PARSER] inject_command: '%s'\n", cmd);
    
    if (interact_history_path[0] != '\0') {
        char *full_path = build_path_malloc(interact_history_path);
        debug_log("[PARSER] Writing to interact_history_path: %s\n", full_path);
        fp = fopen(full_path, "a");
...
void launch_module(const char* launch_str) { 
    // DEBUG: Log module launch attempt
    {
        ...
        debug_log("[%02d:%02d:%02d] PARSER: launch_module(%s) pid=%d\n",
...
void handle_launch_command(const char* cmd) {
    ...
    if (strstr(app_name, ".chtpm") != NULL) {
         ...
         * fix (process_key()) for why: reads stale content from before
         * the freshly launched module runs its own first instruction. */
        resync_render_markers(); /* see that function's own header comment */
        g_send_command_rendered = true; /* see this flag's own top-of-file comment */
        return;
    }
...
#pragma GCC diagnostic pop
         ...
         * NO EAGER compose_frame() (mass-refactor 2026-07-26) - same
         * stale-content-flash bug as the href handler's own fix: this
         * would read content from before the freshly (re)launched module
         * has run its own first instruction. That module's own initial
         * compose+hit_frame bumps frame_changed.txt, already watched
         * independently by this process's own main() while(1) loop. */
...
void parse_attributes(UIElement* el, const char* attr_str) {
    ...
    while (*pos) {
         ...
         * end of the strdup'd string, that pre-increment reads ONE BYTE
         * PAST THE END of the heap allocation, corrupting malloc's own
         * bookkeeping (observed: "malloc(): invalid next size (unsorted)"
         * abort a few frames later). Only advance past the terminator if
         * there IS a real non-null character to skip. */
...
void process_key(int key) {
    /* Reset before dispatch - see g_send_command_rendered's own top-of-
     * file comment. Any send_command()/handle_launch_command() call
     * below that already self-rendered sets this back to true so this
     * function's own trailing clear_nav_on_next=true can be skipped. */
    g_send_command_rendered = false;
    ...
    if (active_index == -1) {
        if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
            ...
            if (focus_index >= 0 && focus_index < element_count) {
                ...
                if (strcmp(el->type, "cli_io") == 0) {
                    ...
                    initialize_focus();
                    export_active_index();
                    /* NO EAGER compose_frame() HERE (mass-refactor 2026-07-26,
                     * "flicker on nearly every keypress" root-caused via direct
                     * pty frame capture): this used to force an immediate
                     * render right after parse_chtm()/launch_module() forked
                     * the freshly (re)launched persistent module, but that
                     * child hasn't run its own first instruction yet - this
                     * eager render read stale/placeholder content (captured
                     * live: a "[No Methods]" flash before the real screen
                     * content appeared one render later - a genuine, not
                     * cosmetic, double-render). main_loop_chtpm.pal's own
                     * FIRST instructions are muchi_compose_frame + hit_frame,
                     * unconditionally, before its `loop:` even starts - that
                     * writes frame_changed.txt, which THIS process's own
                     * always-running main() while(1) loop (a separate
                     * function, not process_key()) already watches
                     * independently and composes from once real content
                     * exists. Removing this call leaves exactly one correct
                     * render per href navigation instead of two.
                     *
                     * STILL NOT ENOUGH ON ITS OWN (found via a second round
                     * of pty capture after the above alone didn't fix it):
                     * the OLD module being killed by cleanup_module() above
                     * can bump frame_changed.txt on its own last natural
                     * tick in the brief window before it's reaped - THIS
                     * SAME loop iteration's own end-of-loop marker check
                     * (main(), not this function) then misreads that
                     * leftover growth as "new content ready" and renders
                     * anyway. resync_render_markers() re-baselines the
                     * marker to its post-cleanup size so only the freshly
                     * launched module's own real tick can trigger a render. */
                    resync_render_markers();
                }
                else if (strcmp(el->onClick, "INTERACT") == 0) { active_index = focus_index; clear_nav_on_next = true; export_active_index(); }
                else if (strlen(el->onClick) > 0) {
                    send_command(el->onClick);
                    wait_for_view_change = true;
                    /* g_send_command_rendered: see that flag's own top-of-
                     * file comment - send_command() may have already
                     * self-rendered (LAUNCH:/transition paths), in which
                     * case setting this too would double-render. */
                    if (!g_send_command_rendered) clear_nav_on_next = true;
                }
            }
        }
        /* QUIT KEY REMOVED (mass-refactor 2026-07-26, direct user
         * instruction: "never use q to quit in any of the refactors") -
         * this used to exit(0) the whole parser process on a literal
         * 'q'/'Q' keystroke, which meant that letter could never be
         * typed as ordinary nav/content input. Real quit now happens
         * exclusively via keyboard_input.c's own Ctrl+C (ETX) handling;
         * this process gets torn down externally by button.sh's own
         * trap/orchestrator cascade when that happens, same as every
         * other tracked child. */
        else {
        ...
        if (key == ESC_KEY || key == JOY_BUTTON_8) { 
            ...
            if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
                ...
                /* DOUBLE-RENDER FIX (mass-refactor 2026-07-26, direct user
                 * report: "flicker on nearly every keypress"): the href
                 * branch below used to call compose_frame() itself AND
                 * still fall through to the unconditional
                 * `clear_nav_on_next = true;` that used to sit here
                 * unconditionally after this whole if/else chain - which
                 * the main loop's own `if (dirty || clear_nav_on_next)
                 * compose_frame();` then used to fire a SECOND compose_frame()
                 * for the exact same keypress (same bug class as this
                 * file's own "triple-rendering" warning elsewhere - see
                 * main()'s "RENDER TRIGGER" comment). did_href_jump scopes
                 * the trailing clear_nav_on_next to the ACTIVATE/onClick
                 * branches only, which never render themselves and
                 * genuinely need it. */
                bool did_href_jump = false;
                ...
                if (focus_index >= 0 && focus_index < element_count) {
                    ...
                    if (child_el->num_children > 0 && strcmp(child_el->onClick, "ACTIVATE") == 0) {
                        ...
                        initialize_focus();
                        export_active_index();
                        /* NO EAGER compose_frame() HERE - see the other href
                         * branch's own identical fix above for the full
                         * "[No Methods]" stale-flash explanation. did_href_jump
                         * still skips the trailing clear_nav_on_next below:
                         * the freshly launched module's own frame_changed.txt
                         * bump is what produces the one real render now.
                         * resync_render_markers() also required here (same
                         * leftover-marker-from-the-killed-old-module race). */
                        resync_render_markers();
                        did_href_jump = true;
                ...
                /* g_send_command_rendered: see that flag's own top-of-file
                 * comment - covers the onClick branch just above (href
                 * already covered by did_href_jump). */
                if (!did_href_jump && !g_send_command_rendered) clear_nav_on_next = true;
            }
        }
        else if (strcmp(el->type, "cli_io") == 0) {
