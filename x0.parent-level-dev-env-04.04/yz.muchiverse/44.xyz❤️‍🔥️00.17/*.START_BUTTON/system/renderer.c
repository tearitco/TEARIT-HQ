...
static void render_frame(void) {
    ...
    /* Optional durable audit log (cleared each button.sh run). Session
     * pieces/display/frame_history.txt dies with the throwaway session;
     * PRISC_FRAME_HISTORY points at install debug/frame_history.txt. */
    const char *extra = getenv("PRISC_FRAME_HISTORY");
    if (extra && extra[0]) {
        FILE *ef = fopen(extra, "a");
        if (ef) {
            fprintf(ef, "\n--- FRAME at %s ---\n%s\n", timestamp, content);
            fclose(ef);
        }
