#!/bin/bash
# test-harn-same/button.sh - MINIMAL entry point for pal-chain's
# key-injection UX test harness (same install, 2 concurrent sessions -
# see 044.pal-chat-irc's own test-harn-same/README.txt for the full
...
# byte-identical copies, these are project-agnostic), scenarios/ holds
...
case "$ACTION" in
    ...
    demo)
        ...
        bash "$SCRIPT_DIR/scenarios/demo_2wallet_ux.sh"
        ;;
    kill|k|stop)
        bash "$PROJECT_DIR/pieces/os/kill_all.sh" >/dev/null 2>&1
        sleep 1
        STRAGGLERS=$(ps aux | grep -E "system/(orchestrator|chtpm_parser_pal|renderer|keyboard_input)|ops/\+x/(palnet_peer|chain_miner|chain_inbox_watcher)|prisc\+x" | grep -v grep)
        if [ -n "$STRAGGLERS" ]; then
            echo "$STRAGGLERS" | awk '{print $2}' | xargs -r kill -9
            sleep 1
        fi
        if ps aux | grep -E "system/(orchestrator|chtpm_parser_pal|renderer|keyboard_input)|ops/\+x/(palnet_peer|chain_miner|chain_inbox_watcher)|prisc\+x" | grep -v grep >/dev/null 2>&1; then
        ...
        ;;
    help|h|-h|--help|*)
        echo "test-harn-same/button.sh - key-injected UX test harness for pal-chain"
