...
2026-08-03, directly after piececraft-xyz's P1 clone was verified working
...
the clone is *civ-txt's* skeleton wearing piececraft-xyz's name — every
...
| civ-txt clone (current) | Phase 1 target |
|---|---|
| No hero state file at all | `pieces/hero_01/state.txt` per PORTABLE_ENTITY_ARCHITECTURE.md's universal shape: `entity_type/hp/pos_x/pos_y/pos_z/owner_id`, PLUS piececraft-xyz-specific `interact_mode`/`xlector_pos_x`/`xlector_pos_y` (design §3a pattern-reuse) |
...
- `board_widget_bridge.txt` + `widget_cmds/inbox.txt` command-bus mechanism
  ...
  every idle tick already) — piececraft-xyz's OWN sending side (key press
...
**Direct instruction, the real split for piececraft-xyz specifically**:
...
- The **hero** (piececraft-xyz's own Minecraft-clone player character)
