/* mua_generate_chunk - real Phase 1 chunk terrain generation, per
 ...
 * Usage: mua_generate_chunk.+x <seed> <chunk_x> <chunk_y> */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    if (argc < 4) {
        fprintf(stderr, "Usage: mua_generate_chunk.+x <seed> <chunk_x> <chunk_y> [flat]\n");
        return 1;
    }
    resolve_root();
     ...
     * mua_clock_daemon.c's update_celestial_body() then placed the sun
    ...
    write_kv_int(world_state_path, "game_time_epoch_ms", 1767268800000L); /* real ms-precision canonical clock, see mua_clock_daemon.c's own header comment */
