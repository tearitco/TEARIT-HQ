...
int main(int argc, char **argv) {
     ...
     * this op never checked interact_mode at all, unlike move_player.c's
     * own real, already-correct guard ("if (interact_mode == 0) return
     * 0;"). Confirmed via direct code comparison against the real,
     * canonical fuzz-op_manager.c reference (1.TPMOS_c_+rmmp.0103.0001) -
     * every real op there gets dispatched from ONE unified route_input()
     * with implicit, uniform gating; mutaclysm's own split-into-separate-
     * binaries architecture (move_player/choice/camera_control) means
     * each op MUST self-guard identically, and this one simply never did.
     * Camera keys relayed while interact mode is logically off (a real,
     * possible state even though the OUTER chtpm_parser_pal.c gating is
     * itself correct - see game_dispatch.c's own new is_active_layout()-
     * style guard, added the same pass, for the other real half of this
     * fix) would otherwise still silently move the camera. */
    int interact_mode = read_kv_int(path, "interact_mode", 0);
    if (interact_mode == 0) return 0;
