 ...
 * whichever mutaclysm screen is currently showing. Modeled directly on
 ...
 * was added.
 *
 * mutaclysm-specific commands (clone phase):
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-truncation"
    snprintf(pdl_path, sizeof(pdl_path), "%s/projects/mutaclysm/pieces/%s/piece.pdl", root, piece_id);
#pragma GCC diagnostic pop
...
static void write_chtpm_bridge(const char *piece_id) {
    ...
    if (cf) {
        fprintf(cf, "project_id=mutaclysm\n");
...
/* Sprint 2: Survival metabolism - ported from old mutaclysm end_turn.c.
 * Per-turn: hunger+=1, thirst+=2, stamina regen +5. HP loss when
 * hunger>=100 or thirst>=100. Runs on every END_TURN and MOVE. */
...
static void tick_hero_metabolism(const char *hero_path) {
    ...
    hunger = clamp_int(hunger + HUNGER_PER_TURN, 0, METABOLISM_CAP);
    thirst = clamp_int(thirst + THIRST_PER_TURN, 0, METABOLISM_CAP);
    stamina = clamp_int(stamina + STAMINA_REGEN, 0, 100);
    if (hunger >= STARVING_THRESHOLD) hp = clamp_int(hp - STARVING_HP_LOSS, 0, 100);
    if (thirst >= DEHYDRATED_THRESHOLD) hp = clamp_int(hp - DEHYDRATED_HP_LOSS, 0, 100);

    write_kv_int(hero_path, "hp", hp);
    write_kv_int(hero_path, "hunger", hunger);
    write_kv_int(hero_path, "thirst", thirst);
    write_kv_int(hero_path, "stamina", stamina);
}

static void log_message(const char *msg) {
    char root[PATH_BUF];
    resolve_real_root(project_root, root, sizeof(root));
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/display/message_log.txt", root);
    FILE *f = fopen(path, "a");
    if (f) { fprintf(f, "%s\n", msg); fclose(f); }
}

/* Sprint 2: EAT handler - scans hero/inventory/ for first food/drink,
 * restores hunger/thirst by item power, consumes (deletes) the item. */
static void handle_eat(const char *proj_root, char *message, size_t msg_sz) {
    char root[PATH_BUF];
    resolve_real_root(proj_root, root, sizeof(root));
    char hero_path[PATH_BUF];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", root);
    char inventory_dir[PATH_BUF];
    snprintf(inventory_dir, sizeof(inventory_dir), "%s/pieces/hero_01/inventory", root);

    DIR *d = opendir(inventory_dir);
    if (!d) { snprintf(message, msg_sz, "You have nothing to eat or drink."); return; }
    ...
    while ((entry = readdir(d)) != NULL) {
        ...
        char item_id[64] = "?";
        read_kv_str_local(state_path, "item_id", item_id, sizeof(item_id));

        char registry_path[PATH_BUF];
        snprintf(registry_path, sizeof(registry_path), "%s/pieces/registry/items/items.txt", root);
        FILE *rf = fopen(registry_path, "r");
        if (!rf) continue;
        char rline[MAX_LINE];
        int found = 0;
        char rname[64] = "?", rcategory[16] = "?", rpower[16] = "0";
        while (fgets(rline, sizeof(rline), rf)) {
            if (rline[0] == '#' || rline[0] == '\n') continue;
            rline[strcspn(rline, "\n")] = '\0';
            char *p1 = strchr(rline, '|');
            if (!p1) continue;
            if ((size_t)(p1 - rline) != strlen(item_id) || strncmp(rline, item_id, p1 - rline) != 0) continue;
            char *field = p1 + 1;
            /* field 1=name */
            char *p2 = strchr(field, '|'); if (p2) { *p2 = '\0'; snprintf(rname, sizeof(rname), "%s", field); field = p2 + 1; }
            /* field 2=category */
            char *p3 = strchr(field, '|'); if (p3) { *p3 = '\0'; snprintf(rcategory, sizeof(rcategory), "%s", field); field = p3 + 1; }
            /* skip field 3=glyph */
            char *p4 = strchr(field, '|'); if (p4) field = p4 + 1;
            /* skip field 4=weight */
            char *p5 = strchr(field, '|'); if (p5) field = p5 + 1;
            /* field 5=power */
            char *p6 = strchr(field, '|'); if (p6) *p6 = '\0';
            if (field) snprintf(rpower, sizeof(rpower), "%s", field);
            found = 1;
            break;
        }
        fclose(rf);
        ...
        int is_food = (strcmp(rcategory, "food") == 0);
        int is_drink = (strcmp(rcategory, "drink") == 0);
        if (!is_food && !is_drink) continue;

        int power = atoi(rpower);
        if (is_food) {
            int hunger = read_kv_int(hero_path, "hunger", 0);
            hunger = clamp_int(hunger - power, 0, METABOLISM_CAP);
            write_kv_int(hero_path, "hunger", hunger);
            snprintf(message, msg_sz, "Ate %s.", rname);
        } else {
            int thirst = read_kv_int(hero_path, "thirst", 0);
            thirst = clamp_int(thirst - power, 0, METABOLISM_CAP);
            write_kv_int(hero_path, "thirst", thirst);
            snprintf(message, msg_sz, "Drank %s.", rname);
        }
        log_message(message);

        /* Consume: delete the piece. */
    ...
    closedir(d);
    if (!consumed) snprintf(message, msg_sz, "You have nothing to eat or drink.");
}

/* Sprint 3: PICKUP handler - scans pieces/world_01/items/ for item at
 * hero position, moves directory to hero/inventory/. */
static void handle_pickup(const char *proj_root, char *message, size_t msg_sz) {
    char root[PATH_BUF];
    resolve_real_root(proj_root, root, sizeof(root));
    char hero_path[PATH_BUF];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", root);
    ...
    snprintf(items_dir, sizeof(items_dir), "%s/pieces/world_01/items", root);
    char inventory_dir[PATH_BUF];
    snprintf(inventory_dir, sizeof(inventory_dir), "%s/pieces/hero_01/inventory", root);

    DIR *d = opendir(items_dir);
    if (!d) { snprintf(message, msg_sz, "Nothing here to pick up."); return; }
    ...
    while ((entry = readdir(d)) != NULL) {
        ...
        char item_id[64] = "?";
        read_kv_str_local(state_path, "item_id", item_id, sizeof(item_id));
        mkdir(inventory_dir, 0755);

        char src[PATH_BUF + 320], dst[PATH_BUF + 320];
        snprintf(src, sizeof(src), "%s/%s", items_dir, entry->d_name);
        snprintf(dst, sizeof(dst), "%s/%s", inventory_dir, entry->d_name);
        if (rename(src, dst) != 0) {
            snprintf(message, msg_sz, "Pickup failed."); closedir(d); return;
        }
        /* Strip pos_x/pos_y from carried state. */
        FILE *sf = fopen(dst, "w");
        ...
        char rpath[PATH_BUF];
        snprintf(rpath, sizeof(rpath), "%s/pieces/registry/items/items.txt", root);
        ...
        if (rf) {
            ...
            while (fgets(rline, sizeof(rline), rf)) {
                ...
                char *p1 = strchr(rline, '|');
                if (!p1) continue;
                if ((size_t)(p1 - rline) == strlen(item_id) && strncmp(rline, item_id, p1 - rline) == 0) {
                    char *n = p1 + 1; char *p2 = strchr(n, '|'); if (p2) *p2 = '\0';
                    snprintf(name, sizeof(name), "%s", n); break;
                }
            }
            fclose(rf);
        }
        snprintf(message, msg_sz, "Picked up %s.", name);
        log_message(message);
        picked = 1;
        break;
    }
    closedir(d);
    if (!picked) snprintf(message, msg_sz, "Nothing here to pick up.");
}

/* Sprint 3: DROP handler - drops first item from inventory to items/ at hero pos. */
static void handle_drop(const char *proj_root, char *message, size_t msg_sz) {
    char root[PATH_BUF];
    resolve_real_root(proj_root, root, sizeof(root));
    char hero_path[PATH_BUF];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", root);
    ...
    DIR *d = opendir(inventory_dir);
    if (!d) { snprintf(message, msg_sz, "Nothing to drop."); return; }
    char picked_name[256] = "";
    struct dirent *entry;
    while ((entry = readdir(d)) != NULL) {
        if (entry->d_name[0] == '.') continue;
        snprintf(picked_name, sizeof(picked_name), "%s", entry->d_name); break;
    }
    closedir(d);
    if (!picked_name[0]) { snprintf(message, msg_sz, "Nothing to drop."); return; }

    mkdir(items_dir, 0755);
    char src[PATH_BUF + 320], dst[PATH_BUF + 320];
    snprintf(src, sizeof(src), "%s/%s", inventory_dir, picked_name);
    snprintf(dst, sizeof(dst), "%s/%s", items_dir, picked_name);

    char item_id[64] = "?";
    char src_state[PATH_BUF + 384];
    snprintf(src_state, sizeof(src_state), "%s/state.txt", src);
    read_kv_str_local(src_state, "item_id", item_id, sizeof(item_id));

    if (rename(src, dst) != 0) { snprintf(message, msg_sz, "Drop failed."); return; }

    char dst_state[PATH_BUF + 384];
    snprintf(dst_state, sizeof(dst_state), "%s/state.txt", dst);
    FILE *sf = fopen(dst_state, "w");
    if (sf) { fprintf(sf, "item_id=%s\npos_x=%d\npos_y=%d\n", item_id, hero_x, hero_y); fclose(sf); }
...
static void launch_clock_daemon_if_needed(const char *proj_root) {
    ...
    snprintf(pid_path, sizeof(pid_path), "%s/pieces/system/mua_clock_daemon.pid", real_root);
...
#ifdef _WIN32
    {
        ...
        snprintf(daemon_path, sizeof(daemon_path), "%s\\ops\\+x\\mua_clock_daemon.+x", real_root);
        for (char *p = daemon_path; *p; p++) if (*p == '/') *p = '\\';
        if (!win_run_pe(daemon_path, NULL, proj_root, 0, 1))
            win_run_pe("ops\\+x\\mua_clock_daemon.+x", NULL, proj_root, 0, 1);
    ...
    snprintf(cmd, sizeof(cmd),
             "PRISC_PROJECT_ROOT='%s' setsid '%s/ops/+x/mua_clock_daemon.+x' >/dev/null 2>&1 < /dev/null &",
             proj_root, proj_root);
 ...
 * decision_mode, no monster_types.txt registry lookup (mutaclysm has
...
/* Sprint 4: Monster AI - tick_monsters() iterates pieces/world_01/monsters/*/
/* state.txt, each monster takes one AI step per tick. Chase hero if within */
/* 3 Manhattan distance, else wander randomly. If monster lands on hero tile */
/* it deals damage. Monster dies at hp<=0 (directory deleted). */
#define CHASE_RANGE 3
...
static void tick_monsters(const char *proj_root, int tick) {
    char real_root[PATH_BUF];
    resolve_real_root(proj_root, real_root, sizeof(real_root));
    char monsters_dir[PATH_BUF];
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_01/monsters", real_root);
    char hero_path[PATH_BUF];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", real_root);
    int hero_x = read_kv_int(hero_path, "pos_x", 0);
    int hero_y = read_kv_int(hero_path, "pos_y", 0);
    int hero_hp = read_kv_int(hero_path, "hp", 100);
    ...
    struct dirent *entry;
    char monster_dirs[64][256];
    int monster_count = 0;
    while (monster_count < 64 && (entry = readdir(d)) != NULL) {
        if (entry->d_name[0] == '.') continue;
        snprintf(monster_dirs[monster_count], 256, "%s", entry->d_name);
        monster_count++;
    }
    closedir(d);
    ...
    for (int i = 0; i < monster_count; i++) {
        char state_path[PATH_BUF + 384];
        snprintf(state_path, sizeof(state_path), "%s/%s/state.txt", monsters_dir, monster_dirs[i]);
        int mx = read_kv_int(state_path, "pos_x", 0);
        int my = read_kv_int(state_path, "pos_y", 0);
        int mhp = read_kv_int(state_path, "hp", 10);
        int mdmg = read_kv_int(state_path, "damage", 1);
        char mtype[64];
        read_kv_str_local(state_path, "monster_type", mtype, sizeof(mtype));

        if (mhp <= 0) {
            /* Monster dead - remove directory */
            remove(state_path);
            rmdir(state_path + (strlen(state_path) - strlen("/state.txt")));
            char details[128];
            snprintf(details, sizeof(details), "killed:%s", mtype);
            ledger_append(real_root, tick, "player", "kill", details);
            continue;
        }

        int nx = mx, ny = my;
        int dist = manhattan(mx, my, hero_x, hero_y);

        if (dist <= CHASE_RANGE) {
            /* Chase: move 1 step toward hero */
            int dx = 0, dy = 0;
            if (hero_x > mx) dx = 1; else if (hero_x < mx) dx = -1;
            if (hero_y > my) dy = 1; else if (hero_y < my) dy = -1;
            /* Prefer axis with larger gap */
            if (abs(hero_x - mx) >= abs(hero_y - my)) { nx = mx + dx; ny = my; }
            else { nx = mx; ny = my + dy; }
        } else {
            /* Wander: random 1 step */
            int rdx = (rand() % 3) - 1;
            int rdy = (rand() % 3) - 1;
            nx = mx + rdx; ny = my + rdy;
        }

        /* Clamp to 16x16 bounds */
        if (nx < 0) nx = 0; if (nx > 15) nx = 15;
        if (ny < 0) ny = 0; if (ny > 15) ny = 15;

        /* Monster attacks hero if on same tile */
        if (nx == hero_x && ny == hero_y) {
            hero_hp = clamp_int(hero_hp - mdmg, 0, 100);
            write_kv_int(hero_path, "hp", hero_hp);
            char atk_msg[128];
            snprintf(atk_msg, sizeof(atk_msg), "A %s hits you for %d damage!", mtype, mdmg);
            log_message(atk_msg);
            char details[128];
            snprintf(details, sizeof(details), "dmg:%d,hero_hp:%d", mdmg, hero_hp);
            ledger_append(real_root, tick, mtype, "attack", details);
            /* Monster stays on its position after attacking */
            nx = mx; ny = my;
        }

        if (nx != mx || ny != my) {
            write_kv_int(state_path, "pos_x", nx);
            write_kv_int(state_path, "pos_y", ny);
            char details[128];
            snprintf(details, sizeof(details), "x:%d,y:%d", nx, ny);
            ledger_append(real_root, tick, mtype, "chase", details);
        }
    }
}

/* Sprint 4: Bump attack - hero moves into a monster tile, deals damage. */
/* Called after hero movement. Returns message for the player. */
static void handle_bump_attack(const char *proj_root, char *message, size_t msg_sz) {
    char real_root[PATH_BUF];
    resolve_real_root(proj_root, real_root, sizeof(real_root));
    char hero_path[PATH_BUF];
    snprintf(hero_path, sizeof(hero_path), "%s/pieces/hero_01/state.txt", real_root);
    int hero_x = read_kv_int(hero_path, "pos_x", 0);
    int hero_y = read_kv_int(hero_path, "pos_y", 0);
    int hero_dmg = 2; /* fists base damage */

    /* Check inventory for best weapon */
    char inv_dir[PATH_BUF];
    snprintf(inv_dir, sizeof(inv_dir), "%s/pieces/hero_01/inventory", real_root);
    DIR *d = opendir(inv_dir);
    if (d) {
        struct dirent *entry;
        while ((entry = readdir(d)) != NULL) {
            if (entry->d_name[0] == '.') continue;
            char item_state[PATH_BUF + 384];
            snprintf(item_state, sizeof(item_state), "%s/%s/state.txt", inv_dir, entry->d_name);
            char item_id[64];
            read_kv_str_local(item_state, "item_id", item_id, sizeof(item_id));
            /* Look up power in registry */
            char reg_path[PATH_BUF];
            snprintf(reg_path, sizeof(reg_path), "%s/pieces/registry/items/items.txt", real_root);
            ...
            if (rf) {
                ...
                while (fgets(rline, sizeof(rline), rf)) {
                    ...
                    if ((size_t)(p1 - rline) == strlen(item_id) && strncmp(rline, item_id, p1 - rline) == 0) {
                        char *n = p1 + 1;
                        char *p2 = strchr(n, '|'); if (p2) *p2 = '\0';
                        char *cat = p2 ? p2 + 1 : NULL;
                        char *p3 = cat ? strchr(cat, '|') : NULL; if (p3) *p3 = '\0';
                        char *glyph = p3 ? p3 + 1 : NULL;
                        char *p4 = glyph ? strchr(glyph, '|') : NULL; if (p4) *p4 = '\0';
                        char *weight = p4 ? p4 + 1 : NULL;
                        char *p5 = weight ? strchr(weight, '|') : NULL; if (p5) *p5 = '\0';
                        char *power_str = p5 ? p5 + 1 : NULL;
                        if (power_str && cat && strcmp(cat, "weapon") == 0) {
                            int pw = atoi(power_str);
                            if (pw > hero_dmg) hero_dmg = pw;
                        }
                        break;
                    }
                }
    ...
    /* Scan monsters at hero position */
    char monsters_dir[PATH_BUF];
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_01/monsters", real_root);
    d = opendir(monsters_dir);
    if (!d) { snprintf(message, msg_sz, "You swing at the air."); return; }
    ...
    while ((entry = readdir(d)) != NULL) {
        ...
        snprintf(mstate, sizeof(mstate), "%s/%s/state.txt", monsters_dir, entry->d_name);
        int mx = read_kv_int(mstate, "pos_x", -1);
        int my = read_kv_int(mstate, "pos_y", -1);
        if (mx != hero_x || my != hero_y) continue;

        int mhp = read_kv_int(mstate, "hp", 10);
        char mtype[64];
        read_kv_str_local(mstate, "monster_type", mtype, sizeof(mtype));
        mhp -= hero_dmg;

        if (mhp <= 0) {
            snprintf(message, msg_sz, "You killed the %s!", mtype);
            remove(mstate);
            rmdir(mstate + (strlen(mstate) - strlen("/state.txt")));
            killed_any = 1;
        } else {
            write_kv_int(mstate, "hp", mhp);
            snprintf(message, msg_sz, "You hit the %s for %d! (%d hp left)", mtype, hero_dmg, mhp);
        }

        char details[128];
        char ws_path[PATH_BUF];
        snprintf(ws_path, sizeof(ws_path), "%s/pieces/world_01/state.txt", real_root);
        snprintf(details, sizeof(details), "dmg:%d,mhp:%d", hero_dmg, mhp);
        ledger_append(real_root, read_kv_int(ws_path, "tick", 0),
                      "player", "bump_attack", details);
        log_message(message);
        killed_any = 1;
        break;
    }
    closedir(d);
    if (!killed_any) snprintf(message, msg_sz, "You swing at the air.");
}

/* Sprint 4: Spawner system - every SPAWN_INTERVAL ticks, chance to spawn */
/* a new monster at a random position (not on hero). */
static void spawn_monsters(const char *proj_root, int tick) {
    if (tick % SPAWN_INTERVAL != 0) return;
    static int seeded = 0;
    if (!seeded) { srand((unsigned)time(NULL) ^ (unsigned)getpid()); seeded = 1; }
    if ((rand() % 100) >= SPAWN_CHANCE) return;
    ...
    /* Pick random spawn position at least 5 away from hero */
    int sx, sy;
    int attempts = 0;
    do {
        sx = rand() % 16;
        sy = rand() % 16;
        attempts++;
    } while (manhattan(sx, sy, hero_x, hero_y) < 5 && attempts < 50);

    /* Pick random monster type from registry */
    const char *types[] = {"zombie", "skeleton", "rat", "bat", "spider"};
    int type_idx = rand() % 5;
    const char *chosen_type = types[type_idx];

    /* Look up stats from registry */
    int reg_hp = 10, reg_dmg = 1;
    char reg_path[PATH_BUF];
    snprintf(reg_path, sizeof(reg_path), "%s/pieces/registry/monsters/monster_types.txt", real_root);
    FILE *rf = fopen(reg_path, "r");
    if (rf) {
        char rline[MAX_LINE];
        while (fgets(rline, sizeof(rline), rf)) {
            ...
            char *p1 = strchr(rline, '|');
            if (!p1) continue;
            if ((size_t)(p1 - rline) == strlen(chosen_type) && strncmp(rline, chosen_type, p1 - rline) == 0) {
                char *n = p1 + 1;
                char *p2 = strchr(n, '|'); if (p2) *p2 = '\0'; /* name */
                char *glyph = p2 ? p2 + 1 : NULL;
                char *p3 = glyph ? strchr(glyph, '|') : NULL; if (p3) *p3 = '\0'; /* glyph */
                char *hp_str = p3 ? p3 + 1 : NULL;
                char *p4 = hp_str ? strchr(hp_str, '|') : NULL; if (p4) *p4 = '\0';
                char *dmg_str = p4 ? p4 + 1 : NULL;
                if (hp_str) reg_hp = atoi(hp_str);
                if (dmg_str) reg_dmg = atoi(dmg_str);
                break;
            }
        }
        fclose(rf);
    ...
    /* Create monster piece directory */
    char monsters_dir[PATH_BUF];
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_01/monsters", real_root);
    mkdir(monsters_dir, 0755);
    char new_monster_dir[PATH_BUF + 128];
    snprintf(new_monster_dir, sizeof(new_monster_dir), "%s/%s_%d", monsters_dir, chosen_type, tick);
    mkdir(new_monster_dir, 0755);
    char new_state[PATH_BUF + 256];
    snprintf(new_state, sizeof(new_state), "%s/state.txt", new_monster_dir);
    FILE *wf = fopen(new_state, "w");
    if (wf) {
        fprintf(wf, "monster_type=%s\npos_x=%d\npos_y=%d\nhp=%d\ndamage=%d\n",
                chosen_type, sx, sy, reg_hp, reg_dmg);
        fclose(wf);
    }
    char details[128];
    snprintf(details, sizeof(details), "x:%d,y:%d,type:%s", sx, sy, chosen_type);
    ledger_append(real_root, tick, "spawner", "spawn", details);
    log_message("A new monster appears!");
}
...
int main(int argc, char **argv) {
    ...
    /* Auto-generate world on first init if no hero state exists.
     * Board-viewer widget is auto-launched by button.sh.
     * FIX: use resolve_real_root to find hero state in the base dir,
     * not the session dir (which lacks hero_01 symlink). */
    if (key == 0) {
        char real_root_check[PATH_BUF];
        resolve_real_root(project_root, real_root_check, sizeof(real_root_check));
        char hero_check[PATH_BUF];
        snprintf(hero_check, sizeof(hero_check), "%s/pieces/hero_01/state.txt", real_root_check);
        struct stat st;
        if (stat(hero_check, &st) != 0) {
            ...
            snprintf(gen_cmd, sizeof(gen_cmd),
                     "PRISC_PROJECT_ROOT='%s' '%s/ops/+x/mua_generate_chunk.+x' %u 0 0 >/dev/null 2>&1",
    ...
    /* === INTERACT MODE + VIEW MODE TOGGLES ===
     * Only active on the main screen. The 'i' key toggles interact mode.
     * In interact mode: arrow keys move cursor, 0-4 change view modes.
     * These override normal menu dispatch. */
    if (strcmp(active_piece, "main") == 0 && key != 0) {
        char real_root_ir[PATH_BUF];
        resolve_real_root(project_root, real_root_ir, sizeof(real_root_ir));
        char hero_state_ir[PATH_BUF];
        snprintf(hero_state_ir, sizeof(hero_state_ir), "%s/pieces/hero_01/state.txt", real_root_ir);

        int interact_mode = read_kv_int(hero_state_ir, "interact_mode", 0);
        int cursor_x = read_kv_int(hero_state_ir, "cursor_x", 8);
        int cursor_y = read_kv_int(hero_state_ir, "cursor_y", 8);
        int render_mode = read_kv_int(hero_state_ir, "render_mode", 0);
        char ir_msg[MAX_LINE];

        /* 'i' or 'I' toggles interact mode */
        if (key == 'i' || key == 'I') {
            interact_mode = !interact_mode;
            write_kv(hero_state_ir, "interact_mode", interact_mode ? "1" : "0");
            snprintf(ir_msg, sizeof(ir_msg), interact_mode ? "Interact mode ON (arrows=move, 0-4=view)" : "Interact mode OFF");
            write_kv(state_path, "last_message", ir_msg);
            char ir_mpath[PATH_BUF];
            snprintf(ir_mpath, sizeof(ir_mpath), "%s/pieces/display/pc_screen_changed.txt", project_root);
            { FILE *mf = fopen(ir_mpath, "a"); if (mf) { fputc('.', mf); fclose(mf); } }
            return 0;
        }

        /* In interact mode, handle cursor movement and view mode */
        if (interact_mode) {
            int moved = 0;
            if (key == 1000) { cursor_y = cursor_y > 0 ? cursor_y - 1 : 0; moved = 1; }      /* UP */
            else if (key == 1001) { cursor_y = cursor_y < 15 ? cursor_y + 1 : 15; moved = 1; } /* DOWN */
            else if (key == 1002) { cursor_x = cursor_x > 0 ? cursor_x - 1 : 0; moved = 1; }   /* LEFT */
            else if (key == 1003) { cursor_x = cursor_x < 15 ? cursor_x + 1 : 15; moved = 1; } /* RIGHT */
            /* View mode toggles 0-4 */
            else if (key >= '0' && key <= '4') {
                render_mode = key - '0';
                char rm_buf[4];
                snprintf(rm_buf, sizeof(rm_buf), "%d", render_mode);
                write_kv(hero_state_ir, "render_mode", rm_buf);
                snprintf(ir_msg, sizeof(ir_msg), "View mode: %s",
                         render_mode == 0 ? "2D Emoji" : render_mode == 1 ? "3D First-Person" :
                         render_mode == 2 ? "3D Third-Person" : render_mode == 3 ? "3D Free-Roam" : "3D Bird's-Eye");
                write_kv(state_path, "last_message", ir_msg);
            }

            if (moved) {
                write_kv_int(hero_state_ir, "cursor_x", cursor_x);
                write_kv_int(hero_state_ir, "cursor_y", cursor_y);
            }

            char ir_mpath2[PATH_BUF];
            snprintf(ir_mpath2, sizeof(ir_mpath2), "%s/pieces/display/pc_screen_changed.txt", project_root);
            { FILE *mf = fopen(ir_mpath2, "a"); if (mf) { fputc('.', mf); fclose(mf); } }
            return 0;
        }
    }
    ...
    else if (key == 10 || key == 13) resolved_item = 1; /* Enter = first menu item */
    ...
    if (cmd) {
        if (strcmp(cmd, "CONFIRM_START") == 0) {
             ...
             * mua_generate_chunk.+x does the actual terrain/world/hero
...
#ifdef _WIN32
            {
                ...
                int ok = win_run_pe("ops\\+x\\mua_generate_chunk.+x", args, project_root, 60000, 0);
                if (!ok) {
                    snprintf(message, sizeof(message),
                             "CONFIRM_START: mua_generate_chunk failed (Win spawn/write).");
                }
            }
#else
            {
...
#ifdef _WIN32
            {
                ...
                int ok = win_run_pe("ops\\+x\\mua_generate_chunk.+x", args, project_root, 120000, 0);
                if (!ok) {
                    snprintf(message, sizeof(message),
                             "CONFIRM_START_DEBUG: mua_generate_chunk failed to launch.");
                }
            }
#else
            {
                char gen_cmd[PATH_BUF + 64];
                snprintf(gen_cmd, sizeof(gen_cmd),
                         "PRISC_PROJECT_ROOT='%s' '%s/ops/+x/mua_generate_chunk.+x' %u 0 0 flat >/dev/null 2>&1",
                         project_root, project_root, world_seed);
                { int _rc = system(gen_cmd); (void)_rc; }
            }
#endif
            ...
            /* Sprint 2: survival metabolism runs on every END_TURN.
             * hunger/thirst rise, stamina regens, HP loss on starvation. */
            int tick = advance_tick(project_root);
            char root_e[PATH_BUF];
            resolve_real_root(project_root, root_e, sizeof(root_e));
            char hero_e[PATH_BUF];
            snprintf(hero_e, sizeof(hero_e), "%s/pieces/hero_01/state.txt", root_e);
            tick_hero_metabolism(hero_e);
            char details[128];
            ...
            tick_monsters(project_root, tick);
            spawn_monsters(project_root, tick);
            snprintf(message, sizeof(message), "Tick %d.", tick);
        } else if (strcmp(cmd, "EAT") == 0) {
            /* Sprint 2: eat/drink from inventory. */
            handle_eat(project_root, message, sizeof(message));
        } else if (strcmp(cmd, "PICKUP") == 0) {
            /* Sprint 3: pick up item at hero position. */
            handle_pickup(project_root, message, sizeof(message));
        } else if (strcmp(cmd, "DROP") == 0) {
            /* Sprint 3: drop first inventory item. */
            handle_drop(project_root, message, sizeof(message));
            ...
            int tick = advance_tick(project_root);
            char root_m[PATH_BUF];
            resolve_real_root(project_root, root_m, sizeof(root_m));
            char hero_m[PATH_BUF];
            snprintf(hero_m, sizeof(hero_m), "%s/pieces/hero_01/state.txt", root_m);
            tick_hero_metabolism(hero_m);
            char details[128];
            snprintf(details, sizeof(details), "tick:%d", tick);
            ...
            tick_monsters(project_root, tick);
            spawn_monsters(project_root, tick);
            /* Sprint 4: bump attack - hero may have moved onto a monster */
            handle_bump_attack(project_root, message, sizeof(message));
             ...
             * See mutaclysm-blueprint.md once written for the real
             * design this will eventually open. */
            snprintf(message, sizeof(message),
                     "View Editor - not built yet (see mutaclysm-blueprint.md).");
        }
    }
