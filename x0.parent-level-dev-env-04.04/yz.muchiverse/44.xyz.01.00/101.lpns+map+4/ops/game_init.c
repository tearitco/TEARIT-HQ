/* game_init.c - Initialize game with map support
 * Setup: players, NPCs, epochs, starting positions
 * Self-contained */
...
int main(int argc, char **argv) {
    ...
    printf("=========================================\n");
    printf("  LEDGER-DRIVEN MAP GAME (LPNS+MAP)\n");
    printf("  Word Game + Spatial Movement\n");
    printf("=========================================\n\n");
    ...
    /* Collect player info (up to 8 actors: 4 players + 4 NPCs) */
    char player_types[9][20] = {"", "human", "human", "human", "human", "", "", "", ""};
    char player_names[9][50] = {"", "Player1", "Player2", "Player3", "Player4", "", "", "", ""};
    int player_start_x[9] = {0, 0, 7, 0, 7, 0, 7, 0, 7};
    int player_start_y[9] = {0, 0, 0, 7, 7, 0, 0, 7, 7};
    ...
    if (!fp) {
        fprintf(stderr, "Error: cannot write config to %s\n", config_path);
        perror("fopen failed");
        return 1;
    }
    ...
    for (int i = 1; i <= 8; i++) {
        ...
        fprintf(fp, "player_%d_start_x=%d\n", i, player_start_x[i]);
        fprintf(fp, "player_%d_start_y=%d\n", i, player_start_y[i]);
    }
    ...
    printf("  %d human players + %d NPCs = %d total\n", num_players, num_npcs, total_actors);
    printf("  %d epochs\n", epoch_length);
    printf("  Map size: 8x8\n");
    printf("=========================================\n\n");
