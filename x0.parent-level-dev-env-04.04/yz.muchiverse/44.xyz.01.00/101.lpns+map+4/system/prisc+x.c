...
void parse_line(char *line, int pass) {
    ...
    if (pass == 2) {
        ...
        if (op_idx >= 0) {
            ...
            /* FIX (2026-07-20, same class of bug as OP_READ_HISTORY's own
             * rs1 fix above - see this file's own header comment):
             * i->rs1/i->rs2 default to 0 (a real register index, reset
             * every instruction) whenever a shorter exec form leaves the
             * count>=2/count>=3 branches below unreached. The runtime
             * exec handler treats `rs1 >= 0`/`rs2 >= 0` as "a register
             * WAS given" - so a bare `exec <path>` (one token) or
             * `exec <path> <literal>` (two tokens) previously appended
             * spurious "0"/"0 0" trailing arguments to the shell command
             * every single time, silently. Never hit in practice only
             * because no pal script anywhere has used `exec` with fewer
             * than 3 tokens yet (grep-confirmed family-wide). Explicit
             * -1 sentinels here, matching this same function's own
             * literal-flag convention a few lines below, make "no
             * register/arg given" and "register 0" distinguishable. */
            i->op = OP_EXEC;
            i->rs1 = -1;
            i->rs2 = -1;
            char *args = strstr(original, part) + strlen(part);
            ...
            if (count >= 3) {
                if (sscanf(arg3, "r%d", &i->rs2) != 1 && sscanf(arg3, "x%d", &i->rs2) != 1) {
                    /* Literal 3rd token - same treatment as arg2 above.
                     * literal_arg3 doesn't exist on Inst; reusing
                     * literal_arg2 with a space-joined "arg2 arg3" is a
                     * new field away from being right, so instead this
                     * is intentionally still not supported as a LITERAL
                     * (only as a register, via rs2 above) - a script
                     * needing 2 literal args should shell to a wrapper
                     * script that takes one literal arg and does its own
                     * further dispatch, matching every op-calls-op
                     * pattern already used family-wide. */
                }
            }
...
int main(int argc, char **argv) {
    ...
    while (pc < inst_count) {
        ...
        if (i.op == OP_CUSTOM) {
            ...
            /* Bible §3: fork/exec/waitpid, NOT system().
             * system() forks a shell which adds ~50-100ms overhead per call.
             * Direct fork/exec matches C game_manager's run_op() exactly. */
            pid_t pid = fork();
            if (pid == 0) {
                /* Child: redirect stdout/stderr to /dev/null, exec the op */
                ...
                if (strlen(arg2) > 0)
                    execl(i.literal_arg, i.literal_arg, arg1, arg2, NULL);
                else if (strlen(arg1) > 0)
                    execl(i.literal_arg, i.literal_arg, arg1, NULL);
                else
                    execl(i.literal_arg, i.literal_arg, NULL);
                _exit(1);
            } else if (pid > 0) {
                int status;
