...
void inject_raw_key(int code) {
    ...
    if (dbg) { fprintf(dbg, "[PARSER] inject_raw_key(%d) called, interact_history_path='%s'\n", code, interact_history_path); fclose(dbg); }
    ...
    if (interact_history_path[0] != '\0') {
        ...
        if (fp) {
            ...
            fclose(fp);
            if (dbg) { dbg = fopen("debug.txt", "a"); fprintf(dbg, "[PARSER] Wrote %d to %s\n", code, full_path); fclose(dbg); }
        }
        free(full_path);
        return;
    }
...
void process_key(int key) {
    ...
    if (active_index == -1) {
        ...
        /* INTERACT mode: raw key injection to interact_relay.txt */
