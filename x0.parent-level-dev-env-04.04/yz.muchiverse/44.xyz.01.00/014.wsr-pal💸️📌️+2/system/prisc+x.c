...
/* MinGW provides asprintf when _GNU_SOURCE is set; if not, provide fallback. */
#ifndef asprintf
static int wsr_asprintf(char **strp, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
/* SHELL-METACHAR PATH FIX (file-menu widget nav bug, 2026-07-29):
 * full_script_path below is PRISC_PROJECT_ROOT + the op's handler path,
 * interpolated unquoted into a popen()/system() command string. Any
 * project living under a directory whose name contains a shell
 * metacharacter - confirmed live with &.widgits/file-menu, where the
 * literal "&" backgrounds the first half of the path and "sh: ...: not
 * found"/"Permission denied" silently swallow the custom op (its own
 * marker file never gets touched, so the PAL loop's marker-diff gate
 * never fires either - looks exactly like "the op never ran" from the
 * caller's side). Single-quote wrapping (with the standard '\'' escape
 * for any literal single quotes already in the path) neutralizes every
 * shell metacharacter without changing behavior for any path that
 * didn't have one. */
...
void exec_custom_op(Inst *i) {
    ...
    if (strncmp(op->handler, "+x/", 3) == 0 || strstr(op->handler, "/+x/")) {
        ...
        if (ext) {
            ...
            char *quoted_script_path = shell_quote(full_script_path);

            /* For layout_op, use system() for interactive stdin */
            if (strcmp(i->custom_name, "layout_op") == 0) {
                /* Pass menu_id from rs1, result goes to shared temp file */
                char layout_result_path[MAX_PATH];
#ifdef _WIN32
                {
                    char tmpdir[MAX_PATH];
                    DWORD n = GetTempPathA(sizeof(tmpdir), tmpdir);
                    if (n == 0 || n >= sizeof(tmpdir)) snprintf(tmpdir, sizeof(tmpdir), ".");
                    snprintf(layout_result_path, sizeof(layout_result_path),
                             "%sprisc_layout_result.txt", tmpdir);
                }
#else
                snprintf(layout_result_path, sizeof(layout_result_path),
                         "/tmp/prisc_layout_result.txt");
#endif
                if (asprintf(&cmd, "%s %d > \"%s\"",
                         quoted_script_path, regs[i->rs1], layout_result_path) != -1) {
                ...
                FILE *rf = fopen(layout_result_path, "r");
                if (rf) {
                    char result[16];
