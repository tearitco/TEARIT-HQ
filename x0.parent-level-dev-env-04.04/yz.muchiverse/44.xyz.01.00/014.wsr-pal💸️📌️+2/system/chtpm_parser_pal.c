...
#ifdef _WIN32
/* Resolve MinGW-style binaries: system/prisc+x may exist only as prisc+x.exe.
 * Also prefer a path relative to CWD when the absolute path sits under
 * project_root — absolute house paths often contain emoji/Unicode that
 * CreateProcessA cannot open. UTF-16 CreateProcessW is used for safety. */
static int win_utf8_to_wide(const char *utf8, wchar_t *out, int out_chars) {
    ...
    if (n > 0) return n;
    /* Fallback: system ANSI code page (paths that aren't valid UTF-8) */
    n = MultiByteToWideChar(CP_ACP, 0, utf8, -1, out, out_chars);
    return n;
}
...
intptr_t win_spawn(const char* path, char* const args[]) {
    ...
    /* 1) Prefer relative path under project_root (avoids long Unicode abs path) */
    ...
    if (!resolved[0]) {
        /* 2) If path is already relative-looking, keep it; else use as-is */
        snprintf(resolved, sizeof(resolved), "%s", path);
    }
    win_slash_to_back(resolved);

    /* 3) MinGW: add .exe when the bare name is missing */
    ...
    if (!win_file_exists_a(resolved) && win_file_exists_a(try_exe)) {
        ...
        /* Also try original path + .exe (absolute) */
        ...
        if (win_file_exists_a(abs_exe)) {
            ...
            /* last resort: absolute original with backslashes */
            snprintf(resolved, sizeof(resolved), "%s", path);
            win_slash_to_back(resolved);
            snprintf(try_exe, sizeof(try_exe), "%s.exe", resolved);
            if (!win_file_exists_a(resolved) && win_file_exists_a(try_exe))
    ...
    /* Build UTF-8 command line: "exe" "arg1" "arg2" ... */
    char cmd_utf8[MAX_PATH * 4];
    cmd_utf8[0] = '\0';
    {
        size_t off = 0;
        /* Always put resolved exe first */
        off += (size_t)snprintf(cmd_utf8 + off, sizeof(cmd_utf8) - off, "\"%s\"", resolved);
        if (args) {
            /* skip args[0] if it is the same executable token; re-emit remaining */
            int start = 0;
            if (args[0]) start = 1;
    ...
    if (win_utf8_to_wide(cmd_utf8, wcmd, (int)(sizeof(wcmd) / sizeof(wcmd[0]))) <= 0)
        return -1;
    if (win_utf8_to_wide(resolved, wexe, (int)(sizeof(wexe) / sizeof(wexe[0]))) <= 0)
        return -1;
    ...
    /* CREATE_NO_WINDOW: daemons (prisc+x) should not steal the console.
     * lpApplicationName = resolved exe; lpCommandLine is mutable. */
    if (CreateProcessW(wexe, wcmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        CloseHandle(pi.hThread);
        return (intptr_t)pi.hProcess;
    }

    /* Retry with NULL app name (SearchPath / PATHEXT style) — some MinGW
     * names only resolve that way when CWD is correct. */
...
void process_key(int key) {
    ...
    if (active_index == -1) {
         ...
         * typed as ordinary nav/content input. Real quit is Ctrl+C
         * (keyboard_input.c already used raw ETX/3 here, not 'q' -
         * this file's own stray check was the one remaining exception). */
        else {
