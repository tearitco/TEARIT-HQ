/* pet_pet - one verb, one binary, no shared headers.
 * Petting a pet: happiness up, no other cost. Same generic
 * <piece_id>-argument shape as feed_pet.c/play_pet.c - see that
 * file's own header comment.
 * Usage: pet_pet.+x <piece_id>
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    set_kv_int(state_path, "happiness", clamp(happiness + 10, 0, 100));
    ...
    if (lf) { fprintf(lf, "%s enjoys the attention.\n", name); fclose(lf); }
    return 0;
}
