 ...
 * or quit_request.txt; we yield so button.sh can exec the child on this
 * terminal, then relaunch the loader when the child exits. */
static int handoff_or_quit_requested(void) {
    char path[PATH_BUF];
    struct stat st;
    snprintf(path, sizeof(path), "%s/pieces/system/handoff_launch.txt", project_root);
    if (stat(path, &st) == 0 && st.st_size > 0) return 1;
    snprintf(path, sizeof(path), "%s/pieces/system/quit_request.txt", project_root);
    if (stat(path, &st) == 0 && st.st_size > 0) return 1;
    return 0;
}
...
int main(void) {
    ...
    for (;;) {
        int key = read_key();
        if (key == -2) break; /* handoff / quit_request — yield TTY to button.sh */
        if (key == -1) continue;
