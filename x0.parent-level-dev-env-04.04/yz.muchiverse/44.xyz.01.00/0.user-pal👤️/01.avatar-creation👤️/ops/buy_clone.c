/* buy_clone - deduct 20 tokens, mint via generate_clone.+x
 * Usage: buy_clone.+x */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
/* Prefer session.pdl when logged_in (current_login often empty). */
...
int main(void) {
    ...
    resolve_login_root();
    resolve_house_root();
    char login_path[PATH_BUF], user_uuid[128], xyzfs_rel[512];
    ...
    if (!xyzfs_rel[0]) read_kv(login_path, "current_xyzfs", xyzfs_rel, sizeof(xyzfs_rel));
    ...
    int bal = 0;
    if (!adjust_tokens(wallet, -CLONE_COST, &bal)) {
        printf("Buy failed: need %d tokens, have %d.\n", CLONE_COST, bal);
        return 1;
    }

    char cmd[PATH_BUF * 2];
    snprintf(cmd, sizeof(cmd), "cd '%s' && ./ops/+x/generate_clone.+x 'Clone' 2>/dev/null", project_root);
    FILE *pf = popen(cmd, "r");
    char id[128] = "";
    if (pf) {
        if (!fgets(id, sizeof(id), pf)) id[0] = '\0';
        pclose(pf);
    }
    id[strcspn(id, "\r\n")] = '\0';
    if (!id[0]) {
        adjust_tokens(wallet, CLONE_COST, &bal);
        printf("Buy failed: mint returned nothing.\n");
        return 1;
    }
    printf("Bought clone %s! Balance: %d\n", id, bal);
    return 0;
}
