...
| taskbar | `_.monads/_.livedesk-taskbar/ops/khtpm_strip_parser.c` + `khtpm_taskbar_manager.c` — real declarative-layout parser (2 processes). Legacy `tp_taskbar.c` fully retired 2026-08-11, archived. | persistent header + submenu popups + bottom tab bar; unified nav cursor; save/load/session/desk switching; agent relay |
