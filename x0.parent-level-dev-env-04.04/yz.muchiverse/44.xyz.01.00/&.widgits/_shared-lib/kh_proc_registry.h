...
/* one registry row */
typedef struct { long pid, pgid; unsigned long long start; char name[128]; } KhprRow;
...
static int khpr_load(const char *path, KhprRow *rows, int max) {
    ...
    while (n < max && fgets(line, sizeof(line), f)) {
        KhprRow r; r.name[0] = '\0';
        unsigned long long st = 0ULL;
        int got = sscanf(line, "%ld %ld %llu %127s",
...
int kh_proc_registry_prune(const char *house_root) {
    ...
    int kept = 0;
    for (int i = 0; i < n; i++) {
        if (!khpr_alive_matches(rows[i].pid, rows[i].start)) continue;
        fprintf(f, "%ld %ld %llu %s\n", rows[i].pid, rows[i].pgid,
                rows[i].start, rows[i].name);
        kept++;
    }
