...
int main(int argc, char *argv[]) {
    ...
    int stock_index = -1;
    float available_shares = 0.0;
    for (int i = 0; i < stocks_count; i++) {
        if (strcmp(stocks[i], symbol) == 0) {
            stock_index = i;
            available_shares = shares[i];
            break;
        }
    }
    if (stock_index == -1 || available_shares < shares_to_sell) {
        printf("[%s] Insufficient shares: %.2f requested, %.2f available\n", symbol, shares_to_sell, available_shares);
        return 1;
    }
    ...
    shares[stock_index] -= shares_to_sell;
    balance += proceeds;
    if (shares[stock_index] == 0) {
    ...
    if (history_count < MAX_HISTORY) {
        ...
        history_shares[history_count] = -shares_to_sell; // Negative for sell
