/*
 * nb_video_player.c - house vendored copy of wraith-alpha's
 * wraith_video_player.c (chtmgl-video-isolate, ops/src/). Same contract,
 * kept as its own copy because the wraith path lives in another tree.
 * Forward-diffs: copy from wrapping project when wraith updates.
 *   wraith_video_player <video_path|--stop|--pause|--resume> [project_root]
 */
#include <signal.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr, "usage: nb_video_player <video_path|--stop|--pause|--resume> [project_root]\n");
        return 1;
    }
