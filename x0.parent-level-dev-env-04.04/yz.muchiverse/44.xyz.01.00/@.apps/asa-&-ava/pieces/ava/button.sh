...
# uses - see TILE_PICKER_DESIGN.md §2.3): ava spawns at column 6, asa
...
case "$ACTION" in
    run|r|start)
        ensure_package
        if [ -x "$TP/+x/khtpm_entity.+x" ]; then
            setsid nohup "$TP/+x/khtpm_entity.+x" "$PKG" >/dev/null 2>&1 < /dev/null &
        ...
        ;;
    kill|k|stop)
        pkill -f "khtpm_entity.+x $PKG" 2>/dev/null
        echo "done"
        ;;
    check|verify)
        [ -x "$TP/+x/khtpm_entity.+x" ] && echo "OK   khtpm_entity.+x" || echo "MISSING khtpm_entity.+x"
