# button.ps1 - Windows launcher for aomorai-editor (parity with button.sh)
...
# House = two levels up from @.apps/aomorai-editor
...
function Invoke-Kill {
    Write-Host "Stopping aomorai-editor processes (name-only)..."
    $names = @(
...
function Invoke-Run {
    Write-Host "=== aomorai-editor Windows launcher ===" -ForegroundColor Cyan
    ...
    foreach ($d in @(
        ...
        "pieces\keyboard", "pieces\os", "projects\aomorai-editor\manager"
    )) {
    ...
    Copy-TreeLink (Join-Path $SCRIPT_DIR "projects\aomorai-editor\pieces") (Join-Path $SESSION "projects\aomorai-editor\pieces")
...
"@
    ...
    $env:PRISC_PROJECT_ID = "aomorai-editor"
    $env:NO_NET = "1"
...
elseif ($a -in @("check", "verify")) { exit (Invoke-Check) }
    ...
    Write-Host @"
aomorai-editor button.ps1 (Windows)
