# PHASE 2 PLAN — piececraft-xyz, written 2026-08-03
...
- The 2D render path (`chtpm_rgb_render.c` itself) calls the on-demand
  ...
  even before any texture files existed on disk for piececraft-xyz's
  own glyphs).
- The 3D raymarcher (`bv_render_3d.c`) does **NOT** call the generator
  ...
  file is missing/empty. This is why several of piececraft-xyz's own
...
- `hero_01`'s own real identity should come FROM a real avatar-creation
  record, not be piececraft-xyz's own separately-invented player
  ...
  don't invent piececraft-xyz's own separate hunger field), real
...
- No login yet / no avatar created yet: graceful default, matching this
  ...
  this same fallback shape) - piececraft-xyz gets a real, generic
