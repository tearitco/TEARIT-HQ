/* media_img3d_hq_manager.c - <module> backend for media-img3d-hq.xhtpm.
 *
 * CONVERSION SKELETON 2026-09-08. Combined 2D image editor + 3D (Blender-shaped) viewport, one app, piececraft-hq-style camera (owner's explicit call).
 * Source app being converted: 44.xyz.01.00/103.media-studio/103.img-editor + 103.3d=blender-clone/
 ...
 * in 103.media-studio/103.img-editor + 103.3d=blender-clone/HOW2_IMAGE.md / HOW2_BLEND.md . See ../README.md for the plan.
 *
 * Forked as: media_img3d_hq_manager.+x <house_root> <package_dir> [id]
 * Publishes <pkg>/state/media_img3d_hq_ui.txt ; polls
 * <pkg>/state/media_img3d_hq_action.txt (seq=N / cmd=...).
 *
...
static void write_ui(void){
    ...
    fprintf(f,"title=Muchi Image + 3D\n");
    fprintf(f,"banner=CONVERSION IN PROGRESS - see @.apps/media-img3d-hq/README.md\n");
    fprintf(f,"source=103.media-studio/103.img-editor + 103.3d=blender-clone/  (HOW2_IMAGE.md / HOW2_BLEND.md)\n");
    fprintf(f,"ticks=%lld\n",(long long)g_ticks);
    fprintf(f,"msg=%s\n",g_msg);
