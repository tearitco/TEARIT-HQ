#!/bin/bash
# test-harn-same/button.sh - MINIMAL entry point for tactics-txt's
...
case "$ACTION" in
    ...
    demo)
        ...
        bash "$SCRIPT_DIR/scenarios/demo_setup_and_battle.sh"
        ;;
    all)
    ...
    kill)
        ...
        ;;
    help|h|-h|--help)
        echo "tactics-txt test-harn-same/button.sh"
        echo ""
        echo "Usage: ./button.sh <action>"
        echo "  compile, c, build   - Build the 4 reusable tk_* test ops"
        echo "  demo                - Run the default scenario (demo_setup_and_battle.sh)"
        echo "  all                 - Run every scenarios/demo_*.sh, print KPI summary"
        echo "  kill                - Kill any lingering tactics-txt processes"
        ;;
    *)
