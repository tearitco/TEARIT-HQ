# button.ps1 - Windows launcher for my-biotech. Linux: button.sh
...
function Invoke-Run{
  Write-Host "=== my-biotech Win ($LAYOUT) ===" -ForegroundColor Cyan
  ...
  foreach($d in @("pieces\system","pieces\display","pieces\apps\player_app","pieces\keyboard","pieces\os","projects\$PROJDIR\manager","projects\$PROJDIR\pieces\mybiotech_menu")){
  ...
  foreach($f in @("pieces\apps\player_app\interact_relay.txt","pieces\keyboard\history.txt","pieces\system\quit_flag.txt","pieces\display\mybiotech_screen_changed.txt","pieces\display\frame_changed.txt","projects\$PROJDIR\manager\gui_state.txt")){
  ...
  if(-not(Test-Path $cfg)){
    Write-NoBom $cfg "game_id=my-biotech-001`nplayer_name=Adam`nday=1`nmax_days=10`nhealth=100`nmoney=500`ngame_state=playing`n"
  }
...
switch($a){
  ...
  default{ Write-Host "my-biotech button.ps1: compile|run|kill|check|help  (wsr PE donor)"; exit 0 }
}
