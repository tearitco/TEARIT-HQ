/* media_vid_hq_manager.c - <module> backend for media-vid-hq.xhtpm.
 *
 * CONVERSION SKELETON 2026-09-08. iMovie-shaped video editor: transport + timecode, preview + inspector, V1/V2/A1/A2 timeline.
 * Source app being converted: 44.xyz.01.00/103.media-studio/103.vid-edit/
 ...
 * in 103.media-studio/103.vid-edit/HOW2_VIDEO.md . See ../README.md for the plan.
 *
 * Forked as: media_vid_hq_manager.+x <house_root> <package_dir> [id]
 * Publishes <pkg>/state/media_vid_hq_ui.txt ; polls
 * <pkg>/state/media_vid_hq_action.txt (seq=N / cmd=...).
 *
...
static void write_ui(void){
    ...
    fprintf(f,"title=Muchi Video\n");
    fprintf(f,"banner=CONVERSION IN PROGRESS - see @.apps/media-vid-hq/README.md\n");
    fprintf(f,"source=103.media-studio/103.vid-edit/  (HOW2_VIDEO.md)\n");
    fprintf(f,"ticks=%lld\n",(long long)g_ticks);
    fprintf(f,"msg=%s\n",g_msg);
