/* pc_compose_frame - renders whichever piececraft-xyz screen is current into
...
int main(void) {
    ...
    if (strcmp(active_piece, "new_game") == 0) {
        ...
        snprintf(pdl_path, sizeof(pdl_path), "%s/projects/piececraft-xyz/pieces/new_game/piece.pdl", project_root);
        ...
        char pdl_path[PATH_BUF];
        snprintf(pdl_path, sizeof(pdl_path), "%s/projects/piececraft-xyz/pieces/main/piece.pdl", project_root);
        FILE *pdl_out = fopen(pdl_path, "w");
        if (pdl_out) {
            fprintf(pdl_out, "SECTION      | KEY                | VALUE\n");
            fprintf(pdl_out, "----------------------------------------\n");
