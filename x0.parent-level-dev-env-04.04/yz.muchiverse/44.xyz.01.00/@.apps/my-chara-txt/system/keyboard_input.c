...
static int read_key(void) {
    ...
    while ((nread = read(STDIN_FILENO, &c, 1)) != 1) {
        /* XYZOS-PITFALLS #22/#51: usleep MUST run before the error-path
         * return, not after - #22's original fix only throttled the
         * nread==0 (EOF) case; the nread==-1 hard-error path (real,
         * live-observed in headless/API-sandbox environments where
         * read() on a backgrounded process's stdin returns a genuine
         * error, not EAGAIN, not clean EOF) hit `return -1` on the same
         * line BEFORE ever reaching usleep(), reproducing the identical
         * 100%-CPU busy-spin #22 already claimed was fixed. Moving the
         * sleep first throttles EVERY non-1 read result, whichever
         * branch caused it, while preserving the existing return
         * contract (still returns -1 upward on a real hard error). */
        usleep(20000);
