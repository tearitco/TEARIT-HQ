# button.ps1 - Windows launcher for my-chara-txt. Linux: button.sh
...
function Invoke-Run{
  Write-Host "=== my-chara-txt Win ($LAYOUT) ===" -ForegroundColor Cyan
  ...
  foreach($d in @("pieces\system","pieces\display","pieces\apps\player_app","pieces\keyboard","pieces\os","projects\$PROJDIR\manager")){
  ...
  foreach($f in @("pieces\apps\player_app\interact_relay.txt","pieces\keyboard\history.txt","pieces\system\quit_flag.txt","pieces\display\mychara_screen_changed.txt","pieces\display\frame_changed.txt","projects\$PROJDIR\manager\gui_state.txt")){
    Write-NoBom (Join-Path $SESSION $f) ""
  }
  $realSys=Join-Path $SCRIPT_DIR "pieces\system"
  New-Item -ItemType Directory -Path $realSys -Force|Out-Null
...
"@
  ...
  $plots=Join-Path $realSys "plots.txt"
  if(-not(Test-Path $plots)){
    Write-NoBom $plots "plot_0_state=empty`nplot_0_crop=`nplot_0_harvest_day=0`nplot_1_state=empty`nplot_1_crop=`nplot_1_harvest_day=0`nplot_2_state=empty`nplot_2_crop=`nplot_2_harvest_day=0`n"
  }
  Copy-TreeLink $cfg (Join-Path $SESSION "pieces\system\config.txt")
  Copy-TreeLink $plots (Join-Path $SESSION "pieces\system\plots.txt")
...
switch($a){
  ...
  default{ Write-Host "my-chara-txt button.ps1: compile|run|kill|check|help  (wsr PE donor)"; exit 0 }
}
