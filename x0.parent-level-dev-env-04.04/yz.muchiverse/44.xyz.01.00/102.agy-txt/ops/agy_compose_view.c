/* agy_compose_view - render agy-txt chrome + buffer canvas into
 * pieces/apps/player_app/view.txt (${game_map}). Copied near-verbatim
 * from 102.editor-📄️00.00/ops/editor_compose_frame.c (PLAN.md §2).
 *
 * Buffer lives at pieces/system/editor_buffer.txt.
 * Cursor: pieces/system/editor_state.txt cursor_pos=N (-1 = end).
 * Cursor glyph in canvas: [X]
 *
 * Writes ONLY view.txt + pings frame_changed.txt (ONE VISIBLE FRAME WRITER).
 * Usage: agy_compose_view.+x
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
