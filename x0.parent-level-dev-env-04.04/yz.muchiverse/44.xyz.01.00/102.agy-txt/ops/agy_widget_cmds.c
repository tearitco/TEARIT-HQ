/* agy_widget_cmds - process command inbox (NEW/SAVE/SAVE_AS/LOAD/PING).
 * Copied VERBATIM (no path changes needed beyond project_id, PLAN.md
 * §2) from 102.editor-📄️00.00/ops/editor_widget_cmds.c - this file
 * never referenced file-menu directly, it only reads/writes files
 * under its own project_root, so it's already fully generic. agy-txt
 * has no separate widget process to relay FROM - Phase T4 writes
 * directly into this same inbox from agy-txt's OWN file_menu/
 * file_browser key handlers instead, reusing the drain side unchanged.
 *
 ...
 *   PING
 *
 * Usage: agy_widget_cmds.+x [max_cmds]
