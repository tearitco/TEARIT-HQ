/* editor_menu_input - INTERACT canvas typing + piece.pdl METHOD dispatch.
 *
 * Usage: editor_menu_input.+x <keycode>
...
static int load_menu_items(MenuItem *items, int max_items) {
    ...
    snprintf(pdl_path, sizeof(pdl_path),
             "%s/projects/agy-editor/pieces/editor/piece.pdl", project_root);
...
int main(int argc, char **argv) {
    ...
    int key = atoi(argv[1]);
    if (key == 0) {
        /* idle: drain widget/file-menu cmd inbox (LOAD/SAVE/NEW).
         * Separate op keeps harness able to call it explicitly too. */
        char cmd[PATH_BUF * 2];
        snprintf(cmd, sizeof(cmd),
                 "cd '%s' && ./ops/+x/editor_widget_cmds.+x 8 >/dev/null 2>&1",
                 project_root);
        system(cmd);
        return 0;
    }
    ...
    /* arrows: left/right char, up/down line (same column) */
