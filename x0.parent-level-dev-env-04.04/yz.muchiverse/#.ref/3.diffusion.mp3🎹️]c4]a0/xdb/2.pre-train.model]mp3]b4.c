...
// Escape file path for FFmpeg command
            ...
            if (use_identity && c == 0) {
                ...
                for (int kx = -4; kx <= 4; kx++) {
                    ...
                    if (idx >= 0 && idx < length * channels) {
                        ...
                        fprintf(log_fp, "Warning: Out-of-bounds access in convolve at x=%d, c=%d, kx=%d\n", x, c, kx);
                    }
                }
            }
    ...
    for (int c = 0; c < channels; c++) {
        for (int x = 4; x < length - 4; x++) {
            for (int kx = -4; kx <= 4; kx++) {
                ...
                int idx = (x + kx) * channels + c;
                if (idx >= 0 && idx < length * channels) {
                    float grad = grad_output[x * channels + c] * input[idx];
                    grad_kernels[kidx] += grad;
                } else {
                    fprintf(log_fp, "Warning: Out-of-bounds access in convolve_backward at x=%d, c=%d, kx=%d\n", x, c, kx);
                }
            }
        }
    }
...
void upsample(float *input, float *output, int length, int channels, FILE *log_fp) {
    ...
    for (int x = 0; x < out_length; x++) {
        for (int c = 0; c < channels; c++) {
            int in_x = x / 2;
            if (in_x < length) {
                output[x * channels + c] = input[in_x * channels + c];
            } else {
                output[x * channels + c] = 0.0f;
            }
        }
    }
...
int load_audio(const char *filepath, float **audio_data, int *length, int channels, FILE *log_fp) {
    ...
    char cmd[2048];
    snprintf(cmd, sizeof(cmd), "ffmpeg -y -i \"%s\" -f f32le -ar 44100 -ac %d -loglevel quiet temp_audio.raw", escaped_filepath, channels);
