...
int main(int argc, char *argv[]) {
    ...
    if (train_mode) {
        ...
        float *pool_temp = (float *)malloc((length / 2) * channels * sizeof(float));
        if (!temp || !pool_temp) {
            fprintf(log_fp, "Error: Memory allocation failed for temp buffers\n");
            free(input);
            free(output);
            free(temp);
            free(pool_temp);
        ...
        for (int epoch = 0; epoch < NUM_EPOCHS; epoch++) {
            ...
            max_pool(output, pool_temp, length, channels, log_fp);
            convolve(pool_temp, output, length / 2, channels, kernels2, log_fp);
            group_norm(output, length / 2, channels, log_fp);
            upsample(output, temp, length / 2, channels, log_fp);
            memcpy(output, temp, length * channels * sizeof(float)); // Fixed: Copy from temp to output
            // Simplified training (no weight updates yet)
            fprintf(log_fp, "Training epoch %d completed\n", epoch + 1);
        }
        free(temp);
        free(pool_temp);
    } else {
        float pool_temp[MAX_AUDIO_LENGTH / 2];
        convolve(input, output, length, channels, kernels, log_fp);
        group_norm(output, length, channels, log_fp);
        max_pool(output, pool_temp, length, channels, log_fp);
        convolve(pool_temp, output, length / 2, channels, kernels2, log_fp);
        group_norm(output, length / 2, channels, log_fp);
        upsample(output, output, length / 2, channels, log_fp);
        add_residual(output, input, size, 0.1f, log_fp);
    }
