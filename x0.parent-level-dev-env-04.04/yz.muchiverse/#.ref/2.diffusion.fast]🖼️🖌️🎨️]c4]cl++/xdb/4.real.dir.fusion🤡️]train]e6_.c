...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < selected_count; i++) {
        ...
        snprintf(command, MAX_PATH * 2, "%sneural_net.+x %s %s train", MODULE_DIR, temp_files[i], neural_output_files[i]);
        printf("Training on %s...\n", temp_files[i]);
        if (system(command) != 0) {
            fprintf(stderr, "Error: Failed to execute neural_net.+x train for %s\n", temp_files[i]);
            cleanup(image_data, filenames, image_count, temp_files, neural_output_files, selected_count, NULL);
            return 1;
        }
    }
    ...
    for (int i = 0; i < selected_count; i++) {
        char command[MAX_PATH * 2];
        snprintf(command, MAX_PATH * 2, "%sneural_net.+x %s %s infer", MODULE_DIR, temp_files[i], neural_output_files[i]);
        if (system(command) != 0) {
            fprintf(stderr, "Error: Failed to execute neural_net.+x infer for %s\n", temp_files[i]);
            cleanup(image_data, filenames, image_count, temp_files, neural_output_files, selected_count, NULL);
            return 1;
        }
    }
    ...
    char command[MAX_PATH * 2];
    snprintf(command, MAX_PATH * 2, "%snoise_schedule.+x %s %s %d %d %s", MODULE_DIR, noise_input_file, noise_output_file, min_width, min_height, temp_files[0]);
    if (system(command) != 0) {
