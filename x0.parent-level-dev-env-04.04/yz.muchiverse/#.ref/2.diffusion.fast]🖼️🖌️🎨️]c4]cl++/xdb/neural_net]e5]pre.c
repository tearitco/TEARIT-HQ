            ...
            for (int x = 1; x < width - 1; x++) {
                for (int ky = -1; ky <= 1; ky++) {
                    for (int kx = -1; kx <= 1; kx++) {
                        ...
                        int idx = ((y + ky) * width + (x + kx)) * channels + c;
                        if (idx < 0 || idx >= width * height * channels) continue;
...
float compute_grad_magnitude(float *grad_kernels, int size) {
    float mag = 0.0f;
    for (int i = 0; i < size; i++) {
...
int main(int argc, char *argv[]) {
    ...
    // Load pre-trained kernels from kernels.dat
    FILE *kernel_fp = fopen("kernels.dat", "rb");
    if (!kernel_fp) {
        fprintf(stderr, "Error: Failed to open kernels.dat\n");
        return 1;
    }
    ...
    if (read != 3 * 3 * 3 || read2 != 3 * 3 * 3) {
        fprintf(stderr, "Error: Failed to read kernels.dat correctly\n");
        return 1;
    }
    ...
    if (train_mode) {
        ...
        for (int epoch = 0; epoch < NUM_EPOCHS; epoch++) {
            ...
            if (!conv1 || !attn1 || !pool1 || !conv2 || !upsampled_conv2) {
                fprintf(stderr, "Error: Memory allocation failed in epoch %d\n", epoch + 1);
                free(input); stbi_image_free(img); free(noisy_input); free(noise); free(pred_noise); free(grad_output);
                free(conv1); free(attn1); free(pool1); free(conv2); free(upsampled_conv2); fclose(loss_fp);
                return 1;
            }
            ...
            convolve(pool1, conv2, width / 2, height / 2, channels, kernels2, 0);
            group_norm(conv2, width / 2, height / 2, channels);
            upsample(conv2, upsampled_conv2, width / 2, height / 2, channels);
            memcpy(pred_noise, upsampled_conv2, size * sizeof(float));

            // Backward pass
            float loss = compute_loss(pred_noise, noise, size);
            for (int i = 0; i < size; i++) {
            ...
            // Log loss, gradients, and noise stats
            float grad_mag = compute_grad_magnitude(grad_kernels, 3 * 3 * 3);
            float grad_mag2 = compute_grad_magnitude(grad_kernels2, 3 * 3 * 3);
            ...
            free(conv1); free(attn1); free(pool1); free(conv2); free(upsampled_conv2);
        }

        // Save updated kernels back to kernels.dat
        kernel_fp = fopen("kernels.dat", "wb");
        if (!kernel_fp) {
            fprintf(stderr, "Error: Failed to open kernels.dat for writing\n");
    ...
    // Inference mode
    // Use kernels loaded from kernels.dat
    // No need to load per-image kernel file (argv[4])
    ...
    convolve(pool1, conv2, width2, height2, channels, kernels2, 0);
