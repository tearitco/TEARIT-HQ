...
void forward_pass(float *input, float *output, int width, int height, int channels, float kernels[3][3][3], float kernels2[3][3][3], FILE *log_fp) {
    ...
    float *conv1 = (float *)calloc(width * height * channels, sizeof(float));
    ...
    float *upsampled_conv2 = (float *)calloc(width * height * channels, sizeof(float));
    if (!conv1 || !pool1 || !conv2 || !upsampled_conv2) {
        fprintf(log_fp, "Error: Memory allocation failed in forward_pass\n");
...
void train_epoch(float **images, int *widths, int *heights, int num_images, int channels, float kernels[3][3][3], float kernels2[3][3][3], FILE *loss_fp) {
    for (int i = 0; i < num_images; i++) {
        ...
        fprintf(loss_fp, "Processing image %d (%dx%d, %d channels)\n", i, width, height, channels);
        if (width < 3 || height < 3) {
            fprintf(loss_fp, "Error: Image %d too small (%dx%d), skipping\n", i, width, height);
            continue;
        }
        ...
        if (!noisy_input || !noise || !pred_noise || !grad_output) {
            fprintf(loss_fp, "Error: Memory allocation failed for image %d\n", i);
        ...
        float *temp_pool1 = (float *)calloc((width / 2) * (height / 2) * channels, sizeof(float));
        if (!temp_pool1) {
            fprintf(loss_fp, "Error: Memory allocation failed for temp_pool1\n");
            free(noisy_input); free(noise); free(pred_noise); free(grad_output); free(temp_pool1);
            continue;
        }
        ...
        fprintf(loss_fp, "Image %d: Loss = %f, GradMag1 = %f, GradMag2 = %f\n", i, loss, grad_mag, grad_mag2);
        log_noise_stats(noise, size, loss_fp, "dataset_image", "target_noise");
        log_noise_stats(pred_noise, size, loss_fp, "dataset_image", "pred_noise");
        fflush(loss_fp);
