    ...
    for (int c = 0; c < channels; c++) {
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                float sum = 0.0f;
                if (use_identity && c == 0) { // Use identity kernel for R channel
                ...
                output[(y * width + x) * channels + c] = fmaxf(0.0f, fminf(sum, 1.0f)); // ReLU and clamp
            }
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Simplified bottleneck (just convolution)
    ...
    convolve(pool2, bottleneck, width2 / 2, height2 / 2, channels, kernels, 0);
    ...
    convolve(upsample2, conv3, width2, height2, channels, kernels, 0);
    ...
    remove("skip1.dat");
    combine_skip(upsample1, skip1, conv4, width, height, channels, 0.7f); // Higher skip weight
    convolve(conv4, output, width, height, channels, kernels, 1);

    // Add residual connection to input
    add_residual(output, input, size, 0.2f); // Reduced to 20%

    // Normalize output to [-1, 1] with minimum threshold
    float max_val = 0.0f;
    for (int i = 0; i < size; i++) {
        if (fabsf(output[i]) > max_val) max_val = fabsf(output[i]);
    }
    max_val = fmaxf(max_val, 0.1f); // Prevent division by small values
    for (int i = 0; i < size; i++) {
