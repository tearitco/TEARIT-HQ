...
int main(int argc, char *argv[]) {
    ...
    // Simplify variance weighting
    for (int i = 0; i < file_count; i++) variances[i] = 1.0f / file_count; // Equal weighting
    ...
    for (int t = NUM_TIMESTEPS - 1; t >= 0; t--) {
        ...
        if (!xt_minus_1) {
            fprintf(stderr, "Error: Memory allocation failed\n");
            free(xt); free(original);
            for (int i = 0; i < file_count; i++) free(noise_preds[i]);
            for (int i = 0; i < file_count; i++) free(input_files[i]);
            return 1;
        }

        // DDPM denoising: x_{t-1} = (x_t - sqrt(1-alpha_t)*eps_t)/sqrt(alpha_t)
    ...
    for (int i = 0; i < size; i++) {
        xt[i] = xt[i] * 0.8f + original[i] * 0.2f; // Reduced to 20%
    }
    ...
    if (!stbi_write_jpg(argv[2], width, height, channels, final_pixels, 90)) {
        ...
        free(xt); free(original); free(final_pixels);
        for (int i = 0; i < file_count; i++) free(noise_preds[i]);
        for (int i = 0; i < file_count; i++) free(input_files[i]);
        return 1;
    } else {
