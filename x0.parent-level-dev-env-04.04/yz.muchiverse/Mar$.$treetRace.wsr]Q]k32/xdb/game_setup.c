...
void setup_game(Player players[], int *num_players, int *game_length, float *starting_cash) {
    ...
    for (int i = 0; i < *num_players; i++) {
        ...
        char player_dir[100];
        sprintf(player_dir, "players/%s", players[i].name);
        mkdir(player_dir, 0777);

        // Create player files
        char balance_sheet_path[100];
        sprintf(balance_sheet_path, "players/%s/balance_sheet.txt", players[i].name);
        ...
        sprintf(portfolio_path, "players/%s/portfolio.txt", players[i].name);
        ...
        if (portfolio_file) {
            ...
            fprintf(portfolio_file, "%s OWNS NO BONDS ....", players[i].name);
