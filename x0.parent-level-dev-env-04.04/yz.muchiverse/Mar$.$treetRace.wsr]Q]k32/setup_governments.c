...
void generate_gov_sheet(Government gov) {
    ...
    // Dynamically allocate memory for paths to avoid buffer overflow warnings
    char *dir_path = malloc(strlen("governments/generated/") + strlen(sanitized_name) + 1);
    if (!dir_path) {
        perror("Memory allocation failed");
        return;
    }
    sprintf(dir_path, "governments/generated/%s", sanitized_name);

    // Create the government-specific directory
    create_directory(dir_path);

    // Construct file paths after directory is created
    char *file_path = malloc(strlen(dir_path) + strlen(sanitized_name) + 6);  // + "/.txt" + null terminator
    char *balance_sheet_path = malloc(strlen(dir_path) + strlen("/balance_sheet.txt") + 1);
    char *financial_profile_path = malloc(strlen(dir_path) + strlen("/financial_profile.txt") + 1);
    
    if (!file_path || !balance_sheet_path || !financial_profile_path) {
        perror("Memory allocation failed");
        free(dir_path);
        free(file_path);
        free(balance_sheet_path);
        free(financial_profile_path);
        return;
    }
    
    sprintf(file_path, "%s/%s.txt", dir_path, sanitized_name);
    sprintf(balance_sheet_path, "%s/balance_sheet.txt", dir_path);
    sprintf(financial_profile_path, "%s/financial_profile.txt", dir_path);
    ...
    char *template = read_template("presets/financial_profile_template]gov.txt");
    if (template == NULL) {
        return;
    }
    ...
    // Calculate financial data based on raw GDP and population (small scale)
    double cash_equivalents = gov.gdp * 0.1000; // 10% of GDP
    double tax_receivables = gov.gdp * 0.0500; // 5% of GDP
    double loans = gov.gdp * 0.0300; // 3% of GDP
    double physical_assets = gov.gdp * 0.0800; // 8% of GDP
    double investments = gov.gdp * 0.0700; // 7% of GDP
    double total_assets = cash_equivalents + tax_receivables + loans + physical_assets + investments;
    double public_debt = gov.gdp * 0.2000; // 20% of GDP
    double intragov_debt = gov.gdp * 0.0500; // 5% of GDP
    double pensions = gov.gdp * 0.0300; // 3% of GDP
    double env_liabilities = gov.gdp * 0.0100; // 1% of GDP
    double interest_payable = gov.gdp * 0.0050; // 0.5% of GDP
    ...
    double total_revenue = gov.gdp * 0.1500; // 15% of GDP
    double individual_tax = gov.gdp * 0.1000; // 10% of GDP
    double corporate_tax = gov.gdp * 0.0300; // 3% of GDP
    double payroll_tax = gov.gdp * 0.0200; // 2% of GDP
    double net_cost = gov.gdp * 0.1800; // 18% of GDP
    double social_security = gov.gdp * 0.0400; // 4% of GDP
    double health = gov.gdp * 0.0300; // 3% of GDP
    double interest = gov.gdp * 0.0100; // 1% of GDP
    double life_support = gov.gdp * 0.0300; // 3% of GDP
    double defense = gov.gdp * 0.0300; // 3% of GDP
    double income_security = gov.gdp * 0.0200; // 2% of GDP
    double other_spending = gov.gdp * 0.0150; // 1.5% of GDP
    ...
    // Generate balance sheet using financial profile data
    ...
    fprintf(bs_fp, "Other Assets....\t\t%s\n", phys_assets_str); // Use physical assets as "other assets"
    fprintf(bs_fp, "Total Assets.....\t\t%s\n", total_assets_str);
    fprintf(bs_fp, "Less Debt....\t\t-%s\n", pub_debt_str); // Use public debt
