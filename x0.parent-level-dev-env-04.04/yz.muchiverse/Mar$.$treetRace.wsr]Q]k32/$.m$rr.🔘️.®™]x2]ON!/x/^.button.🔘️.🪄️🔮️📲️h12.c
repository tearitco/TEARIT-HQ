...
/* Define fallback implementations for non-portable pthread functions */
#ifndef _PTHREAD_NP_AVAILABLE
#define _PTHREAD_NP_AVAILABLE
int pthread_tryjoin_np(pthread_t thread, void **value_ptr) {
    /* Check if thread is still running by attempting to send signal 0 */
...
int pthread_timedjoin_np(pthread_t thread, void **value_ptr, const struct timespec *abstime) {
    /* This is a simplified implementation - proper timed join requires more complex logic */
    /* For the purpose of this fix, we use a busy-wait approach */
    int ret;
    struct timespec current_time;
    while ((ret = pthread_tryjoin_np(thread, value_ptr)) == EBUSY) {
        if (clock_gettime(CLOCK_REALTIME, &current_time) == 0 && 
            (current_time.tv_sec > abstime->tv_sec || 
             (current_time.tv_sec == abstime->tv_sec && current_time.tv_nsec >= abstime->tv_nsec))) {
            return ETIMEDOUT;  /* Timeout */
        }
        usleep(1000);  /* Sleep 1ms before checking again */
    }
    return ret;
}
#endif /* _PTHREAD_NP_AVAILABLE */
...
// Signal handler for main thread (same as g11)
...
int main() {
    ...
    // Set up signal handler for both SIGINT and SIGTERM to ensure Ctrl+C works
    ...
    // With direct exit in signal handler, this loop will end either when all threads finish or when signal is received
    while (keep_running) {
        int all_done = 1;
        for (int i = 0; i < num_threads; i++) {
            void* status;
