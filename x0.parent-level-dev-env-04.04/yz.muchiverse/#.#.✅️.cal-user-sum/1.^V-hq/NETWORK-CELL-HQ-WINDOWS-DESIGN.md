...
- `44.xyz❤️‍🔥️00.17/&.hq-apps/network/cli_io_window.c` - the §5.3
...
| `#.#.✅️.cal-user-sum/1.^V-hq/TASKBAR-MENU-ARCHITECTURE.md` | **The definitive "add a new cell menu / new sub-app" recipe**: two-layer relay, `livedesk:open-X`, file skeleton, 7 pitfalls, test via `nav.sh`. |
| `#.#.✅️.cal-user-sum/1.^V-hq/khtpm-merge-how2.md` | **House reuse standard**: fork/exec manager vs standalone op vs `#include`-shared `.c`. |
| `#.#.✅️.cal-user-sum/1.^V-hq/HQML-DESIGN+PLANS.md` | Window chrome conventions (WM-managed + `_MOTIF_WM_HINTS` + opacity = real input; override_redirect = popups) + network-app use cases. |
| `#.#.✅️.cal-user-sum/1.^V-hq/PIECECRAFT-HQ-KHTPM-INFO-WINDOW-2026-08-30.md` | 4-step recipe to add a khtpm window for a CLI-ish app (manager binary + state file + launcher + `<module>`). |
| `44.xyz❤️‍🔥️00.17/net/TOOLING-MAP.md` + `net/` | LAN-model tooling: `connect_op.+x` (HTTP POST), `json_parser.+x`, `ollama-lan.pdl`, `qwen.sh`. |
| `44.xyz❤️‍🔥️00.17/2.muchi-verse/PAL-NET-STANDARD.txt` | P2P **file-mediated** networking standard (apps never touch sockets). |
| `44.xyz❤️‍🔥️00.17/#.DOX/pal-irc-ft-list.md` | pal-chat-irc feature list (P2P mesh, port auto-alloc). |
| `44.xyz❤️‍🔥️00.17/&.hq-apps/chat-hai/chat-hai-design.md` + `chat-hai.chtpm` | **The windowed template** our conversion models on (session sidebar, message panel, composer, `<module>` manager). |
| `44.xyz❤️‍🔥️00.17/102.agy-txt/manager/BROWSER_CONTRACT.md` | The existing browser contract to honor for the browser stub. |
| `44.xyz❤️‍🔥️00.17/014.wsr-pal💸️📌️+2/system/gl_mirror.c` (+ 205.ttg, 102.agy) | Dual terminal + X11 mirror precedent (optional for windows). |
...
| Primary planning doc | `#.#.✅️.cal-user-sum/1.^V-hq/TASKBAR-MENU-ARCHITECTURE.md` |
| Reuse standard | `#.#.✅️.cal-user-sum/1.^V-hq/khtpm-merge-how2.md` |
| Window template | `44.xyz❤️‍🔥️00.17/&.hq-apps/chat-hai/chat-hai.chtpm` + `chat-hai-design.md` |
