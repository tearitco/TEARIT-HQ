...
**RENDER-FRAME-HISTORY-DRIFT-ASSESSMENT.md** (2026-08-28) — real, confirmed drift: khtpm_entity_menu_render.c (7742 lines, 7 hardcoded "modes": db-hq/events-hq/chat-hai/palettes/bookmarks/stats-hq/taskbar-settings) still carries per-mode layout/redraw/key-handling branches, a leftover from Stage 5's binary-merge (not a deliberate design), instead of being a truly generic engine with all app-specific behavior pushed into manager processes (the way generic onclick dispatch + several managers already prove works). Real end-goal: eliminate mode branches from the renderer entirely, one mode at a time. Assessment only, no refactor code yet. Read before any render-architecture work on this file.
