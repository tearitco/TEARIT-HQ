...
8. **Rung 7 <img> Step 2 — stb_image decode + naturalWidth/complete — SHIPPED cdb51504.** `stb_image.h` decode (`b64_decode` + `stbi_load_from_memory` for `data:image/png;base64`), store via `img_set_decoded` (`g_img_decoded`), expose `naturalWidth`/`naturalHeight`/`complete`; `wcs[img-png]` 1x1 PNG `data:` URL decodes to 1x1, `wcs` 11/11 PASS.
