...
20. HARNECIENT NIGHT-track horizon items: 8 real, concrete proposals
    surfaced while writing the NIGHT_01-13 onboarding/export lesson
    ...
    random cold-start weights for Gemma-authored ones, a two-hop IRL
    bootstrap recursion (full spec: `design-docs/IRL-BOOTSTRAP-
    RECURSION-SPEC.md`), and (item 8, newest) h-ai-lab — a real
    `ai_instances_registry.txt` any FSM/attention-net/decision-pal
    registers into, plus a db-hq-style sidebar+embedded-panel GUI to
    inspect/chat with them, bundled with a real table-driven rewrite
    of `cursword_fsm.c` (today plain procedural, not table-driven) as
