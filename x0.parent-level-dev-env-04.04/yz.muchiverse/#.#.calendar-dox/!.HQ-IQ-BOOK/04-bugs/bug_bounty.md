...
**Documented per direct instruction, unresolved, for whoever picks this up next:** the `notes-pals` ghost row and tax_robot's dropped nav badge are real, reproducible, NOT fixed by the stack_n keying change above. Real next steps, in order:
1. `nav.sh hqcell 6` (not `nav 16` / not `mgrcode 4000+n` by hand) to reopen pals specifically by its `which` code, independent of global nav-badge drift.
2. With it confirmed open (`strip_ui.txt`'s `n_hqitems` in the high-20s/low-30s, not 6), dump `strip_var_hqitems.txt` immediately (same shell round-trip, no gap for a human to close it) to get the manager's OWN authoritative `HQMenuItem[]` content - settles definitively whether `notes-pals`/tax_robot's missing badge are a MANAGER/data-layer bug (wrong content in `livedesk_build_pals_menu()`'s array itself) or a RENDER-layer bug (`layout_dock_bar()`/`kh_diff_match_children()` still misbehaving after today's fix).
3. If the manager's own array is clean (as it was in the first, `asa`/`ava` investigation earlier this session), the bug is real and further downstream of today's fix in `layout_dock_bar()` or `kh_diff_match_children()` - re-examine whether `kh_diff_apply_template()` actually overwrites `label`/`sprite`/`nav_index`-relevant fields on every matched pair, since the many-to-one `target_id` key collision (every dropdown-child sibling shares one key) means `kh_diff_match_children()`'s "first unused old child with matching key" selection is still essentially arbitrary within the group - today's fix only made the Y-POSITION (`stack_n`) robust to that arbitrariness, not necessarily every other copied field.
