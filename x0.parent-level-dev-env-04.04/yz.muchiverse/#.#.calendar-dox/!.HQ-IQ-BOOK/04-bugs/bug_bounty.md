...
**Real, live-confirmed things to stop re-litigating:** the data layer (manager's `HQMenuItem[]`/`hi_N_*`) is clean; `notes-pals` is intentional; the duplicate-row collision this session's `stack_n` fix targeted is genuinely gone. The ONLY open item is `tax_robot`'s missing nav badge, isolated to `layout_dock_bar()`'s render-time `open`/`trigger` check for that one specific Elem.
