...
**Live-verified 2026-09-23 AGAINST THE ACTUAL MERGED `main` BUILD (correcting an earlier, mistaken report in this same entry - the first "longer list, working" observation that night was against the `kilo` checkout, which never had this commit built into it at all, only `main` did):** with the real merged code actually running, the owner reported the pals dropdown now shows FEWER pals than before (missing many real entries) AND no visible Cancel button - the opposite of "working." `DOCK_DROPDOWN_MAX_VISIBLE_ROWS` is hardcoded to 12 in the merged commit, and **no scrollbar thumb/track is visible**, so anything past the first 12 rows (including Cancel, which was always near/at the end of the list) is currently unreachable by a normal user - worse than the old unclipped-but-unscrollable behavior for anyone who needs a pal past position 12. **Real next step:** verify live whether `draw_generic_scrollbars()` is actually being called/rendering for this window (check `g_dock_menu_max_scroll > 0` is true when the real 190+-pal list is open - `dock_paint_menu()` only calls `generic_sbar_register()` when that's the case) and whether the thumb is being drawn but invisible (theme/color/z-order issue) vs never drawn at all. Given Cancel is now unreachable, this is more urgent than "polish" - a real usability regression until the scrollbar (or at least wheel/PageDown reachability to Cancel) is confirmed working. Real commit: `0b454145` (authored as `codex`), squashed into `main` at `e8787770` after resolving a real merge conflict with `main`'s own independently-written, textually-identical Cancel-reservation fix (see git log for the resolution).

---

## ✅ CLOSED (Cancel button half) / ⚠️ OPEN (duplicate-row half, RE-CONFIRMED REAL 2026-09-23) - "pals" dropdown - missing Cancel row (fixed) + first-row content duplicating onto row 2 (real, not a testing artifact)
...
- A live debug `fprintf` at the actual `dock_paint_menu()` draw call, dumping each row's real index/label/y as it draws, is the fastest way to settle this - same technique used successfully to crack the co-lab-hai bug tonight (see that entry above for the exact pattern).
