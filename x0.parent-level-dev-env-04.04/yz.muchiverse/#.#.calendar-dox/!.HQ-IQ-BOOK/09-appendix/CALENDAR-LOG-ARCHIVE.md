...
**🚚 `S1_HOUSE_PATH_MIGRATION.md` (2026-09-01) — DONE, both phases.**
The house has moved off its emoji-laden path (was
`🤖️🪤️🏠️/🥡️🪜️/🪜️-00.00/.../44.xyz❤️‍🔥️00.17`, a real, confirmed "Path
hidden (unsupported character)" tool problem) to
`/home/no/Desktop/github/work/NNEST-12.00/`, and the house's own
internal emoji folders are renamed too: `44.xyz❤️‍🔥️00.17` →
`44.xyz.01.00`, `#.#.✅️.cal-user-sum` → `#.#.calendar-dox` (**this doc
now lives under the new `#.#.calendar-dox/` name**). The OLD location
under `NNEST-11.17/` still exists as a rollback copy, not yet retired.
Read this doc's §7 for the real execution findings before assuming
any path in an older doc/handoff is still current.
...
**🪤 `44.xyz.01.00/HOUSE_CODE_PITFALLS.md` (2026-08-31)** — real,
...
-4. **CENTROID_GOLD_STD.md — GOLD STANDARD, adopted 2026-08-31 (house root,
   `44.xyz.01.00/`)** — the real, final rendering architecture for every
...
5b2. **`taskbar-tpmos-parallel-refactor.md` + `taskbar-history-txt-migration-investigation.md` —
   ...
   `au11-hq/TASKBAR-FRAME-UNIFICATION-HANDOFF.md` (under `44.xyz.01.00/`) for the frame-sharing
...
| `44.xyz.01.00/LINUX_ROUNDTRIP.md` | Linux return-leg status: Mach-O quarantine (76 files), house-wide recompile (44/44 PASS), manual rebuilds (treetRace, hm_assert, apply_theme_op), verification, livedesk smoke-test | When returning from macOS to Linux (or any roundtrip); read before relaunching |
...
7. **No hardcoded UIs, ever** (2026-08-24): every window/menu/layout is DATA and a GENERATED
   ...
   mechanics: `44.xyz.01.00/!.HOUSE_STDS.md` §K. **This extends to any fixed vocabulary of
...
8. **No UI element without a mirror keyboard path** (2026-08-25): every real mouse interaction in
   ...
   `44.xyz.01.00/!.HOUSE_STDS.md` §K.6.
...
| Leg | Status | Doc |
|---|---|---|
| Linux (canonical) | ✅ always current | `44.xyz.01.00/!.HOUSE_STDS.md` |
...
| macOS (return leg) | ⚠️ last verified pass 2026-08-23 — **stale, see pending doc below** | `44.xyz.01.00/LINUX_ROUNDTRIP.md`, `yz.muchiverse/ROUNDTRIP_FIX.md` |
...
1. **Events ladder** — `44.xyz.01.00/#.ref/menu/event.commands.remaining.txt`
...
2. **Palettes after that** — new categories now registered in
   `44.xyz.01.00/#.ref/menu/palletes/pallets-help.txt`:
   ...
   `44.xyz.01.00/#.ref/menu/tiling-palettes-chemistry.txt`.
...
`44.xyz.01.00/!.HOUSE_STDS.md`: store→generated-artifact rule, context windows OLD vs NEW
...
**2026-08-23** (Linux return leg roundtrip fixed: 76 Mach-O binaries quarantined, house-wide recompile 44/44 PASS, livedesk smoke-test passed; full trail in `44.xyz.01.00/LINUX_ROUNDTRIP.md` + `yz.muchiverse/ROUNDTRIP_FIX.md`) by Kilo
...
`44.xyz.01.00/MAC-CONVERSION-STATUS.md`) by opencode (ox-alpha)
...
and `44.xyz.01.00/MAC_COMPAT.md`, the handoff brief for the macOS leg) by opencode (ox-alpha)
...
scenarios fixed, full trail in `44.xyz.01.00/completed-sym-list.md` + `sim-smell-fix.md`;
