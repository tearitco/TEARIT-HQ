        ...
        if (commit_file) {
            ...
            while (fgets(line, sizeof(line), commit_file)) {
                if (strncmp(line, "tree ", 5) == 0) {
                    ...
                    tree_hash[strlen(tree_hash) - 1] = '\0';
                    break;
                }
            }
            fclose(commit_file);
...
void fork_command(char *fork_path) {
    // Create fork directory
    if (mkdir(fork_path, 0777) != 0) {
        perror("mkdir");
        return;
    }
    ...
    chdir(fork_path);
    init_command(1); // Bare repository
    chdir(cwd);

    // Copy objects
    char local_objects[] = ".gitlet/objects";
    char fork_objects[512];
    sprintf(fork_objects, "%s/.gitlet/objects", fork_path);
    DIR *local_dir = opendir(local_objects);
    if (!local_dir) {
        perror("opendir");
        return;
    ...
    while ((ent = readdir(local_dir)) != NULL) {
        if (ent->d_type == DT_REG) {
            ...
            char fork_obj[512];
            sprintf(fork_obj, "%s/%s", fork_objects, ent->d_name);

            FILE *src = fopen(local_obj, "r");
            if (!src) continue;
            FILE *dst = fopen(fork_obj, "w");
            if (!dst) {
                fclose(src);
                continue;
            }
    ...
    // Copy refs
    char local_refs[] = ".gitlet/refs";
    char fork_refs[512];
    sprintf(fork_refs, "%s/.gitlet/refs", fork_path);
    DIR *refs_dir = opendir(local_refs);
    if (!refs_dir) {
        perror("opendir");
        return;
    }
    while ((ent = readdir(refs_dir)) != NULL) {
        if (ent->d_type == DT_DIR && strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0) {
            char local_subdir[512];
            sprintf(local_subdir, "%s/%s", local_refs, ent->d_name);
            char fork_subdir[512];
            sprintf(fork_subdir, "%s/%s", fork_refs, ent->d_name);
            mkdir(fork_subdir, 0777);

            DIR *subdir = opendir(local_subdir);
            if (!subdir) continue;
            struct dirent *sub_ent;
            while ((sub_ent = readdir(subdir)) != NULL) {
                if (sub_ent->d_type == DT_REG) {
                    char local_ref[512];
                    sprintf(local_ref, "%s/%s", local_subdir, sub_ent->d_name);
                    char fork_ref[512];
                    sprintf(fork_ref, "%s/%s", fork_subdir, sub_ent->d_name);
                    FILE *src = fopen(local_ref, "r");
                    if (!src) continue;
                    FILE *dst = fopen(fork_ref, "w");
                    if (!dst) {
                        fclose(src);
                        continue;
                    }
    ...
    // Copy HEAD
    char local_head[] = ".gitlet/HEAD";
    char fork_head[512];
    sprintf(fork_head, "%s/.gitlet/HEAD", fork_path);
    FILE *src_head = fopen(local_head, "r");
    if (src_head) {
        FILE *dst_head = fopen(fork_head, "w");
        if (dst_head) {
            char c;
            while ((c = fgetc(src_head)) != EOF) {
                fputc(c, dst_head);
            }
            fclose(dst_head);
        }
    ...
    // Add fork as a remote in local config
    FILE *config = fopen(".gitlet/config", "a");
    if (config) {
        fprintf(config, "[remote \"fork\"]\n\turl = %s\n", fork_path);
        fclose(config);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "init") == 0) {
        ...
        if (argc < 3) {
            fprintf(stderr, "Usage: gitlet fork <fork_path>\n");
            return 1;
        }
        fork_command(argv[2]);
    } else {
