...
void add_command(char *filename) {
    ...
    unsigned long content_hash = hash((unsigned char *)buffer);

    char blob_path[256];
    sprintf(blob_path, ".gitlet/objects/%lu", content_hash);
    FILE *blob_file = fopen(blob_path, "w");
    fwrite(buffer, 1, length, blob_file);
    fclose(blob_file);

    FILE *index_file = fopen(".gitlet/index", "a");
    fprintf(index_file, "%s %lu\n", filename, content_hash);
    fclose(index_file);
...
void commit_command(char *commit_message) {
    ...
    // Create tree object
    unsigned long tree_hash = hash((unsigned char *)tree_content);
    char tree_path[256];
    sprintf(tree_path, ".gitlet/objects/%lu", tree_hash);
    ...
    sprintf(commit_content, "tree %lu\nparent %s\nDate: %s\n%s\n", tree_hash, parent_hash, ctime(&now), commit_message);

    unsigned long commit_hash = hash((unsigned char *)commit_content);
    char commit_path[256];
    sprintf(commit_path, ".gitlet/objects/%lu", commit_hash);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "init") == 0) {
        ...
        fprintf(stderr, "Unknown command: % différence\n", command);
        return 1;
    }
