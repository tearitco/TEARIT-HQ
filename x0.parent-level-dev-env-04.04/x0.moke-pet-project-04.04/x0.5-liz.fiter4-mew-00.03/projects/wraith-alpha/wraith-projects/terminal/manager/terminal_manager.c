...
/*
 * TPMOS compliance note: every project's layout must load a manager, the
 * same way HTML loads a <script> tag. This project's real hot-path logic
 * (KEY_PRESSED accumulation, shell exec, cwd, scrollback) lives entirely
 * in ops/wraith_project_input.c, invoked fresh on every keystroke by the
 * top-level wraith-alpha_manager.c -- same split as fs_manager.c. This
 * manager's only job is to exist as the loaded entry point and trigger
 * one initial render before the first keypress arrives.
 */
...
void resolve_paths() {
    ...
    snprintf(project_session_dir, sizeof(project_session_dir),
             "%s/projects/wraith-alpha/wraith-projects/terminal/session", project_root);
...
void trigger_initial_render() {
    ...
    snprintf(op_path, sizeof(op_path),
             "%s/projects/wraith-alpha/wraith-projects/terminal/ops/+x/wraith_project_input.+x", project_root);
    ...
    if (pid == 0) {
        ...
        snprintf(project_dir, sizeof(project_dir),
                 "%s/projects/wraith-alpha/wraith-projects/terminal", project_root);
...
int main() {
    ...
    log_debug("Wraith Terminal Manager Started");
    ...
    log_debug("Wraith Terminal Manager init complete (hot path owned by ops/wraith_project_input.c)");
