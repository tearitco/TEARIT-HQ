...
/*
 * TPMOS compliance note: every project's layout must load a manager,
 * the same way HTML loads a <script> tag — regardless of whether the
 * project's hot-path logic already lives entirely in a synchronous op.
 * fs's ops/wraith_project_input.c already owns all real state mutation
 * (scan_entries/process_map_key/etc run unconditionally on every
 * invocation, not gated on a manager-provided initial state), so this
 * manager's only job is to exist as the loaded entry point and trigger
 * one initial render before the first keypress arrives — matching
 * piececraft-wraith_manager.c's "manager is init-only" pattern.
 */
...
void resolve_paths() {
    ...
    snprintf(project_session_dir, sizeof(project_session_dir),
             "%s/projects/wraith-alpha/wraith-projects/fs/session", project_root);
...
void trigger_initial_render() {
    ...
    snprintf(op_path, sizeof(op_path),
             "%s/projects/wraith-alpha/wraith-projects/fs/ops/+x/wraith_project_input.+x", project_root);
    ...
    if (pid == 0) {
        ...
        snprintf(project_dir, sizeof(project_dir),
                 "%s/projects/wraith-alpha/wraith-projects/fs", project_root);
...
int main() {
    ...
    log_debug("Wraith FS Manager Started");
    ...
    log_debug("Wraith FS Manager init complete (hot path owned by ops/wraith_project_input.c)");
