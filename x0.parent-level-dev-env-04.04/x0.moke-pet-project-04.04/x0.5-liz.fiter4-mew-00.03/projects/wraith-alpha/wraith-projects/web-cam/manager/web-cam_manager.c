 ...
 *
 * NOTE (web-cam specific): this project's ops/wraith_project_input.c
 * reads/writes session state through pieces/chtpm/ops/lib/
 * tpmos_share_kvp_runtime.c (POSIX shared memory, /tpmos_share_kvp_v1),
 * not plain file I/O. Confirmed safe to trigger from here: that runtime
 * is self-bootstrapping (tpmos_share_kvp_ensure_daemon() auto-starts the
 * backing daemon if the shared-memory segment isn't attached yet), and
 * this manager invokes the op the exact same way wraith-alpha's own
 * run_active_project_input_op() already does per keypress — no new
 * timing/ordering risk introduced. See wra-mana-checklist.txt.
 */
...
void resolve_paths() {
    ...
    snprintf(project_session_dir, sizeof(project_session_dir),
             "%s/projects/wraith-alpha/wraith-projects/web-cam/session", project_root);
...
void trigger_initial_render() {
    ...
    snprintf(op_path, sizeof(op_path),
             "%s/projects/wraith-alpha/wraith-projects/web-cam/ops/+x/wraith_project_input.+x", project_root);
    ...
    if (pid == 0) {
        ...
        snprintf(project_dir, sizeof(project_dir),
                 "%s/projects/wraith-alpha/wraith-projects/web-cam", project_root);
...
int main() {
    ...
    log_debug("Web Cam Manager Started");
    ...
    log_debug("Web Cam Manager init complete (hot path owned by ops/wraith_project_input.c, uses shared-memory KVP DB)");
