...
static char *expand_raw_layout_fragments(const char *src);
...
static void sync_wraith_alpha_state(void) {
     ...
     * Reload it after generic parser state so the rendered index cannot drift. */
    load_state_file("projects/wraith-alpha/session/alpha_state.txt", NULL);
}

static void debug_focus_state(const char *phase, int idx) {
    FILE *dbg = fopen("parser.out", "a");
    if (!dbg) return;
    if (idx >= 0 && idx < element_count) {
        UIElement *el = &elements[idx];
        fprintf(dbg, "DEBUG %s: idx=%d type=%s label=%s onClick=%s href=%s active_index=%d focus_index=%d\n",
                phase,
                idx,
                el->type,
                el->label,
        ...
        fprintf(dbg, "DEBUG %s: idx=%d out_of_range active_index=%d focus_index=%d\n",
                phase, idx, active_index, focus_index);
    }
    fclose(dbg);
}

static void dispatch_focused_element_enter(const char *phase) {
    if (focus_index < 0 || focus_index >= element_count) {
        return;
    }

    UIElement *el = &elements[focus_index];
    debug_focus_state(phase, focus_index);
    ...
    if (strcmp(el->type, "cli_io") == 0) {
        ...
        if (!handle_desktop_action_command(el->onClick)) send_command(el->onClick);
...
void resolve_root() {
    ...
    /* Aggressive Search-Up for location_kvp anchor */
    char search_path[MAX_PATH];
    strncpy(search_path, cwd, MAX_PATH-1);
    
    bool found = false;
    for (int i = 0; i < 5; i++) { /* Max 5 levels up */
        char kvp_try[MAX_PATH];
        snprintf(kvp_try, sizeof(kvp_try), "%s/pieces/locations/location_kvp", search_path);
        FILE *kvp = fopen(kvp_try, "r");
        if (kvp) {
            char line[2048];
            while (fgets(line, sizeof(line), kvp)) {
                if (strncmp(line, "project_root=", 13) == 0) {
                    ...
                    v[strcspn(v, "\n\r")] = 0;
                    if (strlen(v) > 0) {
                        strncpy(project_root_path, v, MAX_PATH-1);
                        found = true;
                        break;
                    }
                }
    ...
    if (!found) strncpy(project_root_path, cwd, MAX_PATH-1);
...
static bool append_fragment(char **dst, size_t *cap, size_t *len, const char *frag) {
    size_t frag_len;

    if (!dst || !*dst || !cap || !len || !frag) {
        return false;
    }

    frag_len = strlen(frag);
    if (*len + frag_len + 1 > *cap) {
        size_t new_cap = *cap;
        while (*len + frag_len + 1 > new_cap) {
            new_cap *= 2;
        }
...
static char *expand_raw_layout_fragments(const char *src) {
    const struct {
        ...
        { "${desktop_shell_markup}", "desktop_shell_markup" },
        { "${piece_methods}", "piece_methods" }
    };
    size_t cap;
    size_t len = 0;
    char *out;
    ...
    cap = strlen(src) + MAX_VAR_VALUE + 1;
    if (cap < 4096) {
        cap = 4096;
    }
    ...
    while (*p) {
        ...
        for (size_t i = 0; i < sizeof(raw_vars) / sizeof(raw_vars[0]); i++) {
            size_t needle_len = strlen(raw_vars[i].needle);
            if (strncmp(p, raw_vars[i].needle, needle_len) == 0) {
                if (!append_fragment(&out, &cap, &len, get_var(raw_vars[i].name))) {
                    free(out);
                    return NULL;
                }
                p += needle_len;
...
void load_state_file(const char* rel_path, const char* prefix) {
    ...
    /* DEBUG: Log variable loading attempt */
    {
        FILE *audit = fopen("parser.out", "a");
        if (audit) {
            fprintf(audit, "DEBUG: load_state_file trying: %s (prefix=%s) - %s\n", 
                    path, prefix ? prefix : "none", f ? "SUCCESS" : "FAILED");
            fclose(audit);
        }
    }
            ...
            if (eq) {
                *eq = '\0';
                ...
                if (var_name) { 
                    ...
                    /* DEBUG: Log successful variable load */
                    FILE *audit2 = fopen("parser.out", "a");
                    if (audit2) {
                        fprintf(audit2, "  -> Loaded: %s=%s\n", var_name, trim_pmo(eq + 1));
                        fclose(audit2);
                    }
...
static void save_desktop_ui_state(bool collapsed, bool fullscreen, bool open, int window_count) {
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s/projects/wraith-alpha/session/desktop_ui_state.txt", project_root_path);
    FILE *f = fopen(path, "w");
    if (!f) return;
    fprintf(f, "desktop_window_count=%d\n", window_count < 0 ? 0 : window_count);
    fprintf(f, "desktop_window_1_open=%s\n", open ? "true" : "false");
    fprintf(f, "desktop_window_1_collapsed=%s\n", collapsed ? "true" : "false");
    fprintf(f, "desktop_window_1_minimized=%s\n", collapsed ? "true" : "false");
    fprintf(f, "desktop_window_1_minimized_visible=%s\n", collapsed ? "true" : "false");
    fprintf(f, "desktop_window_1_restore_title=%s\n", collapsed && window_count > 0 ? "Terminal #1" : "");
    fprintf(f, "fold_terminal=%s\n", collapsed ? "folded" : "open");
    fprintf(f, "desktop_window_1_body_visible=%s\n", (open && !collapsed) ? "true" : "false");
    fprintf(f, "desktop_window_1_collapse_glyph=%s\n", collapsed ? "+" : "-");
    fprintf(f, "desktop_window_1_fullscreen=%s\n", fullscreen ? "true" : "false");
    fprintf(f, "desktop_window_1_mode=%s\n", fullscreen ? "full" : "half");
    fprintf(f, "desktop_window_1_halfscreen_visible=%s\n", fullscreen ? "false" : "true");
    fclose(f);
}

static void apply_desktop_ui_state(bool collapsed, bool fullscreen, bool open, int window_count) {
    char terminal_title[64];
    char count_buf[32];
    if (window_count > 0) snprintf(terminal_title, sizeof(terminal_title), "Terminal #%d", window_count);
    else snprintf(terminal_title, sizeof(terminal_title), "Terminal");

    snprintf(count_buf, sizeof(count_buf), "%d", window_count < 0 ? 0 : window_count);
    set_var("desktop_window_count", count_buf);
    set_var("desktop_window_1_open", open ? "true" : "false");
    set_var("desktop_window_1_collapsed", collapsed ? "true" : "false");
    set_var("desktop_window_1_minimized", collapsed ? "true" : "false");
    set_var("desktop_window_1_minimized_visible", collapsed ? "true" : "false");
    set_var("desktop_window_1_restore_title", collapsed ? terminal_title : "");
    set_var("fold_terminal", collapsed ? "folded" : "open");
    set_var("desktop_window_1_body_visible", (open && !collapsed) ? "true" : "false");
    set_var("desktop_window_1_collapse_glyph", collapsed ? "+" : "-");
    set_var("desktop_window_1_fullscreen", fullscreen ? "true" : "false");
    set_var("desktop_window_1_mode", fullscreen ? "full" : "half");
    set_var("desktop_window_1_halfscreen_visible", fullscreen ? "false" : "true");
    set_var("desktop_window_1_hidden", open ? "false" : "true");
    set_var("desktop_window_1_title", terminal_title);
    set_var("desktop_focused_window_title", open ? terminal_title : "Desktop");
    save_desktop_ui_state(collapsed, fullscreen, open, window_count);
}
...
static bool handle_desktop_action_command(const char* cmd) {
    ...
    if (strncmp(cmd, "SET_ACTIVE:", 11) != 0 &&
        strncmp(cmd, "DESKTOP_ACTION:", 15) != 0) {
        return false;
    }

    if (asprintf(&queue_path, "%s/projects/wraith-alpha/session/desktop_actions.txt", project_root_path) != -1) {
        queue_fp = fopen(queue_path, "a");
        if (queue_fp) {
            fprintf(queue_fp, "COMMAND: %s\n", cmd);
            fclose(queue_fp);
        }
        free(queue_path);
    }
    
    /* THIN THEATER: Delegate all work to the Manager via History Inbox */
    inject_command(cmd);
    
    FILE *audit = fopen("parser.out", "a");
    if (audit) {
        fprintf(audit, "DELEGATED ACTION TO MANAGER: %s\n", cmd);
        fclose(audit);
    }
    return true;
}
...
static void set_project_id_from_layout(void) {
    const char *project_root_marker;
    const char *layout_marker;
    const char *project_pdl_marker;
    const char *start;
    const char *end;
    char proj_name[256];
    size_t len;

    project_root_marker = strstr(current_layout, "projects/");
    if (!project_root_marker) {
        return;
    }

    start = project_root_marker + 9;
    layout_marker = strstr(start, "/layouts/");
    project_pdl_marker = strstr(start, "/project.pdl");

    end = NULL;
    if (layout_marker && (!project_pdl_marker || layout_marker < project_pdl_marker)) {
        end = layout_marker;
    } else if (project_pdl_marker) {
        end = project_pdl_marker;
    } else {
        end = strrchr(start, '/');
    }
    ...
    if (len >= sizeof(proj_name)) {
        len = sizeof(proj_name) - 1;
    }
...
void load_vars() {
    ...
    /* WRAITH-ALPHA: Load alpha_state.txt */
    load_state_file("projects/wraith-alpha/session/alpha_state.txt", NULL);
    load_state_file("projects/wraith-alpha/manager/gui_state.txt", NULL);

    /* Pitfall #31 (project_id drift): alpha_state.txt above carries
       project_id=wraith-alpha, whose load clobbers the layout-derived
       project_id set earlier (via set_project_id_from_layout()). For an
       href-reached project layout (e.g.
       wraith-projects/settings/window-geom) this makes the project-state
       load below read the WRONG project's manager/state.txt, so that
       project's own ${vars} render blank. Re-assert the layout-derived
       project_id here -- after the last state load that can drift it -- and
       refresh proj_id (its pointer may now point at the clobbered value).
       Zero effect for alpha-shell.chtpm: it re-derives the same
       "wraith-alpha". */
    if (strstr(current_layout, "projects/") != NULL) {
        set_project_id_from_layout();
    }
    proj_id = get_var("project_id");

    if (strlen(get_var("desktop_mode")) == 0) set_var("desktop_mode", "desktop");
    if (strlen(get_var("desktop_window_count")) == 0) set_var("desktop_window_count", "0");
    if (strlen(get_var("desktop_default_window_id")) == 0) set_var("desktop_default_window_id", "terminal");
    if (strlen(get_var("desktop_default_window_title")) == 0) set_var("desktop_default_window_title", "Terminal");
    if (strlen(get_var("desktop_focused_window_id")) == 0) set_var("desktop_focused_window_id", "terminal");
    if (strlen(get_var("desktop_focused_window_title")) == 0) set_var("desktop_focused_window_title", "Desktop");
    if (strlen(get_var("desktop_launcher_count")) == 0) set_var("desktop_launcher_count", "1");
    if (strlen(get_var("desktop_launcher_1_id")) == 0) set_var("desktop_launcher_1_id", "terminal");
    if (strlen(get_var("desktop_launcher_1_title")) == 0) set_var("desktop_launcher_1_title", "Terminal");
    if (strlen(get_var("desktop_launcher_1_kind")) == 0) set_var("desktop_launcher_1_kind", "launcher");
    if (strlen(get_var("desktop_window_1_id")) == 0) set_var("desktop_window_1_id", "terminal");
    if (strlen(get_var("desktop_window_1_title")) == 0) set_var("desktop_window_1_title", "Terminal");
    if (strlen(get_var("desktop_window_1_kind")) == 0) set_var("desktop_window_1_kind", "terminal");
    if (strlen(get_var("desktop_window_1_open")) == 0) set_var("desktop_window_1_open", "false");
    if (strlen(get_var("desktop_window_1_collapsed")) == 0) set_var("desktop_window_1_collapsed", "false");
    if (strlen(get_var("desktop_window_1_minimized")) == 0) set_var("desktop_window_1_minimized", "false");
    if (strlen(get_var("desktop_window_1_minimized_visible")) == 0) set_var("desktop_window_1_minimized_visible", "false");
    if (strlen(get_var("desktop_window_1_restore_title")) == 0) set_var("desktop_window_1_restore_title", "");
    if (strlen(get_var("desktop_window_1_hidden")) == 0) set_var("desktop_window_1_hidden", "true");
    if (strlen(get_var("desktop_window_1_fullscreen")) == 0) set_var("desktop_window_1_fullscreen", "false");
    if (strlen(get_var("desktop_window_1_mode")) == 0) set_var("desktop_window_1_mode", "half");
    if (strlen(get_var("desktop_window_1_halfscreen_visible")) == 0) set_var("desktop_window_1_halfscreen_visible", "true");
    {
        bool terminal_collapsed = false;
        bool terminal_minimized = false;
        bool terminal_open = (strcmp(get_var("desktop_window_1_open"), "true") == 0);
        int terminal_count = atoi(get_var("desktop_window_count"));
        if (terminal_count < 0) terminal_count = 0;
        if (strcmp(get_var("desktop_window_1_collapsed"), "true") == 0) terminal_collapsed = true;
        if (strcmp(get_var("fold_terminal"), "folded") == 0) terminal_collapsed = true;
        if (strcmp(get_var("desktop_window_1_minimized"), "true") == 0) terminal_minimized = true;
        if (strcmp(get_var("desktop_window_1_minimized_visible"), "true") == 0) terminal_minimized = true;
        if (!terminal_minimized && terminal_count > 0 && !terminal_open) terminal_minimized = true;
        set_var("desktop_window_1_collapsed", terminal_collapsed ? "true" : "false");
        set_var("desktop_window_1_minimized", terminal_minimized ? "true" : "false");
        set_var("desktop_window_1_minimized_visible", terminal_minimized ? "true" : "false");
        set_var("fold_terminal", terminal_collapsed ? "folded" : "open");
        set_var("desktop_window_1_body_visible", (terminal_open && !terminal_collapsed && !terminal_minimized) ? "true" : "false");
        set_var("desktop_window_1_collapse_glyph", terminal_collapsed ? "+" : "-");
        set_var("desktop_window_1_mode", strcmp(get_var("desktop_window_1_fullscreen"), "true") == 0 ? "full" : "half");
        set_var("desktop_window_1_halfscreen_visible", strcmp(get_var("desktop_window_1_fullscreen"), "true") == 0 ? "false" : "true");
        if (terminal_count > 0) {
            char terminal_title[MAX_LABEL_LEN];
            snprintf(terminal_title, sizeof(terminal_title), "Terminal #%d", terminal_count);
            set_var("desktop_window_1_title", terminal_title);
            set_var("desktop_focused_window_title", terminal_title);
            set_var("desktop_window_1_open", terminal_open ? "true" : "false");
            set_var("desktop_window_1_hidden", terminal_open ? "false" : "true");
            set_var("desktop_window_1_restore_title", terminal_minimized ? terminal_title : "");
        } else {
            set_var("desktop_window_1_title", "Terminal");
            set_var("desktop_focused_window_title", "Desktop");
            set_var("desktop_window_1_open", "false");
            set_var("desktop_window_1_hidden", "true");
            set_var("desktop_window_1_minimized", "false");
            set_var("desktop_window_1_minimized_visible", "false");
            set_var("desktop_window_1_restore_title", "");
        }
    }
    ...
    /* Desktop UI state must win over generic GUI state so chrome toggles survive reloads. */
    load_state_file("projects/wraith-alpha/session/desktop_ui_state.txt", NULL);

    /* Re-normalize after the final desktop UI load so stale minimized flags from
     * earlier state files do not override the current open/closed projection. */
    {
        bool terminal_collapsed = false;
        bool terminal_minimized = false;
        bool terminal_open = (strcmp(get_var("desktop_window_1_open"), "true") == 0);
        ...
        /* Minimize is a hidden-from-desktop state; if the window is open, any
         * stale minimized flag must be cleared so the launcher label stays plain. */
        if (!terminal_open && terminal_count > 0) terminal_minimized = true;
        ...
        if (terminal_count > 0) {
            ...
            snprintf(terminal_title, sizeof(terminal_title), "Terminal #%d", terminal_count);
            set_var("desktop_window_1_title", terminal_title);
            set_var("desktop_focused_window_title", terminal_open ? terminal_title : "Desktop");
            set_var("desktop_window_1_open", terminal_open ? "true" : "false");
            set_var("desktop_window_1_hidden", terminal_open ? "false" : "true");
            set_var("desktop_window_1_restore_title", terminal_minimized ? terminal_title : "");
        } else {
    ...
    /* WRAITH-ALPHA: Environment Debug */
    char cwd[MAX_PATH];
    if (getcwd(cwd, sizeof(cwd))) set_var("PARSER_CWD", cwd);
    set_var("PARSER_ROOT", project_root_path);

    /* WRAITH-ALPHA: High Priority State Load (Final Say) */
    load_state_file("projects/wraith-alpha/session/alpha_state.txt", NULL);

    /* WRAITH-ALPHA: Keyboard debug is loaded here so it survives every render pass.
     * compose_frame() may re-enter load_vars(), so do not set this later in render flow. */
    {
...
void inject_raw_key(int code) {
    FILE *fp;
    bool wraith_session_layout = (strstr(current_layout, "projects/wraith-alpha/") != NULL ||
                                  strstr(current_layout, "projects/wraith/") != NULL);
    ...
    if (wraith_session_layout) {
        char *path = build_path_malloc("projects/wraith-alpha/session/history.txt");
        fp = fopen(path, "a");
        if (fp) { fprintf(fp, "%d\n", code); fclose(fp); }
        free(path);
        return;
...
void inject_command(const char* cmd) {
    ...
    if (strlen(project_id) > 0 && strcmp(project_id, "template") != 0) {
        char *path = NULL;
        if (asprintf(&path, "%s/projects/%s/session/history.txt", project_root_path, project_id) != -1) {
            fp = fopen(path, "a");
            if (fp) { fprintf(fp, fmt, cmd); fclose(fp); }
            free(path);
            return;
...
void send_command(const char* cmd) {
    ...
    if (handle_desktop_action_command(cmd)) return;
...
void parse_chtm() {
    ...
    load_vars();
    sync_wraith_alpha_state();
    char* content = read_file_to_string(current_layout); 
    if (!content) return;
    char *expanded_content = expand_raw_layout_fragments(content);
    if (expanded_content) {
        free(content);
        content = expanded_content;
    }
    
    /* FIX: DO NOT substitute vars here! We must keep ${var} in the tokens 
       so that render_element can substitute them live on every frame. */
    int tc; 
    Token* tokens = tokenize(content, &tc); 
    free(content);
    ...
    for (int i = 0; i < tc && element_count < MAX_ELEMENTS; i++) {
        ...
        if (t->type == TOKEN_TEXT) {
            /* Keep module launch substitution as it's a structural event */
            ...
            /* target_id-keyed restore: lets multiple cli_io fields on one
             * layout persist independently across re-parses (parse_chtm()
             * rebuilds elements[] from scratch every frame), instead of all
...
void render_element(int idx, char* frame, int* p_global_counter, int* p_scoped_counter) {
    ...
    if (strcmp(el->type, "br") == 0) strcat(frame, "\n");
        ...
        char *line = NULL;
        if (chrome_mode) {
            char compact_label[MAX_LABEL_LEN];
            strncpy(compact_label, scratch_substituted, MAX_LABEL_LEN - 1);
            compact_label[MAX_LABEL_LEN - 1] = '\0';
            char *trimmed_label = trim_pmo(compact_label);
            if (display_num > 0) {
                if (strlen(trimmed_label) <= 1) asprintf(&line, "%s%d[%s]", pref, display_num, trimmed_label);
                else asprintf(&line, "%s%d.[%s]", pref, display_num, trimmed_label);
            } else {
                asprintf(&line, "%s[%s]", pref, trimmed_label);
            }
        } else {
...
void compose_frame() {
    ...
    char wraith_desktop_sig[MAX_VAR_VALUE];
    snprintf(
        wraith_desktop_sig,
        sizeof(wraith_desktop_sig),
        "%s|%s|%s|%s|%s|%s|%s|%s|%s",
        get_var("active_gui_index"),
        get_var("desktop_window_count"),
        get_var("desktop_open_window_count"),
        get_var("desktop_minimized_window_count"),
        get_var("desktop_active_window_visible"),
        get_var("desktop_active_window_title"),
        get_var("desktop_focused_window_title"),
        get_var("desktop_projection_signature"),
        get_var("desktop_restore_count")
    );

    bool active_changed = (strcmp(active_id, last_active_id) != 0);
    bool methods_changed = (strcmp(methods_raw, last_methods_raw) != 0);
    bool wraith_desktop_changed = (strcmp(wraith_desktop_sig, last_wraith_desktop_sig) != 0);

    if (active_changed || methods_changed || wraith_desktop_changed || wait_for_view_change) {
        ...
        if (wraith_desktop_changed) strncpy(last_wraith_desktop_sig, wraith_desktop_sig, MAX_VAR_VALUE - 1);
...
void handle_mouse(int btn, int x, int y) {
    ...
    snprintf(project_hist, sizeof(project_hist), "projects/%s/session/history.txt", project_id);
...
void process_key(int key) {
    auto void debug_focus_state(const char *phase, int idx) {
        FILE *dbg = fopen("parser.out", "a");
        if (!dbg) return;
        if (idx >= 0 && idx < element_count) {
            UIElement *el = &elements[idx];
    ...
    if (active_index == -1) {
        if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
            ...
            if (digit_accum > 0) {
                ...
                if (focus_index >= 0 && focus_index < element_count) {
                    UIElement *el = &elements[focus_index];
                    debug_focus_state("ENTER-NORMAL", focus_index);
        ...
        if (key == ESC_KEY || key == JOY_BUTTON_8) { 
            ...
            if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
                ...
                if (focus_index >= 0 && focus_index < element_count) {
                    ...
                    debug_focus_state("ENTER-ACTIVE", focus_index);
                    ...
                    } else if (strcmp(child_el->onClick, "INTERACT") == 0) {
                        active_index = focus_index;
                        clear_nav_on_next = true;
                        export_active_index();
                    } else if (strlen(child_el->onClick) > 0) {
                        if (!handle_desktop_action_command(child_el->onClick)) send_command(child_el->onClick);
                        wait_for_view_change = true;
        ...
        else if ((key == 10 || key == 13 || key == JOY_BUTTON_0) && strcmp(el->type, "cli_io") != 0) {
            dispatch_focused_element_enter("ENTER-ACTIVE-FALLBACK");
            clear_nav_on_next = true;
        }
        else if (strcmp(el->type, "cli_io") == 0) {
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (dirty || clear_nav_on_next) { 
            /* RE-WRAITH: Ensure we refresh state before composing if dirty. */
            var_count = 0;
            load_vars();
            sync_wraith_alpha_state();
            compose_frame(); 
