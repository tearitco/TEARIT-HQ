// projects/gem-xo/ops/src/json_parser.c - Robust dot-notation JSON parser
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char* argv[]) {
    ...
    char* val = find_in_json(raw, argv[2]);
    if (!val) {
        const char *trimmed = skip_ws(raw);
        if (strncmp(trimmed, "```json", 7) == 0 || strncmp(trimmed, "```", 3) == 0) {
            char *fenced = strdup(trimmed);
            char *start = strstr(fenced, "```json");
            char *json_start = start ? start + 7 : fenced;
            char *end = strstr(json_start, "```");
            if (end) *end = '\0';
            val = find_in_json(json_start, argv[2]);
            free(fenced);
        }
    }
