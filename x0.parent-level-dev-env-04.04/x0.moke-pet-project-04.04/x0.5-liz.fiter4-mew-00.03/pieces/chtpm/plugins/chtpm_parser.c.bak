...
static char last_focus_layout[MAX_PATH] = "";
static bool focus_restore_ready = false;
static bool last_completion_visible = false;
static bool startup_input_blank = true;
...
static void sync_cli_input_from_gui_state(void) {
    ...
    for (int i = 0; i < element_count; i++) {
        ...
        if (strcmp(elements[i].id, "input_text") == 0) {
            if (state_input && state_input[0] != '\0') {
    ...
    if (f) {
        ...
        while (fgets(line, MAX_LINE, f)) {
            ...
            if (eq) {
                *eq = '\0';
                char *var_name = NULL;
                char *key = trim_pmo(line);
                if (prefix) asprintf(&var_name, "%s_%s", prefix, key);
                else asprintf(&var_name, "%s", key);
                if (var_name) {
                    char *value = eq + 1;
                    if (strcmp(key, "input_text") != 0) {
                        value = trim_pmo(value);
                    } else {
                        /* Preserve prompt whitespace exactly. */
                        value[strcspn(value, "\r\n")] = '\0';
                    }
...
void save_to_gui_state(const char* name, const char* value) {
    ...
    char *path = NULL;
    /* gem-xo reads the manager-owned gui_state.txt. Keep writes on the same
     * file so the input buffer does not get restored from stale legacy state. */
...
void load_vars() {
    ...
    if (strlen(proj_id) > 0 && strcmp(proj_id, "template") != 0) {
        ...
        snprintf(project_state_path, sizeof(project_state_path), "projects/%s/session/state.txt", proj_id);
    ...
    if (strlen(proj_id) > 0 && strcmp(proj_id, "template") != 0) {
        ...
        char *g_paths[] = {
            ...
            "projects/%s/pieces/manager/gui_state.txt",
            "projects/%s/session/state.txt",
    ...
    if (strlen(get_var("module_path")) == 0 && strlen(proj_id) > 0 && strcmp(proj_id, "template") != 0) {
        ...
        // Construct module path from convention: projects/<id>/pieces/manager/+x/<id>_manager.+x
        char module_path[1024];
        snprintf(module_path, sizeof(module_path), "projects/%s/pieces/manager/+x/%s_manager.+x", proj_id, proj_id);
...
void parse_chtm() {
    ...
    for (int i = 0; i < tc && element_count < MAX_ELEMENTS; i++) {
        ...
        if (t->type == TOKEN_TEXT) {
            ...
            /* Initialize cli_io input_buffer from the session state.
             * input_text is manager-owned; do not restore it from cli_buffers.txt,
             * which is only used for legacy buffered input types. */
            if (strcmp(t->tag_name, "cli_io") == 0) {
                if (strstr(el->id, "input_text")) {
                    const char* state_input = get_var("input_text");
                    if (state_input && state_input[0] != '\0') {
                        strncpy(el->input_buffer, state_input, sizeof(el->input_buffer) - 1);
...
void initialize_focus() {
    ...
    bool layout_changed = (strcmp(last_focus_layout, current_layout) != 0);
    if (layout_changed) {
        focus_restore_ready = false;
        strncpy(last_focus_layout, current_layout, sizeof(last_focus_layout) - 1);
        last_focus_layout[sizeof(last_focus_layout) - 1] = '\0';
    }

    if (focus_restore_ready && saved_idx_str && strlen(saved_idx_str) > 0) {
        int saved_idx = atoi(saved_idx_str);
        if (saved_idx > 0) {
            for (int i = 0; i < element_count; i++) {
                if (elements[i].interactive_idx == saved_idx && is_navigable(i)) {
                    focus_index = i;
                    focus_restore_ready = true;
                    return;
                }
            }
        }
