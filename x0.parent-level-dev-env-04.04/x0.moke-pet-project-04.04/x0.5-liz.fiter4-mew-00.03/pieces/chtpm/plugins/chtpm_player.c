...
static void preload_layout_from_marker(void) {
    char *layout_ch = build_path_malloc("pieces/display/layout_changed.txt");
    if (!layout_ch) return;

    FILE *lf = fopen(layout_ch, "r");
    if (lf) {
        char line[MAX_PATH];
        char last_line[MAX_PATH] = "";
        while (fgets(line, sizeof(line), lf)) {
            char *trimmed = trim_pmo(line);
            if (trimmed && *trimmed) {
                strncpy(last_line, trimmed, sizeof(last_line) - 1);
...
int main(int argc, char **argv) {
    ...
    preload_layout_from_marker();
    signal(SIGINT, handle_sigint);
    parse_chtm();
    initialize_focus();
    usleep(50000);
    handle_module_output();
    compose_frame();
    struct stat st; 
