...
SYNC_OP="./pieces/chtpm/ops/+x/sync_project_routes_op.+x"
if [ -x "$SYNC_OP" ]; then
    echo "Refreshing project route registry..."
    "$SYNC_OP" || echo "WARNING: route sync failed"
else
    echo "WARNING: $SYNC_OP not found; run compile_all.sh first if routes are stale."
fi
...
> projects/xo-pet-v1/pieces/manager/gui_state.txt
> projects/xo-pet-v1/pieces/manager/active_target.txt
> projects/xo-pet-v1/pieces/manager/sim_control.txt
