        ...
        if [ -d "${nested_dir}${subdir}/" ]; then
            ...
            for src in "${nested_dir}${subdir}/"*.c; do
                ...
                if [ "$subdir" = "ops/src" ]; then
                    mkdir -p "${nested_dir}ops/+x"
                    tmp_runtime_bin="${nested_dir}ops/+x/.${bin_name}.+x.tmp"
                    cp "${nested_dir}${subdir}/+x/${bin_name}.+x" "$tmp_runtime_bin"
                    mv -f "$tmp_runtime_bin" "${nested_dir}ops/+x/${bin_name}.+x"
                fi
            done
        fi
    done
